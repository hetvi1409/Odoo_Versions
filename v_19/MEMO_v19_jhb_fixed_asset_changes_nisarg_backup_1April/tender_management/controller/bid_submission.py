...
class TenderInformation(http.Controller):
    ...
    def tender_rfq_details(self, page=1, **kwargs):
        """RFQ"""
        rfq_model = request.env['tender.tender']
        domain = []
        domain = [('type', '=', 'rfq'), ('website_published', '=', True)]
        # if kwargs.get('searchRefNo'):
        #     domain.append(('name', 'ilike', kwargs.get('searchRefNo')))
        print(domain, 'aaaaaaa', kwargs)
        # Pagination setup
        items_per_page = 5
        total = rfq_model.search_count(domain)
        print(total)

        pager = request.website.pager(url='/tenders-rfqs/', total=total, page=page, step=items_per_page, scope=10)
        print(pager['offset'], 'dddd')

        rfqs = rfq_model.search(domain, limit=items_per_page, offset=pager['offset'])
    ...
    def tender_rfp_details(self, page=1, **kwargs):
        ...
        rfq_model = request.env['tender.tender']
        domain = []
        domain = [('type', '=', 'rfp'), ('website_published', '=', True)]
        if kwargs.get('searchRefNo'):
            domain.append(('name', 'ilike', kwargs.get('searchRefNo')))
        print(domain, 'aaaaaaa', kwargs)
        # Pagination setup
        ...
        rfps = rfq_model.search(domain, limit=items_per_page, offset=pager['offset'])
    ...
    @http.route(['/tenders-archive', '/tenders-archive/page/<int:page>'], auth='public', website=True, csrf=True,)
    def tender_archive_information_list(self, page=1, **kwargs):
        """RFI"""
        rfq_model = request.env['tender.tender']
        domain = []
        domain = [('state', '=', 'closed')]
        ...
        pager = request.website.pager(url='/tenders-archive/', total=total, page=page, step=items_per_page, scope=10)
    ...
    @http.route(['/bid-status'], auth="public", website=True, csrf=True)
    def tender_bid_status(self, page=1, **kwargs):
        """Bid status"""
        bid_model = request.env['tender.bid']
        domain = []
        if kwargs.get('searchRefNo'):
            domain.append(('bid_name', 'ilike', kwargs.get('searchRefNo')))
        print(domain, 'aaaaaaa', kwargs)
        # Pagination setup
        items_per_page = 10
        total = bid_model.search_count(domain)

        pager = request.website.pager(url='/bid-status/', total=total,
                                      page=page, step=items_per_page, scope=10)

        bid = bid_model.search(domain, limit=items_per_page,
                                offset=pager['offset'])
    ...
    @http.route('/tender/timer', type='jsonrpc', auth='public', csrf=False)
    ...
    def download_all_rfq_documents(self, tender_id, **kw):
        ...
        with zipfile.ZipFile(zip_buffer, 'w',
            ...
            for document in tender_id.rfq_document_ids:

...
class clouds_folder(models.Model):
    ...
    @api.depends("access_user_ids", "access_group_ids.user_ids", "parent_id.restricted_users")
    ...
    def check_access_rule(self, operation):
        ...
        """
        if self.env.context("ir_attachment_security"):
        ...
        * if a folder does not have res_model and res_id, we check an access to parent recursively  
        ...
        if self:
            ...
            for res_model, res_id, rule_id, restricted_users in self._cr.fetchall():            
                ...
                if rule_id and mode == "write" and not self.env.context("ir_attachment_security"):
    ...
    def create(self, vals_list):
        ...
        snapshots_vals_list = records.mapped(lambda rec: {
            "folder_id": rec.id,
            "snapshot": self.env.context("save_snapshot_state") \
    ...
    def write(self, values):
        ...
        if values.get("active") is not None and not self.env.context("no_recursive_deactivation"):
    ...
    def search_read(self, domain=None, fields=None, offset=0, limit=None, order=None, **read_kwargs):
        ...
        """
        check_rights_mode = self.env.context("write_mode") and "write" or "read"
        ...
        print(limit, 'limit.....')
        print(order, 'order.....')
        print(read_kwargs, 'read_kwargs.....')
        print(check_rights_mode, 'check_rights_mode.....')
        ids = super(clouds_folder, self).search_read(read_kwargs,offset=offset, limit=limit, order=order, domain=domain)
        print(ids,'ids....')
        if self.env.is_superuser():
            return super().search_read(domain, offset, limit, order,read_kwargs)
        # if not ids:
        ...
        self._cr.execute("""SELECT id, access_res_model, access_res_id, restricted_users 
                                FROM clouds_folder WHERE id IN %s""",
                             [tuple(ids)])
        restricted_folders = []
        ...
        if len(orig_ids) == limit and len(result) < self._context.get("need", limit):
            ...
            result.extend(
                self.with_context(need=need).search_read(offset=offset + len(orig_ids), limit=limit, order=order, count=False)[:limit - len(result)]
            )
        return list(result)

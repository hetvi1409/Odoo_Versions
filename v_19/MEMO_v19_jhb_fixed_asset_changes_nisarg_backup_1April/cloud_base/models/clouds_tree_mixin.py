...
class clouds_tree_mixin(models.AbstractModel):
    ...
    def _check_node_recursion(self):
        ...
        """
        if not self.env.context("prepare_queue_context") and not self._check_recursion():
    ...
    def action_return_mass_actions(self):
        ...
        for mass_action in mass_action_ids:
            if not mass_action.groups_id or (self.env.user.group_ids & mass_action.groups_id):

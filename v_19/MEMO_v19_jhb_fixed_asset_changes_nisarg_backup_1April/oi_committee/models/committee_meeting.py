...
class CommitteeMeeting(models.Model):
    ...
    date_start = fields.Datetime(readonly=False, states=_states, required=True)
    date_end = fields.Datetime(readonly=False, states=_states, required=True)

    attendance_ids = fields.Many2many('committee.member', required=True, domain="[('committee_id','=', committee_id)]",
                                      readonly=False, states=_states)

    subject = fields.Char(readonly=False, states=_states, required=True)
    description = fields.Text(readonly=False, states=_states)
    report = fields.Html(readonly=False, states=_states)

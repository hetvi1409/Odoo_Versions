...
class CommitteeType(models.Model):
    ...
    committee_ids = fields.One2many('committee', 'type_id', domain=[('state', '=', 'active')],
                                    string='Active Committee')
    ...
    def _update_members(self):
        ...
        for record in records:
            ...
            record.group_id.write({'user_ids': [(6, 0, user_ids)]})

    def _calc_xml_id(self):
        xml_ids = self.get_external_id()
        for record in self:
            record.xml_id = xml_ids[record.id] if xml_ids else False

...
class JMCDocumentWizard(models.TransientModel):
    ...
    folder_id = fields.Many2one('documents.document', string="Workspace", required=True)
    res_model = fields.Char('Resource Model')
    res_id = fields.Integer('Resource ID')
    # folder_ids = fields.Many2many('documents.document', compute="_compute_folder_ids")
    ...
    def action_upload_document(self):
        ...
        jmc_folder = self.env['documents.document'].search([('name', '=', property_name.jmc_number),
                                                          ('folder_id', '=', self.folder_id.id)])
        if not jmc_folder:
            jmc_folder = self.env['documents.document'].create({
                'name': property_name.jmc_number,
                'folder_id': self.folder_id.id
            })
        document_folder = self.env['documents.document'].search([
            ('name', '=', self.folder),
            ('folder_id', '=', jmc_folder.id)])
        if not document_folder:
            document_folder = self.env['documents.document'].create({
                'name': self.folder,
                'folder_id': jmc_folder.id
            })
        final_folder = document_folder
        if self.transaction_folder:
            transaction_folder = self.env['documents.document'].search([
                ('name', '=', self.transaction_folder),
                ('folder_id', '=', document_folder.id)])
            if not transaction_folder:
                transaction_folder = self.env['documents.document'].create({
                    'name': self.transaction_folder,
                    'folder_id': document_folder.id
                })
            final_folder = transaction_folder
        if self.committee_folder:
            committee_folder = self.env['documents.document'].search([
                ('name', '=', self.committee_folder),
                ('folder_id', '=', document_folder.id)])
            if not committee_folder:
                committee_folder = self.env['documents.document'].create({
                    'name': self.committee_folder,
                    'folder_id': document_folder.id
                })
            final_folder = committee_folder
        if self.tender_folder:
            tender_folder = self.env['documents.document'].search([
                ('name', '=', self.tender_folder),
                ('folder_id', '=', document_folder.id)])
            if not tender_folder:
                tender_folder = self.env['documents.document'].create({
                    'name': self.tender_folder,
                    'folder_id': document_folder.id
                })
            final_folder = tender_folder
        if self.executive_folder:
            executive_folder = self.env['documents.document'].search([
                ('name', '=', self.executive_folder),
                ('folder_id', '=', document_folder.id)])
            if not executive_folder:
                executive_folder = self.env['documents.document'].create({
                    'name': self.executive_folder,
                    'folder_id': document_folder.id
                })
            final_folder = executive_folder
        if self.agreement_folder:
            agreement_folder = self.env['documents.document'].search([
                ('name', '=', self.agreement_folder),
                ('folder_id', '=', document_folder.id)])
            if not agreement_folder:
                agreement_folder = self.env['documents.document'].create({'type':'folder',
                    'name': self.agreement_folder,
                    'folder_id': document_folder.id
                })

...
class CalendarEvent(models.Model):
    ...
    document_folder_id = fields.Many2one('documents.document', string="Folder",domain="[('type', '=', 'folder'), ('shortcut_document_id', '=', False)]")
        ...
        if vals.get('attachment_ids'):
            ...
            for rec in vals.get('attachment_ids'):
                if rec[0] == 4:
                    ...
                    meeting_folder = self.env['documents.document'].search(
                        [('name', '=', self.name),
                         ('folder_id', '=', main_folder.id),('type', '=', 'folder')
                         ])
                    if not meeting_folder:
                        meeting_folder = self.env['documents.document'].create({
                            'name': self.name,
                    ...
                    document_folder = self.env['documents.document'].search(
                        [('name', '=', "Document Library"),
                         ('folder_id', '=', meeting_folder.id),('type', '=', 'folder')
                         ])
    ...
    def create_mom_document(self):
        ...
        mom_document = self.env['documents.document'].search(
            [('name', '=', "MOM"),
             ('folder_id', '=', meeting_folder.id),('type', '=', 'folder')
             ])
        if not mom_document:
            mom_document = self.env['documents.document'].create({
                'name': "MOM",
                'folder_id': meeting_folder.id,
                'type': 'folder'
            })
    ...
    def create_agenda_document(self):
        ...
        agenda_document = self.env['documents.document'].search(
            [('name', '=', "Agenda"),
             ('folder_id', '=', meeting_folder.id),('type', '=', 'folder')
             ])
        if not agenda_document:
            agenda_document = self.env['documents.document'].create({
                'name': "Agenda",
                'folder_id': meeting_folder.id,
                'type': 'folder'
            })

...
class RecruitmentRequisition(models.Model):
    ...
    position_type = fields.Selection([
        ...
        ('new', 'New Position'),
    ], required=True, default='existing', tracking=True)
    job_grade = fields.Char(tracking=True)
    ...
    hr_coordinator_id = fields.Many2one('res.users')
    ...
    unpublish_date = fields.Date(string='Unpublish Date')
    ...
    def write(self, vals):
        # Update previous_incumbent when position_type or job_id changes
    ...
    @api.depends('requisition_form_file', 'job_description_file', 'organogram_file')
    def _compute_document_complete(self):
        for rec in self:
            rec.document_complete = bool(
                rec.organogram_file
            )
    ...
    def _validate_hcm_checklist(self):
        self.ensure_one()
        if not self.job_description_file:
            raise ValidationError(_("Job description must be attached."))
        if not self.organogram_file:
    ...
    def _create_job_position_if_needed(self):
        ...
        job_vals = {
            ...
            'company_id': self.company_id.id,
            'requisition_id': self.id,
            'description': self.business_justification or False,

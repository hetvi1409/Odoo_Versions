...
class Memo(models.Model):
    ...
    qa_user_ids = fields.Many2many('res.users', 'memo_qa_user_rel', 'memo_id', 'user_id', string='QA Users',
                                   compute='_compute_qa_user_ids', copy=False)
    can_delete_lines = fields.Boolean(compute="_compute_can_delete_lines",string="Can Delete QA/Recommender Lines")
    admin_it_group = fields.Many2many('res.users', 'memo_admin_it_user_rel', 'memo_id', 'user_id',
                                        string='Admin IT Users', compute='_compute_admin_it_group',compute_sudo=True, )

    def _compute_admin_it_group(self):
        for rec in self:
            it_group = self.env.ref('e_system.group_it_team')
            admin_group = self.env.ref('base.group_system')
            if it_group in self.env.user.group_ids or admin_group in self.env.user.group_ids:
                rec.admin_it_group = [(6, 0, [self.env.user.id])]

    def _compute_can_delete_lines(self):
        it_group = self.env.ref('e_system.group_it_team')  # replace with your XML ID
        for rec in self:
            rec.can_delete_lines = not (it_group in self.env.user.group_ids)
    ...
    def _compute_allowed_user_ids(self):
        for rec in self:
            ...
            # Approver logic
            if rec.approver_id:
                # Approver should always see memo when approval is pending or done/rejected
                if rec.state in ['pending_approval', 'approved', 'rejected', 'change_requested']:
    ...
    def _compute_qa_user_ids(self):
        for memo in self:
            qas = set()
            qa_users = memo.quality_assurance_ids.mapped('user_id')
            if qa_users:
                qas.update(qa_users.ids)
            memo.qa_user_ids = [(6, 0, list(qas))]
    ...
    def create(self, vals_list):
        ...
        # Create all records at once (vectorized)
        records = super().create(vals_list)

        # Assign folders for each record
        for rec in records:
            rec.sudo()._assign_document_folder()
        ...
        # }
        #
        # submission_type = res.submission_type or self.env.context.get('default_submission_type')
        # existing_number = res.memo_number
        # if not existing_number or existing_number == '':
        #     seq_code = sequence_codes.get(submission_type)
        #     if seq_code:
        #         next_number = self.env['ir.sequence'].sudo().next_by_code(seq_code)
        #         res.memo_number = next_number or '/'
        #     else:
        #         _logger.warning(f"No sequence code defined for submission_type: {submission_type}")
        # else:
        #     _logger.info(f"Memo number already set: {existing_number}")
    ...
    def write(self, vals):
        ...
        for rec in self:
            ...
            if not evidence and not self.env.user.has_group('e_system.group_it_team'):
    ...
    def action_sign_request(self):
        if not self.requester_id:
            ...
            # if self.submission_type == 'circular':
            #     raise ValidationError("Please add Responsible Officer before sign the circular.")
            if self.submission_type == 'procurement':
    ...
    def action_reset_to_draft(self):
        for memo in self:
            # Security Check
            it_group = self.env.ref('e_system.group_it_team', raise_if_not_found=False)
            admin_group = self.env.ref('base.group_system', raise_if_not_found=False)

            # Check user permissions
            user = self.env.user
            is_creator = user == memo.create_uid
            is_requester = user == memo.requester_id
            is_qa_user = user.id in memo.quality_assurance_ids.mapped('user_id.id')
            is_it = it_group in user.group_ids if it_group else False
            is_admin = admin_group in user.group_ids if admin_group else False

            # Allowed users check
            if not (is_admin or is_it or is_creator or is_requester or is_qa_user):
                raise ValidationError(_(
                    "Only the Creator, Requester, QA users, IT Team, or Administrators can reset this memo to Draft."
                ))

            # Reset QA lines
            memo.quality_assurance_ids.sudo().with_context(reset_to_draft=True).write({
                'status': '',
                'comment': '',
                'date': False,
                'sign_initials': False,
            ...
            # Reset Recommender lines
            memo.recommender_ids.sudo().with_context(reset_to_draft=True).write({
                'status': '',
                'comment': '',
                'date': False,
                'sign_initials': False,
            ...
            # Reset Approver Signature (if your model has fields like these)
            memo.approver_sign = False
            memo.approve_comment = False
            memo.approve_date = False

            # Reset memo state
            memo.state = 'draft'

            # Log in chatter
            memo.message_post(body=_("Memo has been reset to Draft. All signatures and approvals were removed."))
...
class MemoApprover(models.Model):
    ...
    def _compute_is_editable(self):
        ...
        it_group = self.env.ref('e_system.group_it_team')  # replace if custom
        for rec in self:
            rec.is_editable = (
                    current_user.id in [
                rec.memo_request_id.create_uid.id if rec.memo_request_id.create_uid else False,
                rec.memo_request_id.requester_id.id if rec.memo_request_id.requester_id else False,
                rec.user_id.id if rec.user_id else False,
            ] or it_group in current_user.group_ids
            )
    ...
    def write(self, vals):
        # Skip validations when resetting memo to draft
        if self.env.context.get('reset_to_draft'):
            return super(MemoApprover, self).write(vals)
        trigger_next = False
        for record in self:
    ...
    def _onchange_template_id(self):
        ...
        if self.env.context.get('sign_directly_without_mail') or sign_item_role_user:
            ...
            if len(roles) == 1 and signer_ids:
                signer_ids[0][2]['partner_id'] = roles[0].user_id.partner_id if roles[0].user_id and roles[
                    0].user_id.partner_id else default_signer
            elif not roles:
...
class SignRequest(models.Model):
    ...
    def _sign(self):
        ...
        if not self._check_is_encrypted():
            ...
            if not self.reference_doc:
                _logger.warning(
                    "SignRequest %s: reference_doc is missing after _send_completed_document(); skipping record posting/linking.",
                    self.id)
            else:
                ...
                try:
                    ...
                    _logger.exception("SignRequest %s: error while posting signed document to reference_doc: %s",
                                      self.id, e)
    ...
    def _send_completed_document(self):
        ...
        if not self.reference_doc:
            _logger.warning(
                "SignRequest %s: reference_doc missing when sending completed document; skipping record post.", self.id)
        else:
            try:
                if self.reference_doc._name != 'memo.document.upload':
...
class MemoQualityAssurance(models.Model):
    ...
    def write(self, vals):
        # Skip validations when resetting memo to draft
        if self.env.context.get('reset_to_draft'):
            return super(MemoQualityAssurance, self).write(vals)
        trigger_next = False
        for record in self:
            if 'sign_initials' in vals and record.sudo().user_id.id != self.env.uid:
                raise ValidationError("You cannot sign on behalf of another user.")

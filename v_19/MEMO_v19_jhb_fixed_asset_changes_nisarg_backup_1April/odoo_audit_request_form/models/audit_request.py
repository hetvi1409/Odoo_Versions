...
class CustomAuditRequest(models.Model):
    ...
    def create(self, vals_list):
        for vals in vals_list:
            sequence_code = 'custom.audit.request'
            vals['sequence_name'] = self.env['ir.sequence'].next_by_code(
                sequence_code)
        res = super(CustomAuditRequest, self).create(vals_list)
        return res
    ...
    def custom_audit_action_confirm(self):
        ...
        template = self.env.ref('odoo_audit_request_form.custom_email_template_audit_request_confirm_probc')
    ...
    def custom_action_audit_send_mail(self):
       ...
       try:
           ...
           template_id = self.env.ref('odoo_audit_request_form.custom_email_send_template_all_audit_request_state_probc')
       except ValueError:
           template_id = False
       ctx = dict()
       ctx.update({

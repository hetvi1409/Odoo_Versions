...
class IrModelFieldSelectionCustom(models.Model):
    ...
    def _get_custom_fields_selection(self):
        # cr = self._cr
        if odoo.tools.sql.table_exists(self.env.cr, self._table):
            self.env.cr.execute("select distinct field_id from ir_model_fields_selection_custom where active")

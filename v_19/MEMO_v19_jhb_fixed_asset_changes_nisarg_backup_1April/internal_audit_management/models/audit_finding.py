...
class AuditFinding(models.Model):
    ...
    def create(self, vals_list):
        res = super(AuditFinding, self).create(vals_list)
        for vals in vals_list:
            task_id = vals['task_id']
            task = self.env['project.task'].browse(task_id)
            if task.exists():
                task.audit_finding_ids = [(4, res.id)]
        return res
    # @api.model
    # def create(self, vals):
    #     """Create the sub folders based on the templates"""
    #     # vals['finding_number'] = self.env['ir.sequence'].next_by_code(
    #     #     'audit.finding')
    #     res = super(AuditFinding, self).create(vals)
    #     if vals.get('project_id') and vals.get('task_id'):

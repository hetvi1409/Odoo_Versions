...
class MinutesOfMeeting(models.Model):
    ...
    def create(self, vals_list):
        for vals in vals_list:
            sequence_code = 'minutes.meeting'
            vals['sequence_no'] = self.env['ir.sequence'].next_by_code(
                sequence_code)
        res = super(MinutesOfMeeting, self).create(vals_list)
        return res
    # @api.model
    # def create(self, vals):
    #     vals['sequence_no'] = self.env['ir.sequence'].next_by_code('minutes.meeting')
    #     res = super(MinutesOfMeeting, self).create(vals)
    #     return res
    ...
    def get_form_url(self):
        ...
        base_url = self.env['ir.config_parameter'].sudo().get_param(
            'web.base.url')
        Urls = urls.url_join(base_url, 'web#id=%s&model=minutes.meeting&view_type=form' % self.id)
        return Urls

    def action_review(self):
        """method for review"""
        mail_template = self.env.ref(
            'internal_audit_management.email_template_mom_review')
    ...
    def action_2nd_review(self):
        """Second Review"""
        mail_template = self.env.ref(
            'internal_audit_management.email_template_mom_sec_review')
        mail_template.send_mail(self.id, force_send=True)
    ...
    def action_approve(self):
        """Method for approve"""
        mail_template = self.env.ref(
            'internal_audit_management.email_template_mom_approve')
        mail_template.send_mail(self.id, force_send=True)
        self.state = 'approved'
    ...
    def action_mark_discrepancy(self):
        ...
        mail_template = self.env.ref(
            'internal_audit_management.email_template_mom_rejected')
        mail_template.send_mail(self.id, force_send=True)
        self.state = 'rejected'

    def action_send_back_review(self):
        mail_template = self.env.ref(
            'internal_audit_management.email_template_mom_reverted')
        mail_template.send_mail(self.id, force_send=True)
        return {
            ...
            'context': {
                'default_minutes_meeting_id': self.id
            },
            'view_mode': 'form',
            'target': 'new',
        }
    ...
    def action_send_back_new(self):
        ...
        mail_template = self.env.ref(
            'internal_audit_management.email_template_mom_send_bak')
        mail_template.send_mail(self.id, force_send=True)
        self.state = 'preparer'
    ...
    def action_excel_report(self):
        ...
        return {
            ...
            'data': {'model': self._name,
                     ...
                     'output_format': 'xlsx',
                     'report_name': 'Minutes of Meeting',
                     },
            'report_type': 'xlsx',
        }
    ...
    def get_xlsx_report(self, data, response):
        ...
        sheet.merge_range('E2:L3', 'Minutes Of Meeting', head)

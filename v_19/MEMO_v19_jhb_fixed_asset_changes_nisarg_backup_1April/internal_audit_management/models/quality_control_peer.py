...
class QualityControlInternalPeerReview(models.Model):
    ...
    def create(self, vals_list):
        for vals in vals_list:
            sequence_code = 'quality.control.peer'
            vals['sequence_no'] = self.env['ir.sequence'].next_by_code(
                sequence_code)
        res = super(QualityControlInternalPeerReview, self).create(vals_list)
        for record in res:
    ...
    #     record = super(QualityControlInternalPeerReview, self).create(vals)
    #     if not self.folder_id:
    #         folder = self._create_folder(record.name)
    #         record.folder_id = folder.id
...
class QualityControlOutsourcedPeerReviews(models.Model):
    ...
    def create(self, vals_list):
        for vals in vals_list:
            sequence_code = 'quality.control.outsourced.peer'
            vals['sequence_no'] = self.env['ir.sequence'].next_by_code(
                sequence_code)
        res = super(QualityControlOutsourcedPeerReviews, self).create(vals_list)
        for record in res:
            if not self.folder_id:
                folder = self._create_folder(record.name)
                record.folder_id = folder.id
    ...
    #     record = super(QualityControlOutsourcedPeerReviews, self).create(vals)
    #     if not self.folder_id:
    #         folder = self._create_folder(record.name)
    #         record.folder_id = folder.id
    #     if 'attachment_ids' in vals:

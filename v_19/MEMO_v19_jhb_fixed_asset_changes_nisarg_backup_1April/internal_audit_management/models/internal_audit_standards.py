...
class InternalAuditStandards(models.Model):
    """Financial Statements And Performance Report"""
    _name = 'audit.standards'
    _description = "Internal Audit Standards"
    _rec_name = "name"
    ...
    name = fields.Char(string="Name", help="Name", required=True, tracking=True)
    ...
    def create(self, vals_list):
        for vals in vals_list:
            sequence_code = 'audit.standards'
            vals['sequence_no'] = self.env['ir.sequence'].next_by_code(
                sequence_code)
        res = super(InternalAuditStandards, self).create(vals_list)
        return res
    # @api.model
    # def create(self, vals):
    #     vals['sequence_no'] = self.env['ir.sequence'].next_by_code('audit.standards')
    #     res = super(InternalAuditStandards, self).create(vals)
    #     return res
    ...
    def get_form_url(self):
        ...
        base_url = self.env['ir.config_parameter'].sudo().get_param(
            'web.base.url')
        Urls = urls.url_join(base_url, 'web#id=%s&model=audit.standards&view_type=form' % self.id)
        return Urls

    def action_review(self):
        """method for review"""
        mail_template = self.env.ref(
            'internal_audit_management.email_template_audit_standards_review')
    ...
    def action_2nd_review(self):
        """Second Review"""
        mail_template = self.env.ref(
            'internal_audit_management.email_template_aud_stand_sec_review')
        mail_template.send_mail(self.id, force_send=True)
    ...
    def action_approve(self):
        """Method for approve"""
        mail_template = self.env.ref(
            'internal_audit_management.email_template_aud_stand_approve')
        mail_template.send_mail(self.id, force_send=True)
        self.state = 'approved'
    ...
    def action_mark_discrepancy(self):
        ...
        mail_template = self.env.ref(
            'internal_audit_management.email_template_aud_stand_rejected')
        mail_template.send_mail(self.id, force_send=True)
        self.state = 'rejected'

    def action_send_back_review(self):
        mail_template = self.env.ref('internal_audit_management.email_template_aud_stand_reverted')
        mail_template.send_mail(self.id, force_send=True)
        return {
            ...
            'context': {
                'default_audit_standards_id': self.id
            },
            'view_mode': 'form',
            'target': 'new',
        }
    ...
    def action_update(self):
        mail_template = self.env.ref('internal_audit_management.email_template_audit_standards_review')
        mail_template.send_mail(self.id, force_send=True)
        self.state = self.previous_state

    def action_send_back_new(self):
        """Action send back to new"""
        mail_template = self.env.ref('internal_audit_management.email_template_aud_stand_send_bak')
        mail_template.send_mail(self.id, force_send=True)
        self.state = 'preparer'

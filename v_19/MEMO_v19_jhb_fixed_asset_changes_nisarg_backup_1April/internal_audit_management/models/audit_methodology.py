...
class AuditMethodology(models.Model):
    ...
    def create(self, vals):
        # Create the record
        record = super(AuditMethodology, self).create(vals)
    ...
    def _create_document_records(self):
        ...
        for record in self:
            for attachment in record.document_ids:
                documents = document.search([('attachment_id', '=', attachment.id)])
                if not documents:
                    document.create({
                        'name': attachment.name,
                        'attachment_id': attachment.id,
                        'folder_id': self.env.ref('internal_audit_management.documents_audit_methodology_folder').id,
    ...
    def create(self, values):
        ...
        if res.audit_id:
            res.audit_id.audit_methodology_id = res.id
        return res

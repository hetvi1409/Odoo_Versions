...
class QualityControlInternalReviewFramework(models.Model):
    ...
    def create(self, vals_list):
        for vals in vals_list:
            sequence_code = 'internal.quality.control'
            vals['sequence_no'] = self.env['ir.sequence'].next_by_code(
                sequence_code)
        res = super(QualityControlInternalReviewFramework, self).create(vals_list)
        for record in res:
            ...
            if 'review_attachment_ids' in vals_list:
                self._sync_documents(vals_list['review_attachment_ids'])
        return res

    # @api.model
    # def create(self, vals):
    #     vals['sequence_no'] = self.env['ir.sequence'].next_by_code('internal.quality.control')
    #     record = super(QualityControlInternalReviewFramework, self).create(vals)
    #     if not self.folder_id:
    #         folder = self._create_folder(record.name)
    #         record.folder_id = folder.id

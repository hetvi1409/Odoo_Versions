...
class AuditFollowUp(models.Model):
    ...
    def create(self, vals_list):
        for vals in vals_list:
            sequence_code = 'audit.followup'
            vals['audit_number'] = self.env['ir.sequence'].next_by_code(
                sequence_code)
        res = super(AuditFollowUp, self).create(vals_list)
        return res
    # @api.model
    # def create(self, vals):
    #     """Create the sub folders based on the templates"""
    #     vals['audit_number'] = self.env['ir.sequence'].next_by_code(
    #         'audit.followup')
    #     res = super(AuditFollowUp, self).create(vals)
    #     return res
...
class AuditFollowUpSetting(models.Model):
    ...
    def create(self, vals):
        ...
        res = super(AuditFollowUpSetting, self).create(vals)
        return res
...
class AuditCapturing(models.Model):
    ...
    @api.model
    def create(self, vals):
        vals['follow_up_num'] = self.env['ir.sequence'].next_by_code(
            'audit.capturing')
        res = super(AuditCapturing, self).create(vals)
        return res

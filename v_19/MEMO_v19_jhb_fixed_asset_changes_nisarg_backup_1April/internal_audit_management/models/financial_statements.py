...
class FinancialStatementsPerformanceReport(models.Model):
    ...
    user_reviewer_2_ids = fields.Many2one('res.users', string="Second Reviewer",
                                           tracking=True)
    is_current_user_reviewer2 = fields.Boolean(compute='_compute_is_reviewer2', store=False)

    @api.depends('user_reviewer_2_ids')
    def _compute_is_reviewer2(self):
    ...
    def create(self, vals_list):
        for vals in vals_list:
            sequence_code = 'financial.statement'
            vals['sequence_no'] = self.env['ir.sequence'].next_by_code(
                sequence_code)
        res = super().create(vals_list)
        for rec in res:
            rec._create_document_records()
        return res

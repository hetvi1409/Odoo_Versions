...
class MasterSearch(models.Model):
    ...
    search_by = fields.Selection(
        [('any', 'Any'),
         ...
         ('property_lease', 'Property Portfolio Leases'),
         ('informal_trade', 'Informal Trade Leases'),
         ('outdoor_lease', 'Outdoor Advertisement Leases'),
         ],
    ...
    property_ids = fields.Many2many('building',
                                    'master_search_building_details_rel')
    asset_count = fields.Integer(string="Asset Count",
                                   compute="_compute_asset_count")
    asset_ids = fields.Many2many('account.asset',
                                 'master_search_asset_details_rel')
    outdoor_ids = fields.Many2many('outdoor.advertisement',
                                   'master_search_outdoor_details_rel')
    outdoor_count = fields.Integer(string="Outdoor Count",
                                 compute="_compute_outdoor_count")
    property_lease_ids = fields.Many2many('rental.contract',
                                          'master_search_property_lease_details_rel')
    property_lease_count = fields.Integer(string="Property Lease Count",
                                 compute="_compute_property_lease_count")
    informal_trade_ids = fields.Many2many('rental.contract',
                                          'master_search_informal_lease_details_rel')
    informal_trade_count = fields.Integer(string="Outdoor Count",
                                 compute="_compute_property_lease_count")
    outdoor_lease_ids = fields.Many2many('rental.contract',
                                          'master_search_outdoor_lease_details_rel')
    outdoor_lease_count = fields.Integer(string="Outdoor Advertisement Leases Count",
                                 compute="_compute_property_lease_count")
    ...
    def _compute_asset_count(self):
        """ Get customer count """
        self.asset_count = len(self.asset_ids)

    @api.depends('property_ids')
    def _compute_property_count(self):
        """ Get customer count """
        self.property_count = len(self.property_ids)

    @api.depends('outdoor_ids')
    def _compute_outdoor_count(self):
        """ Get customer count """
        self.outdoor_count = len(self.outdoor_ids)

    @api.depends('property_lease_ids', 'informal_trade_ids', 'outdoor_lease_ids')
    def _compute_property_lease_count(self):
        """ Get customer count """
        self.property_lease_count = len(self.property_lease_ids)
        self.informal_trade_count = len(self.informal_trade_ids)
        self.outdoor_lease_count = len(self.outdoor_lease_ids)
    ...
    def action_clear_search(self):
        ...
        self.outdoor_ids = False
        self.property_lease_ids = False
        self.informal_trade_ids = False
        self.outdoor_lease_ids = False
    ...
    def _search_query(self, key):
        ...
        self._search_buildings(key, active_qry, company_id) \
            if self.search_by in ['any', 'property'] else False
        self._search_assets(key, active_qry, company_id) \
            if self.search_by in ['any', 'asset'] else False
        self._search_outdoor(key, active_qry, company_id) \
            if self.search_by in ['any', 'outdoor'] else False
        self._search_property_lease(key, active_qry, company_id) \
            if self.search_by in ['any', 'property_lease'] else False
        self._search_informal_trade(key, active_qry, company_id) \
            if self.search_by in ['any', 'informal_trade'] else False
        self._search_outdoor_lease(key, active_qry, company_id) \
            if self.search_by in ['any', 'outdoor_lease'] else False
    ...
    def _search_buildings(self, key, active_qry, company_id):
        ...
        WHERE 
            (b.name ILIKE '%{key}%'
            OR 
             b.jmc_number ILIKE '%{key}%'
            OR 
             b.address ILIKE '%{key}%'
            OR 
             b.sg_id ILIKE '%{key}%'
            OR 
             b.stand_number ILIKE '%{key}%'
            OR 
             b.street_number ILIKE '%{key}%'
            OR 
             b.street_type_name ILIKE '%{key}%'
             {active})
    """
        self._cr.execute(pt_query.format(op_id=company_id, key=key,
                                         active=active_qry).replace('obj',
                                                                    'b'))
        template_ids = self._cr.dictfetchall()
        property_ids = self.env['building'].browse(
            [i['id'] for i in template_ids])
        self.property_ids += property_ids

    def _search_assets(self, key, active_qry, company_id):
        """Search for buildings """
        pt_query = """
        SELECT 
            a.id 
        ...
        WHERE 
            ...
            (a.name ILIKE '%{key}%'
            OR 
             a.alternative_ref ILIKE '%{key}%'
            OR 
             a.description ILIKE '%{key}%'
            OR 
             a.identification_number ILIKE '%{key}%'
             {active}
             )
    """
        self._cr.execute(pt_query.format(op_id=company_id, key=key,
                                         active=active_qry).replace('obj',
                                                                    'a'))
        template_ids = self._cr.dictfetchall()
        asset_ids = self.env['account.asset'].browse(
            [i['id'] for i in template_ids])
        self.asset_ids += asset_ids

    def _search_outdoor(self, key, active_qry, company_id):
        """Search for Outdoor """
        pt_query = """
        SELECT 
            oa.id 
        FROM 
            outdoor_advertisement AS oa
        WHERE 
            (oa.name ILIKE '%{key}%'
            OR
            
             oa.jmc_number ILIKE '%{key}%'
            OR 
             oa.address ILIKE '%{key}%'
            OR 
             oa.erf ILIKE '%{key}%'
             )
    """
        self._cr.execute(pt_query.format(op_id=company_id, key=key,
                                         active=active_qry).replace('obj',
                                                                    'oa'))
        template_ids = self._cr.dictfetchall()
        outdoor_ids = self.env['outdoor.advertisement'].browse(
            [i['id'] for i in template_ids])
        self.outdoor_ids += outdoor_ids

    def _search_property_lease(self, key, active_qry, company_id):
        """Search for Outdoor """
        pt_query = """
        SELECT 
            rc.id 
        ...
        WHERE 
            (rc.lease_type = 'portfolio_lease') AND
            (rc.name ILIKE '%{key}%'
            OR
             rc.jmc_number ILIKE '%{key}%'
            OR 
             rc.address ILIKE '%{key}%'
            OR 
             rc.bldg_name ILIKE '%{key}%'
            OR 
             rc.ref ILIKE '%{key}%'
             )"""
        self._cr.execute(pt_query.format(op_id=company_id, key=key,
                                         active=active_qry).replace('obj',
                                                                    'oa'))
        template_ids = self._cr.dictfetchall()
        property_lease_ids = self.env['rental.contract'].browse(
            [i['id'] for i in template_ids])
        self.property_lease_ids += property_lease_ids

    def _search_informal_trade(self, key, active_qry, company_id):
        """Search for Outdoor """
        pt_query = """
        SELECT 
            rc.id 
        ...
        self._cr.execute(pt_query.format(op_id=company_id, key=key,
                                         active=active_qry).replace('obj',
                                                                    'rc'))
        template_ids = self._cr.dictfetchall()
        informal_trade_ids = self.env['rental.contract'].browse(
            [i['id'] for i in template_ids])
        self.informal_trade_ids += informal_trade_ids

    def _search_outdoor_lease(self, key, active_qry, company_id):
        """Search for Outdoor """
        pt_query = """
        SELECT 
            rc.id 
        FROM 
            rental_contract AS rc
        WHERE 
            (rc.lease_type = 'outdoor_lease') AND
            (rc.name ILIKE '%{key}%'
            OR
             rc.jmc_number ILIKE '%{key}%'
            OR 
        ...
        self._cr.execute(pt_query.format(op_id=company_id, key=key,
                                         active=active_qry).replace('obj',
                                                                    'rc'))
        template_ids = self._cr.dictfetchall()
        outdoor_lease_ids = self.env['rental.contract'].browse(
            [i['id'] for i in template_ids])
        self.outdoor_lease_ids += outdoor_lease_ids

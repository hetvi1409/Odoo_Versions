...
class PropertyEnquiry(http.Controller):
    ...
    def assessment_circulation(self, **kw):
        ...
        circulation_assessment = request.env['circulation.assessment'].sudo().create({
            ...
            'user_id': request.uid,
            'group_ids': request.env['res.users'].sudo().browse(request.uid).sudo().group_ids
        })
        documents = []
        for rec in attached_files:
            if rec.filename == '':
                ...
                if assessment.property_id.sudo().region_id:
                    region = request.env['documents.document'].sudo().search([
                        ('name', '=', assessment.property_id.sudo().region_id.name),
                        ('folder_id', '=', assessment_folder.id),('type','=','folder')
                    ])
                    if not region:
                        region = request.env['documents.document'].sudo().create({
                            'name': assessment.property_id.sudo().region_id.name,
                ...
                jmc = request.env['documents.document'].sudo().search([
                    ('name', '=', assessment.jmc_number),
                    ('folder_id', '=', region.id),('type','=','folder')
                ])
                if not jmc:
                    jmc = request.env['documents.document'].sudo().create({
                        'name': assessment.jmc_number,
                        'folder_id': region.id,
                        'type':'folder'
                    })
                enquiry_folder = request.env['documents.document'].sudo().search([
                    ('name', '=', assessment.name),
                    ('folder_id', '=', jmc.id),('type','=','folder')
                ])
                if not enquiry_folder:
                    enquiry_folder = request.env['documents.document'].create({
                        'name': assessment.name,
                        'folder_id': jmc.id,
                        'type':'folder'
                    })
                circulation = request.env['documents.document'].sudo().search([
                    ('name', '=', 'Circulation for Comment'),
                    ('folder_id', '=', enquiry_folder.id),('type','=','folder')
                ])
                if not circulation:
                    circulation = request.env['documents.document'].sudo().create({
                        'name': 'Circulation for Comment',
                        'folder_id': enquiry_folder.id,
                        'type':'folder'
                    })
                feedback = request.env['documents.document'].sudo().search([
                    ('name', '=', 'Feedback on Comments'),
                    ('folder_id', '=', circulation.id),('type','=','folder')
                ])
                if not feedback:
                    feedback = request.env['documents.document'].sudo().create({
                        'name': 'Feedback on Comments',
                        'folder_id': circulation.id,
                        'type':'folder'
                    })

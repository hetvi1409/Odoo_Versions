...
class ClientEnquiry(models.Model):
    ...
    type = fields.Selection([('social_lease', 'Social Lease/ Sale'),
                            ...
                            string='Type of enquiry',required=True)
    ...
    cell_phone = fields.Char(string="Cell",required=True)
    ...
    passport_no = fields.Char(string="Identity Number /Passport No",
                              help="Enter the identity/passport no", required=False)
    ...
    user_id = fields.Many2one('res.users', string="Assignee",
                              domain = lambda self: [
                                  ('group_ids', 'in', self.env.ref('client_enquiry.group_cbo').id)])
    ...
    first_name_title = fields.Selection([('Mr', 'Mr'), ('Mrs', 'Mrs'), ('Ms', 'Ms'),
                              ('Miss', 'Miss')], string="Title")
    attachment_ids = fields.Many2many('ir.attachment', string="GIS Report")
    enquiry_folder_id = fields.Many2one('documents.document', string='Supporting documents')
    ...
    def create(self, vals_list):
        for vals in vals_list:
            sequence_code = 'client.enquiry'
            vals['name'] = self.env['ir.sequence'].next_by_code(
                sequence_code)
        res = super(ClientEnquiry, self).create(vals_list)
        for rec in res:
            rec.state = 'draft'
            rec.last_stage_updated = fields.Datetime.now()
            ...
            policy_status = self.env['client.enquiry.sla.policy.status'].create(
                {
                    ...
                    'name': policy.name,
                    'policy_id': policy.id,
                    'process_status': policy.process_status
                })
            rec.sla_policy_ids = [(4, policy_status.id)]
        return res
    ...
    def action_create_assessment(self):
        ...
        if self.env.user != self.user_id:
            ...
            recipient_ids = self.env.ref('client_enquiry.group_asset_evaluator').user_ids
            ...
            if self.env.user in self.env.ref('client_enquiry.group_asset_evaluator').user_ids:
                action = {
                    'name': _('Assessment'),
                    'type': 'ir.actions.act_window',
            ...
            if self.env.user in self.env.ref('client_enquiry.group_cbo').user_ids:
                return {
                    'effect': {
                        'fadeout': 'slow',
    ...
    def _onchange_partner_id(self):
        ...
        self.cell_phone = self.partner_id.phone
    ...
    def action_submit(self):
        ...
        recipient_ids = self.env.ref(
            'client_enquiry.group_cbo').user_ids
        partner = recipient_ids.mapped('partner_id')
        email_values = {
            'recipient_ids': [(6, 0, partner.ids)]
        }
        ...
        try:
            if self.state == 'assessment':
                template_id = self.env.ref(
                    'client_enquiry.email_template_client_enquiry_supported')
            if self.state == 'completed':
                template_id = self.env.ref(
                    'client_enquiry.email_template_client_enquiry_assessment_completed')
        except ValueError:
            template_id = False
        try:

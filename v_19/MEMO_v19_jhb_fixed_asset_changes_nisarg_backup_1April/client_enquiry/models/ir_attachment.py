...
class Attachment(models.Model):
    ...
    def create(self, values):
        ...
        # Comment below code by Nisarg, facing issue while uploading more than one image for asset verification
        # *************************************************
        ...
        # *************************************************
            ...
            # jmc_folder = self.env['documents.document'].sudo().search([
            #     ('name', '=', enquiry.asset_number),
            #     ('folder_id', '=', folder.id)
            # ])
            # if not jmc_folder:
            #     jmc_folder = self.env['documents.document'].sudo().create({
            #         'name': enquiry.asset_number,
            #         'folder_id': folder.id
            #     })

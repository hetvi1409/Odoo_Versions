...
import SETASelectFilterTemp from "@seta_dashboard/js/component/main/seta_select_filter_temp";
...
var SETAViewAnalysis = Widget.extend({
    ...
    events: {
        'click .seta_view_analysis_explore_container': '_onClickAnalysisExpore',
        'click .seta_submit_analysis_explore': '_onClickSubmitAnalysisExpore',
        'click .seta_view_analysis_explore_bg': '_onClickBgAnalysisExpore',
        'click .seta_update_script': '_onClickUpdateScript',
        'click .seta_close_script': '_onClickCloseScript',
        'click .seta_dashboard_script_close': '_onClickCloseScript',
        'click .seta_toggle_float_script': '_onClickToggleFloatScript',
        'click .seta_script_ai': '_onClickContinueScriptAI',
        'click #seta_tab_js': '_onClickShowJsScript',
        'click #seta_tab_css': '_onClickShowCssScript',
    },
    ...
    start: function () {
        ...
        // Analysis Explore
        self.$viewAnalysisExplore = self.$('.seta_view_analysis_explore');
        ...
        setTimeout(function () {
            ...
            if ($dialog.length) {
                $dialog.addClass('seta_modal_dialog_full');
    ...
    _onClickBgAnalysisExpore: function (ev) {
        var self = this;
        self.$viewAnalysisExplore.closest('.seta_dialog').hide();
    },
    ...
    _onClickSubmitAnalysisExpore: function (ev) {
        ...
        new swal({
            ...
            if (result.isConfirmed) {
                ...
                rpc('/web/dataset/call_kw/seta.analysis/save_lab_analysis_explore', {
                    model: 'seta.analysis',
    ...
    _loadVisualScriptEditor: function () {
        ...
        if (self.scriptEditorType == 'Python' || self.scriptEditorType == 'PostgreSQL') {
            self.$editorContainer.find('.seta_dashboard_script_type').attr('src', '/seta_dashboard/static/src/img/js.png');
        }
        // Query Editor
        ace.config.set('basePath', '/seta_dashboard/static/lib/ace-1.3.1/');
        ace.config.set('modePath', '/seta_dashboard/static/lib/ace-1.3.1/');
        ace.config.set('themePath', '/seta_dashboard/static/lib/ace-1.3.1/');
        ace.config.set('workerPath', '/seta_dashboard/static/lib/ace-1.3.1/');
        self.$editor = ace.edit('seta_script_editor', {
            fontSize: "12px",
        ...
        self.$editorCss = ace.edit('seta_script_editor_css', {
            fontSize: "12px",
            fontFamily: "JetBrainsMono",
        });
        ...
        self.$editorContainer = self.$('.seta_dashboard_script_editor');
        self.$editorContainer.find('.seta_script_tab').removeClass("d-none")
    ...
    _loadDataScriptEditor: function (type) {
        ...
        if (type == 'python') {
            ...
            self.$editorContainer.find('.seta_dashboard_script_type').attr('src', '/seta_dashboard/static/src/img/python.png');
        } else if (type == 'sql') {
            self.scriptEditorType = 'PostgreSQL';
            self.$editor.session.setMode('ace/mode/sql');
            self.$editorContainer.find('.seta_dashboard_script_type').attr('src', '/seta_dashboard/static/src/img/psql.png');
        }
        self.$editorContainer.find('.seta_script_tab').addClass("d-none")
        self.$editorContainer.find('#seta_script_editor').addClass("h-100")
        self.$editorContainer.draggable();
        self.$editorContainer.resizable();
    ...
    _resetScriptEditor: function () {
        ...
        if (self.$editorContainer) {
            ...
            self.$editorContainer.find('.seta_update_script').attr('data-analysis-id', 0);
            self.$editorContainer.find('.seta_update_script').attr('data-analysis-name', '');
            self.$editorContainer.find('.seta_update_script').attr('data-block-id', 0);
            self.$editorContainer.find('.seta_update_script').attr('data-script-type', '');
            ...
            var $dialog = self.$el.closest('.modal-dialog');
            if ($dialog.length) {
                $dialog.addClass('seta_modal_dialog_no_scroll');
            }
        }
    },
    ...
    _closeScript: function () {
        ...
        if (self.$editorContainer) {
            ...
            var $dialog = self.$el.closest('.modal-dialog');
            if ($dialog.length) {
                $dialog.removeClass('seta_modal_dialog_no_scroll');
            }
        }
    },
    ...
    _onClickShowJsScript: function (ev) {
        var self = this
        $("#seta_tab_js").addClass("seta_tab_active")
        $("#seta_tab_css").removeClass("seta_tab_active")
        $("#seta_script_editor").removeClass("d-none")
        $('#seta_script_editor').removeClass("h-100")
        $("#seta_script_editor_css").addClass("d-none")
        $('.seta_dashboard_script_type').attr('src', '/seta_dashboard/static/src/img/js.png');
    },
    _onClickShowCssScript: function (ev) {
        var self = this
        $("#seta_tab_css").addClass("seta_tab_active")
        $("#seta_tab_js").removeClass("seta_tab_active")
        $("#seta_script_editor").addClass("d-none")
        $("#seta_script_editor_css").removeClass("d-none")
        $('.seta_dashboard_script_type').attr('src', '/seta_dashboard/static/src/img/css.png');
    },
    _onClickContinueScriptAI: function (ev, last_generated_code = '', last_error_message = '') {
        ...
        if (self.$editor && self.analysis_id) {
            ...
            if (origin_code) {
                ...
                rpc('/web/dataset/call_kw/seta.analysis/action_get_lab_script', {
                    model: 'seta.analysis',
        ...
        new swal({
            ...
            if (result.isConfirmed && script_type) {
                if (script_type == 'visual') {
                    ...
                    $('.spinner-container').addClass('d-flex');
                    rpc('/web/dataset/call_kw/seta.analysis/try_write_data_script', {
                        model: 'seta.analysis',
...
export default SETAViewAnalysis;
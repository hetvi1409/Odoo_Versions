...
import SETATags from "@seta_dashboard/js/component/general/seta_tags";
var SETASelectFilterTemp = Widget.extend({
    ...
    events: {
        ...
        'click .seta_select_dynamic_field_filter_temp': '_onClickSelectDynamicFieldFilterTemp',
        'click .dynamic_filter_button': '_onClickRemoveDynamicFilter',
        'click .seta_select_date_format': '_onClickSelectDateFormat',
        'click #seta_export_capture_analysis': '_onClickCaptureAnalysis',
        'click #seta_analysis_limit': '_onClickOpenLimitInput',
        'change #seta_limit_input_number': '_onChangeConfirmLimit',
        'keypress #seta_limit_input_number': '_onKeypressConfirmLimit',

        'click #seta_dynamic_filter_container_custom': '_onClickDynamicFilterCustom',
    },
    ...
    start: function () {
        var self = this;
        this._super.apply(this, arguments);
        self.$btnExportCapture = self.$('#seta_export_capture_analysis');
        self.$btnExportLoading = self.$('#seta_export_capture_loading_analysis');
    ...
    _onClickDynamicFilterCustom: function (ev) {
        var self = this
        var dynamicFilterCustom = self.$el.find('#seta_dynamic_filter_container_custom')
    ...
    _initFilter: function (type) {
        ...
        if (type == 'date_range') {
            ...
            var $dateFromElm = self.$filter[type].elm.find('.seta_analysis_filter_temp_content').find('#seta_date_from');
            $dateFromElm.bootstrap_datepicker({
                language: "en",
                format: "yyyy-mm-dd",
                autoclose: true,
            });
            var $dateToElm = self.$filter[type].elm.find('.seta_analysis_filter_temp_content').find('#seta_date_to');
            ...
            self.$filter[type].elm.find('.seta_analysis_filter_temp_content').append(`
                <div class="seta_dropdown seta_block_left seta_inline dropdown">
                    ...
                    <div class="dropdown-menu">
                        <a class="dropdown-item seta_select_date_format" data-date_format="yesterday">Yesterday</a>
                        ...
                        <a class="dropdown-item seta_select_date_format" data-date_format="last_10">Last 10 Days</a>
                        <a class="dropdown-item seta_select_date_format" data-date_format="last_30">Last 30 Days</a>
                        <a class="dropdown-item seta_select_date_format" data-date_format="last_60">Last 60 Days</a>
                    </div>
                </div>
            `);
        }
    ...
    _loadFilters: function (callback) {
        var self = this;
        if (self.analysis_id) {
            rpc('/web/dataset/call_kw/seta.analysis/ui_get_filter_info', {
                model: 'seta.analysis',
                ...
                if (result) {
                    ...
                    self.filter_types.forEach(type => {
                        if (type !== 'field_search') {
                            ...
                            self.$filter[type].elm.find('.seta_analysis_filter_temp_title .dropdown-menu').empty();
                            self.$filter[type].elm.find('.seta_analysis_filter_temp_title .dropdown-menu').append(`<a class="dropdown-item seta_select_dynamic_field_filter_temp seta_col_transparent" data-type="${type}" data-id="-1">None</a>`);
                            self.fields[type].forEach(field => {
                                var activeClass = self.$filter[type].field_id == field.id ? 'active' : ''
                                var $elm = `
                                    <a class="dropdown-item seta_select_dynamic_field_filter_temp ${activeClass}" data-type="${type}" data-label="${field.name}" data-name="${field.field_name}">${field.name}</a>
                                `;
                                self.$filter[type].elm.find('.seta_analysis_filter_temp_title .dropdown-menu').append($elm);
                            });
                        }
    ...
    _onClickRemoveDynamicFilter: function (ev) {
        ...
        if (self.dynamicFilter.length == 0) {
            self.el.querySelector('#seta_dynamic_filter_container_custom').className = 'seta_analysis_filter_temp'
        }
    },
    ...
    _fillSelectedDynamicFilter: function () {
        var self = this
        self.el.querySelector('#seta_dynamic_filter_container_custom').className = 'seta_analysis_filter_temp active'

        var element = self.el.querySelector('#seta_selected_dynamic_filter')
        element.innerHTML = ""
        for (let i = 0; i < self.dynamicFilter.length; i++) {
            let filter = self.dynamicFilter[i];
    ...
    _onClickSelectFieldFilterTemp: function (ev) {
        ...
        if (self.analysis_id && field_id && type) {
            rpc('/web/dataset/call_kw/seta.analysis/ui_add_filter_temp_by_field', {
                model: 'seta.analysis',
    ...
    _changeLimit: function () {
        ...
        var limit_value = parseInt(self.el.querySelector('#seta_limit_input_number').value)
        if (limit_value > 0) {
        ...
        self.$visual._getDataAnalysis(args, function (result) {
            ...
            if (self.$visual.drilldown_title) {
                self.$visual.$el.find('.seta_reset_drilldown').show();
                self.$visual.$el.find('.seta_drillup').show();
            }
    ...
    _onClickOpenLimitInput: function (ev) {
        var self = this
        var input_element = self.el.querySelector('#seta_limit_input').style.display
        if (input_element == 'none') {
            self.el.querySelector('#seta_limit_input').style.display = 'block';
            self.$('#seta_analysis_filter_temp_limit').addClass('active');
        } else {
            self.el.querySelector('#seta_limit_input').style.display = 'none';
            self.$('#seta_analysis_filter_temp_limit').removeClass('active');
        }
    },
    ...
    _onClickCaptureAnalysis: function (ev) {
        ...
        var blockId = $(ev.currentTarget).closest('.seta_dashboard_block_item').attr('data-id');
        var querySelector = document.querySelector('.seta_view_analysis .seta_dashboard_block_item');
        if (blockId) {
            querySelector = document.querySelector(`.seta_dashboard_block_item[data-id="${blockId}"]`);
        }
...
export default SETASelectFilterTemp;
...
class DashboardWebsiteController(http.Controller):
    ...
    @http.route('/seta/dashboard/<int:dashboard_id>/token', auth='public', type='http', cors='*', csrf=False)
    def get_access_token(self, dashboard_id, **kw):
        # Get System Parameter: Access Key
        access_key = request.env['ir.config_parameter'].sudo().get_param('seta_dashboard.access_key')
        if not access_key:
        ...
        whitelist_ip_addresses = request.env['ir.config_parameter'].sudo().get_param('seta_dashboard.whitelist_ip_addresses')
        ...
        # Expired in 1 Hour
        access_token = request.env['seta.dashboard.token'].sudo().create({
    ...
    @http.route('/seta/dashboard/<int:dashboard_id>/page', auth='public', type='http', website=True, cors='*', csrf=False)
    def get_dashboard_page(self, dashboard_id, **kw):
        access_token = request.env['seta.dashboard.token'].sudo().search([('expired_date', '>=', fields.Datetime.now()), ('dashboard_id', '=', dashboard_id), ('is_active', '=', True), ('token', '=', kw.get('access_token', ''))], limit=1)
        if not access_token:
        ...
        dashboard_name = request.env['seta.dashboard'].with_user(token_user_id).with_context(context).search([('id', '=', dashboard_id)]).name
        return request.render('seta_dashboard.dashboard_page', {
            'dashboard_id': dashboard_id,
            'dashboard_name': dashboard_name,
            'access_token': kw.get('access_token'),
        })
    
    @http.route('/seta/dashboard/<int:dashboard_id>', auth='public', type='http', cors='*', csrf=False)
    def get_dashboard(self, dashboard_id, **kw):
        ...
        dashboard = request.env['seta.dashboard'].with_user(token_user_id).with_context(context).browse(dashboard_id)
        # Search Read Dashboard Block By Dashboard Id
        blocks = request.env['seta.dashboard.block'].with_user(token_user_id).with_context(context).search_read(
        ...
        # get all themes
        dashboard_themes = request.env['seta.dashboard.theme'].with_user(token_user_id).with_context(context).search([], order='name asc')
    ...
    @http.route('/seta/analysis/<int:analysis_id>/ui_get_available_fields', auth='public', type='http', cors='*', csrf=False)
    def ui_get_available_fields(self, analysis_id):
        analysis = request.env['seta.analysis'].sudo().browse(analysis_id)
        fields = analysis.ui_get_available_fields({})
        return self.make_valid_response(fields)
    
    @http.route('/seta/analysis/<int:analysis_id>/try_get_analysis_data_dashboard', auth='public', type='http', cors='*', csrf=False)
    def try_get_analysis_data_dashboard(self, analysis_id, **kw):
        access_token = request.env['seta.dashboard.token'].sudo().search([('expired_date', '>=', fields.Datetime.now()), ('is_active', '=', True), ('token', '=', kw.get('access_token', ''))], limit=1)
        if not access_token:
            return self.make_error_response(401, 'Unauthorized', 'Invalid or Expired Access Token')
        token_user_id = access_token.user_id
        context = access_token.context
        ...
        analysis = request.env['seta.analysis'].with_user(token_user_id).with_context(context).browse(analysis_id)
        kwargs = {}
        if 'kwargs' in kw:
            try:
        ...
        return self.make_valid_response(data)
    @http.route('/seta/analysis/<int:analysis_id>/data', auth='public', type='http', cors='*', csrf=False)
    def get_analysis_data(self, analysis_id, **kw):
        ...
        analysis = request.env['seta.analysis'].with_user(token_user_id).browse(analysis_id)
        if not analysis:
    ...
    @http.route('/seta/analysis/fields/dynamic/<int:analysis_id>', auth='public', type='http', cors='*', csrf=False)
    def ui_get_fields_dynamic(self, analysis_id, **kw):
        if not analysis_id:
            return self.make_error_response(500, 'Error', 'Analysis ID is required')
        analysis = request.env['seta.analysis'].browse(analysis_id)
        if not analysis:
            return self.make_error_response(500, 'Error', 'Analysis not found')
        kwargs = json.loads(kw['kwargs'])
    ...
    @http.route('/seta/dashboard/filters/<int:dashboard_id>', auth='user', csrf=False, website=True, save_session=False)
    def fetch_by_dashboard(self, dashboard_id, **kw):
        dashboard_filter_obj = request.env['seta.dashboard.filter']
        result = dashboard_filter_obj.fetch_by_dashboard(dashboard_id)
        return self.make_valid_response(result)
    
    @http.route('/seta/dashboard/filters/values', auth='user', csrf=False, website=True, save_session=False)
    ...
    @http.route('/seta/dashboard/slide/<int:dashboard_id>', auth='user', csrf=False, website=True, save_session=False)
    def show_slide(self, dashboard_id, **kw):
        dashboard = request.env['seta.dashboard'].sudo().browse(dashboard_id)
        slides = dashboard.slide_ids.sorted(key=lambda r: r.sequence)
        vals = []
        default_bg_attachment_url = ''
        if dashboard.general_bg_file:
            default_bg_attachment_url = '/web/image/seta.dashboard/%s/general_bg_file' % (dashboard.id)

        for slide in slides:
            if slide.bg_file:
                bg_attachment_url = '/web/image/seta.dashboard.slide/%s/bg_file' % (slide.id)
            else:
        ...
        return request.render('seta_dashboard.seta_dashboard_slide', {'data':vals, 'global_data':global_vals})
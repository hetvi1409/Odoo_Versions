...
class SETADashboardConfigWizard(models.TransientModel):
    _name = 'seta.dashboard.config.wizard'
    _description = 'SETA Dashboard Config Wizard'
            ...
            for dt in data:
                try:
                    ...
                    if dt['method'] == 'model':
                        ...
                        table = self.env['seta.table'].search([('table_name', '=', table_name)], limit=1)
                        if not table:
                    ...
                    source = self.env['seta.data.source'].search([('name', '=', source_name)], limit=1)
                    if not source:
                    ...
                    analysis = self.env['seta.analysis'].search([('name', '=', dt.get('name'))], limit=1)
                    # analysis = False
                    ...
                    if dt['method'] == 'model':
                        ...
                        if not analysis:
                            analysis_vals['db_query'] = db_query
                            analysis = self.env['seta.analysis'].create(analysis_vals)
                        else:
                        ...
                        table_name = dt.get('table_name')
                        table = self.env['seta.table'].search([('name', '=', table_name)], limit=1)
                        if not table:
                            if dt.get('query'):
                        ...
                        if dt.get('table_name'):
                            table = self.env['seta.table'].search([('table_name', '=', dt['table_name'])])
                        if not table:
                            table_obj = self.env['seta.table']
                    ...
                    for metric in dt.get('metrics'):
                        ...
                        if not field_name:
                            continue
                        field = self.env['seta.table.field'].search([('field_name', '=', field_name), ('table_id', '=', table.id)], limit=1)
                        if not field:
                            # continue
                    ...
                    if date_field_name:
                        date_field = self.env['seta.table.field'].search([('field_name', '=', date_field_name), ('table_id', '=', table.id)], limit=1)
                        if date_field:
                    ...
                    block = self.env['seta.dashboard.block'].search([('analysis_id', '=', analysis.id), ('dashboard_id', '=', self.dashboard_id.id)], limit=1)
                    if not block:
                        block_vals = {
                        ...
                        #     x = 0
                        #     y += 4
                        self.env['seta.dashboard.block'].create(block_vals)

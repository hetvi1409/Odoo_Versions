...
class SETAAnalysis(models.Model):
    ...
    ai_explore_analysis_ids = fields.One2many('seta.analysis', 'parent_analysis_id', string='AI Explore Analysis')
    parent_analysis_id = fields.Many2one('seta.analysis', string='Parent Analysis')
        ...
        for rec in recs:
            if self._context.get('ai_create'):
                try:
                    ...
                    res = requests.post('''%s/lab/analysis/create''' % (seta_lab_url), json={
    ...
    def prepare_analysis_from_config(self, config):
        ...
        field_by_name = {}
        operator_by_name = {}
        operators = self.env['seta.analysis.filter.operator'].search([])
        for field in fields:
            if field.field_type in ('numeric', 'number'):
                metric_by_name[field.field_name] = field.id
            dimension_by_name[field.field_name] = field.id
    ...
    def start_lab_analysis_explore(self):
        ...
        try:
            res = requests.post('''%s/lab/analysis/explore''' % (seta_lab_url), json={
                'seta_lab_api_key': self.env.company.seta_lab_api_key,
        ...
        if not res_explore_values:
            res_explore_values = []
        analysis_explores = []
        existing_analysis_explore = self.env['seta.analysis'].search(['|', ('active', '=', False), ('active', '=', True), ('parent_analysis_id', '=', self.id)])
        ...
        for val in res_explore_values:
            ...
            if val.get('metrics'):
                for metric in val.get('metrics'):
                    ...
                    metric_field = self.env['seta.table.field'].search([('table_id', '=', self.table_id.id), ('field_name', '=', metric_field_name)], limit=1)
                    if metric_field:
            ...
            if val.get('dimensions'):
                for dimension in val.get('dimensions'):
                    ...
                    dimension_field = self.env['seta.table.field'].search([('table_id', '=', self.table_id.id), ('field_name', '=', dimension_field_name)], limit=1)
                    if dimension_field:
    ...
    def action_get_lab_description(self, ai_analysis_data, block_id = False, dashboard_id = False):
        ...
        if not dashboard_id:
            language = self.env['seta.dashboard.block'].browse(block_id).dashboard_id.lang_id.name
        else:
            language = self.env['seta.dashboard'].browse(dashboard_id).lang_id.name

        # if self.ai_analysis_text == 'There is no description yet.' or self.ai_language != language:
        if True:
            try:
                self.ai_language = language
                res = requests.post('''%s/lab/analysis/description''' % (seta_lab_url), json={
                    'seta_lab_api_key': self.env.company.seta_lab_api_key,
    ...
    def action_get_lab_insight(self, ai_analysis_data, drilldown_level, drilldown_title, languange):
        ...
        if not seta_lab_url:
            raise UserError(_('Please set SETA Lab URL in System Parameters.'))
        # seta_lab_url = "http://localhost:8014"
        analysis_name = self.name
        visual_type_name = self.visual_type_id.name
        try:
            res = requests.post('''%s/lab/analysis/insight''' % (seta_lab_url), json={
                'seta_lab_api_key': self.env.company.seta_lab_api_key,
                'analysis_name': analysis_name,
                'visual_type_name': visual_type_name,
                'language': False,
    ...
    def action_get_speech_text_ai(self, data):
        ...
        try:
            ...
            res = requests.post('''%s/lab/analysis/ai/speech/text''' % (seta_lab_url), json={
                'seta_lab_api_key': self.env.company.seta_lab_api_key,
                'data': res_data,
            }, timeout=120)
            res = res.json()
    ...
    def action_get_lab_speech_ai(self):
        ...
        visual_type_name = self.visual_type_id.name
        try:
            res = requests.post('''%s/lab/analysis/ai/speech''' % (seta_lab_url), json={
                'seta_lab_api_key': self.env.company.seta_lab_api_key,
                'analysis_name': analysis_name,
                'visual_type_name': visual_type_name,
    ...
    def action_get_lab_script(self, script_type, origin_code, origin_after_code, num_of_space, last_generated_code, last_error_message):
        ...
        if not seta_lab_url:
            raise UserError(_('Please set SETA Lab URL in System Parameters.'))
        try:
            res = requests.post('''%s/lab/analysis/script''' % (seta_lab_url), json={
                'seta_lab_api_key': self.env.company.seta_lab_api_key,

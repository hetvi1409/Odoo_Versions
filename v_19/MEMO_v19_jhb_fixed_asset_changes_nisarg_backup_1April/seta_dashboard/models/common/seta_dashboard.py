...
class SETADashboardFilter(models.Model):
    _name = 'seta.dashboard.filter'
    _description = 'SETA Dashboard Filter'
    
    name = fields.Char(required=True)
    dashboard_id = fields.Many2one('seta.dashboard', string="Dashboard", required=True, ondelete='cascade')
    ...
    table_id = fields.Many2one('seta.table', string="Table")
    table_field_id = fields.Many2one('seta.table.field', string="Field")
    ...
    value_ids = fields.Many2many('seta.dashboard.filter.value', string="Values")
    filter_analysis_ids = fields.One2many('seta.dashboard.filter.analysis', 'filter_id', string="Filter Analysis")
    parent_filter_id = fields.Many2one('seta.dashboard.filter', string='Parent Filter', domain="[('dashboard_id', '=', dashboard_id), ('id', '!=', id)]")
    link_field_id = fields.Many2one('ir.model.fields', string="Link Field")
    allowed_link_field_ids = fields.Many2many('ir.model.fields', string='Allowed Link Field', compute='_compute_allowed_link_field',)
    
    @api.depends('model_id','parent_filter_id')
    def _compute_allowed_link_field(self):
        for rec in self:
            if rec.model_id and rec.parent_filter_id and rec.source_type == 'model' :
                domain = [
                    ('model_id','=',rec.model_id.id),
                    ('ttype','=','many2one'),
                    ('relation','=',rec.parent_filter_id.model_id.model),
                ]
                model_fields = self.env['ir.model.fields'].search(domain)
                rec.allowed_link_field_ids = model_fields
            else:
                rec.allowed_link_field_ids = False
    ...
    def fetch_values(self, params, query='', parent_filter_value=''):
        ...
        link_field = params.get('linkField', False)
        filter = False
        field_type = False
        if filter_id:
            filter = self.env['seta.dashboard.filter'].browse(filter_id)
            if filter.table_field_id:
                field_type = filter.table_field_id.field_type
        if analysis_id:
            analysis = self.env['seta.analysis'].browse(analysis_id)
            ...
            if analysis.table_id:
                field_record = self.env['seta.table.field'].search([('table_id', '=', analysis.table_id.id), ('field_name', '=', field)], limit=1)
                if field_record:
        ...
        if get_table:
            if query:
                tables = self.env['seta.table'].search([('name','ilike',query)],limit=limit,order='is_template desc,name asc')
            else:
                tables = self.env['seta.table'].search([],limit=limit,order='is_template desc,name asc')
            vals = []
            for table in tables:
                vals.append({
                    'id':table.id,
        ...
        if params.get('sourceType') == 'model' and model:
            ...
            if model_field.ttype == 'many2one':
                ...
                table = model.replace('.', '_')
                field = 'name'
            res_domain = [(field, 'ilike', query)]
            if parent_filter_value and parent_filter_id:
                parent_record = self.env[parent_filter_model].search([('name','=',parent_filter_value)])
                res_domain.append((link_field,'in',parent_record.ids))
            res = self.env[model].search_read(res_domain, ['id', field], limit=10)
            if model_field.ttype == 'many2one':
                for r in res:
                    if field in r:
        ...
        res = self.env['seta.analysis']._transform_json_data(res)
        return res

    def fetch_by_dashboard(self, dashboard_id):
        res = []
        filters = self.env['seta.dashboard.filter'].search([('dashboard_id', '=', dashboard_id)])
        for filter in filters:
            ...
            if filter.source_type == 'model':
                ...
                filter_vals['params'] = {
                    ...
                    'linkField': filter.link_field_id.name,
                }
...
class SETADashboardFilterValue(models.Model):
    _name = 'seta.dashboard.filter.value'
    _description = 'SETA Dashboard Filter Value'
    
    filter_id = fields.Many2one('seta.dashboard.filter', string="Filter")
    name = fields.Char(string="Name")
    value = fields.Char(string="Value")

class SETADashboardFilterAnalysis(models.Model):
    _name = 'seta.dashboard.filter.analysis'
    _description = 'SETA Dashboard Filter Analysis'
    
    name = fields.Char(string="Name")
    
    filter_id = fields.Many2one('seta.dashboard.filter', string="Filter", required=True, ondelete='cascade')
    dashboard_id = fields.Many2one('seta.dashboard', related='filter_id.dashboard_id')
    table_id = fields.Many2one('seta.table', string="Table", required=False, ondelete='cascade')
    allowed_analysis_ids = fields.One2many('seta.analysis', related='table_id.analysis_ids', string='Allowed Analysis')
    analysis_id = fields.Many2one('seta.analysis', string="Analysis", required=False, ondelete='cascade')
    allowed_field_ids = fields.One2many('seta.table.field', string="Analysis Fields", related='table_id.field_ids')
    field_id = fields.Many2one('seta.table.field', string="Field", required=False)
...
class SETAAnalysis(models.Model):
    _inherit = 'seta.analysis'
    
    filter_analysis_ids = fields.One2many('seta.dashboard.filter.analysis', 'analysis_id', string="Filter Analysis")

class SETADashboard(models.Model):
    _name = 'seta.dashboard'
    _description = 'SETA Dashboard'
    _order = 'sequence,id'

    def _default_theme(self):
        default_theme = False
        try:
            default_theme = self.env.ref('seta_dashboard.seta_dashboard_theme_contrast').id
        except Exception as e:
            pass
        return default_theme

    name = fields.Char('Name', required=True)
    block_ids = fields.One2many(comodel_name='seta.dashboard.block',
                                inverse_name='dashboard_id', string='Dashboard Blocks')
    theme_id = fields.Many2one(comodel_name='seta.dashboard.theme', string='Dashboard Theme', default=_default_theme)
    ...
    filter_ids = fields.One2many('seta.dashboard.filter', 'dashboard_id', string="Filters")
    analysis_ids = fields.Many2many('seta.analysis', 'seta_dashboard_block', 'dashboard_id', 'analysis_id', string='Analysis')
    rtl = fields.Boolean('RTL (Right to Left)', default=False)
    lang_id = fields.Many2one('res.lang', string='Language')
    table_id = fields.Many2one('seta.table', 'Table')
    ...
    # SLIDE FIELDS ------------
    slide_ids = fields.One2many('seta.dashboard.slide', 'dashboard_id', string='Slide')
    ...
    category_id = fields.Many2one('seta.dashboard.category', string='Category')
    use_sidebar = fields.Boolean('Use Sidebar', default=False)
    ...
    def get_user_groups(self):
        manager_dashboard = self.env.ref('seta_dashboard.group_manager_dashboard')
        manager_analysis = self.env.ref('seta_data.group_manager_analysis')
    ...
    def write(self, vals):
        ...
        res = super(SETADashboard, self).write(vals)
        return res
    ...
    def action_open_slide(self):
        url = f"seta/dashboard/slide/{self.id}"
...
class SetaDashboardCategory(models.Model):
    _name = 'seta.dashboard.category'
    _description = 'SETA Dashboard Category'

    name = fields.Char('Name')
    active = fields.Boolean('Active', default=True)

class SetaDashboardSlide(models.Model):
    _name = 'seta.dashboard.slide'
    _description = 'SETA Dashboard Slide'

    dashboard_id = fields.Many2one('seta.dashboard', string='Dashboard')
    slide_name = fields.Char('Slide Name')
    slide_title = fields.Char('Slide Title')
    sequence = fields.Integer(string='Sequence')
    analysis_id = fields.Many2one('seta.analysis', string='Analysis', domain="[('id', 'in', analysis_domain)]")
    analysis_domain = fields.Many2many('seta.analysis', compute='_onchange_dashboard_id', store=False)
    ...
    def _constrains_analysis_id(self):
        for record in self:
            dashboard = self.env['seta.dashboard'].browse(record.dashboard_id.id)
    ...
    def action_generate_content_ai(self):
        ...
        return {
            ...
            'target': 'new',
            'res_id': self.id,
            'res_model': 'seta.dashboard.slide',
            'views': [[False, 'form']],
        }

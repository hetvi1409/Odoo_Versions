...
class SETAVisualType(models.Model):
    _name = 'seta.visual.type'
    _rec_name = 'title'
    _description = 'SETA Visual Type'
    ...
    _name_unique = models.Constraint(
        'unique(name)',
        'Visual Type Name Already Exist.',
    )

    def get_visual_config(self, visual_type, analysis_id=False):
        res = []
        visual_type_ids = self.env['seta.visual.type'].search(['|', ('name', '=', visual_type), ('title', '=', visual_type)])
        analysis_id = self.env['seta.analysis'].browse(analysis_id)

...
class SETAAnalysisCategory(models.Model):
    _name = 'seta.analysis.category'
    _description = 'SETA Analysis Category'

    name = fields.Char(string='Name', required=True)

class SETAAnalysis(models.Model):
    _name = 'seta.analysis'
    _description = 'SETA Analysis'

    name = fields.Char(string='Name', required=True)
    source_id = fields.Many2one('seta.data.source', string='Data Source', required=True, ondelete='cascade', default=lambda self: self._default_source())
    ...
    table_id = fields.Many2one('seta.table', string='Table', required=False, ondelete='cascade')
    table_view_id = fields.Many2one('seta.table', string='Table View', required=False, ondelete='cascade')
    table_model_id = fields.Many2one('seta.table', string='Table Model', required=False, ondelete='cascade')
    db_query = fields.Text('Query', related='table_id.db_query', readonly=False, store=True)
    metric_ids = fields.One2many('seta.analysis.metric', 'analysis_id', string='Metrics', required=True)
    dimension_ids = fields.One2many('seta.analysis.dimension', 'analysis_id', string='Dimensions')
    ...
    drilldown_dimension_ids = fields.One2many('seta.analysis.drilldown.dimension', 'analysis_id', string='Drilldown Dimensions')
    filter_temp_ids = fields.One2many('seta.analysis.filter.temp', 'analysis_id', string='Filters Temp')
    filter_ids = fields.One2many('seta.analysis.filter', 'analysis_id', string='Filters')
    limit = fields.Integer(string='Limit', default=100)
    query_preview = fields.Text(string='Query Preview', compute='get_query_preview')
    sort_ids = fields.One2many(comodel_name='seta.analysis.sort', inverse_name='analysis_id', string="Sorts")
    field_ids = fields.Many2many(comodel_name='seta.table.field', compute='_get_analysis_fields')
    group_ids = fields.Many2many(comodel_name='res.groups', string='Groups')
    date_field_id = fields.Many2one('seta.table.field', string='Date Field')
    identifier_field_id = fields.Many2one('seta.table.field', string='Identifier Field')
    model_id = fields.Many2one('ir.model', string='Model')
    model_name = fields.Char('Model Name', related='model_id.model')
    domain = fields.Char('Domain')
    category_id = fields.Many2one('seta.analysis.category', string='Category')
    kpi_id = fields.Many2one('seta.kpi', 'Key Performance Indicator')
    ...
    def _default_source(self):
        source_id = False
        source = self.env['seta.data.source'].search([('type', '=', 'db_odoo')], limit=1)
        if source:
            source_id = source.id
        return source_id
    ...
    def onchange_kpi_id(self):
        ...
        if self.method == 'kpi' and self.kpi_id:
            table = self.env['seta.table'].search([('model_id.model', '=', 'seta.kpi.line')], limit=1)
            if not table:
    ...
    def action_save_and_close(self):
        if self.env.context('dashboard_id'):
            dashboard_id = self.env.context('dashboard_id')
            analysis_id = self.id
            # Create Dashboard Block
            dashboard_block = self.env['seta.dashboard.block'].create({
                'dashboard_id': dashboard_id,
                'analysis_id': analysis_id,
            })
            if self.env.context('action_open_new'):
                return {
    ...
    # Unlink
    def unlink(self):
        res = super(SETAAnalysis, self).unlink()
        return res
    ...
    def build_query(self):
        ...
        if self.method == 'query':
            table = self.env['seta.table'].search([('name', '=', self.name), ('is_query', '=', True)], limit=1)
            if not table:
                table = self.env['seta.table'].create({
                    'name': self.name,
    ...
    def prepare_direct_table(self):
        ...
        if self.method == 'table' and self.name and self.source_id:
            table = self.env['seta.table'].search([('name', '=', self.name)], limit=1)
            if not table:
                table = self.env['seta.table'].create({
                    'name': self.name,
                    'source_id': self.source_id.id,
    ...
    def create(self, vals_list):
        recs = super(SETAAnalysis, self).create(vals_list)
    ...
    def write(self, vals):
        for analysis in self:
            if vals.get('analysis_data_script') and not analysis.server_action_id:
                ...
                server_action = self.env['ir.actions.server'].create({
                    ...
                    'model_id': self.env['ir.model'].search([('model', '=', 'seta.analysis')], limit=1).id,
                    'state': 'code',
        ...
        res = super(SETAAnalysis, self).write(vals)
        # for analysis in self:
        #     if not analysis.metric_ids:
        #         analysis._set_default_metric()
        return res
    ...
    def copy(self, default=None):
        self.ensure_one()
        if self.env.context('action_copy'):
        ...
        analysis = super(SETAAnalysis, self.with_context(copy=True)).copy(default)
        if self.env.context('action_copy'):
            if self.method in ('table_view', 'query') and self.table_id:
                tables = self.env['seta.table'].search([('name', 'like', self.table_id.name)])
                ...
                if not self.env.context('action_copy_from_conversation', False):
                    tables = self.env['seta.table'].search([('name', 'like', self.table_id.name), ('is_stored', '=', True)])
                    new_identifier = str(len(tables) + 1)
                    new_code = ''
            ...
            for metric in self.metric_ids:
                field = self.env['seta.table.field'].search([('field_name', '=', metric.field_id.field_name), ('table_id', '=', analysis.table_id.id)], limit=1)
                if field:
            ...
            for dimension in self.dimension_ids:
                field = self.env['seta.table.field'].search([('field_name', '=', dimension.field_id.field_name), ('table_id', '=', analysis.table_id.id)], limit=1)
                if field:
            ...
            for sort in self.sort_ids:
                field = self.env['seta.table.field'].search([('field_name', '=', sort.field_id.field_name), ('table_id', '=', analysis.table_id.id)], limit=1)
                if field:
                    new_sort = sort.copy({
                        'field_id': field.id,
                    })
    ...
    def get_analysis_data_frame(self, **kwargs):
        ...
        if self.table_id and self.table_id.cron_id and self.table_id.cron_id.code:
            res = self.table_id.cron_id.with_context(seta_table=self, kwargs=kwargs).ir_actions_server_id.run()
    ...
    def get_analysis_data_model(self, **kwargs):
        ...
        if kwargs.get('drilldown_level'):
            ...
            if kwargs.get('drilldown_field'):
                drilldown_field = self.env['seta.table.field'].search([('field_name', '=', kwargs.get('drilldown_field')), ('table_id', '=', self.table_id.id)], limit=1)
                drilldown_visual_type = self.env['seta.visual.type'].search([('name', '=', 'bar')])
                ...
                if drilldown_field.field_type in ['date', 'datetime']:
                    ...
                    drilldown_visual_type = self.env['seta.visual.type'].search([('name', '=', 'line')])
                if drilldown_field:
                    self.drilldown_dimension_ids.unlink()
                    dimensions = self.env['seta.analysis.drilldown.dimension'].create({
                        'analysis_id': self.id,
        ...
        if self.env.context('action_return_domain'):
            return domain
    ...
    def check_special_variable(self, table_query):
        ...
        company_ids = []
        if self._context and self.env.context('allowed_company_ids'):
                ...
                for dynamic_filter in kwargs.get('filters').get('dynamic'):
                    if dynamic_filter.get('field_name') and dynamic_filter.get('operator') and dynamic_filter.get('values'):
                        ...
                        if dynamic_filter.get('field_id'):
                            table_field = self.env['seta.table.field'].browse(dynamic_filter.get('field_id'))
        ...
        try:
            if self.method == 'table':
                if self.table_id and self.table_id.cron_id:
                    res = {
                        'code': 200,
                        'response': self.table_id.cron_id.with_context(seta_table=self.table_id).method_direct_trigger(),
                    }
...
class SETAAnalysisMetric(models.Model):
    _name = 'seta.analysis.metric'
    _description = 'SETA Analysis Metric'
    _order = 'sequence,id'

    sequence = fields.Integer('Sequence')
    analysis_id = fields.Many2one('seta.analysis', string='Analysis', required=True, ondelete='cascade')
    table_id = fields.Many2one('seta.table', string='Table', related='analysis_id.table_id')
    field_id = fields.Many2one('seta.table.field', string='Field', required=True, domain=[('field_type', 'in', ('numeric', 'number'))])
...
class SETAAnalysisDrilldownDimension(models.Model):
    _name = 'seta.analysis.drilldown.dimension'
    _description = 'SETA Analysis Drilldown Demension'
    _order = 'sequence'

    sequence = fields.Integer('Sequence')
    analysis_id = fields.Many2one('seta.analysis', string='Analysis', required=True, ondelete='cascade')
    table_id = fields.Many2one('seta.table', string='Table', related='analysis_id.table_id')
    field_id = fields.Many2one('seta.table.field', string='Field', required=True, domain=[('field_type', 'not in', ('numeric', 'number'))])
    field_type = fields.Char('Field Type', related='field_id.field_type')
...
class SETAAnalysisDimension(models.Model):
    _name = 'seta.analysis.dimension'
    _description = 'SETA Analysis Demension'
    _order = 'sequence,id'
...
class SETAAnalysisFilterTemp(models.Model):
    _name = 'seta.analysis.filter.temp'
    _description = 'SETA Analysis Filter Temp'

    analysis_id = fields.Many2one('seta.analysis', string='Analysis', required=True, ondelete='cascade')
    table_id = fields.Many2one('seta.table', string='Table', related='analysis_id.table_id')
    field_id = fields.Many2one('seta.table.field', string='Field', required=True, ondelete='cascade')
    field_type = fields.Char('Field Type', related='field_id.field_type')
...
class SETAAnalysisFilter(models.Model):
    _name = 'seta.analysis.filter'
    _description = 'SETA Analysis Filter'
    _order = 'id'

    analysis_id = fields.Many2one('seta.analysis', string='Analysis', required=True, ondelete='cascade')
    table_id = fields.Many2one('seta.table', string='Table', related='analysis_id.table_id')
    source_id = fields.Many2one('seta.data.source', string='Data Source', related='analysis_id.source_id')
    source_type = fields.Selection(string='Data Source Type', related='analysis_id.source_id.type')
    field_id = fields.Many2one('seta.table.field', string='Field', required=True, ondelete='cascade')
    operator_id = fields.Many2one(comodel_name='seta.analysis.filter.operator', string='Operator', required=True)
...
class SETAAnalysisFilterOperator(models.Model):
    _name = 'seta.analysis.filter.operator'
    _description = 'SETA Analysis Filter Operator'
    _order = 'id'
...
class SETAAnalysisSort(models.Model):
    _name = 'seta.analysis.sort'
    _description = 'SETA Analysis Sort'
    _order = 'id'

    sequence = fields.Integer(string='Sequence')
    analysis_id = fields.Many2one(comodel_name='seta.analysis', string='Analysis', required=True, ondelete='cascade')
    table_id = fields.Many2one(comodel_name='seta.table', string='Table', related='analysis_id.table_id')
    source_id = fields.Many2one(comodel_name='seta.data.source', string='Data Source', related='analysis_id.source_id')
    source_type = fields.Selection(string='Data Source Type', related='analysis_id.source_id.type')
    field_id = fields.Many2one(comodel_name='seta.table.field', string='Field', required=True)
    field_type = fields.Char(string='Field Type', related='field_id.field_type')
    metric_id = fields.Many2one(comodel_name='seta.analysis.metric', string='Metric', ondelete='cascade')
    dimension_id = fields.Many2one(comodel_name='seta.analysis.dimension', string='Dimension', ondelete='cascade')
    ...
    def create(self, vals_list):
        for vals in vals_list:
            analysis_id = self.env['seta.analysis'].browse(vals.get('analysis_id'))
            ...
            for metric in analysis_id.metric_ids:
                if metric.field_id.id == vals.get('field_id'):
                    ...
                    break
        recs = super(SETAAnalysisSort, self).create(vals_list)
        return recs

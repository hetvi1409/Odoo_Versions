...
# Using SETA Data Processing Tools
...
res = seta.query_fetch('''%(db_query)s''')

seta.query_execute('TRUNCATE %(store_table_name)s')
for r in res:
  seta.query_insert('%(store_table_name)s', r)
"""
...
# # Query example
# start_datetime = seta_table.start_datetime;
# end_datetime = seta_table.end_datetime;
...
# data = seta_table.get_data_query(query=query);
""" % DEFAULT_PYTHON_CODE
...
# seta_table.delete_store_table_data(condition_query=condition_query);
""" % DEFAULT_PYTHON_CODE

DEFAULT_INSERT_PYTHON_CODE = """%s
# seta_table.insert_store_table_data(data=data);
""" % DEFAULT_PYTHON_CODE
...
class SETATable(models.Model):
    _name = 'seta.table'
    _description = 'SETA Table'
    _order = 'name'
    ...
    field_ids = fields.One2many('seta.table.field', 'table_id', string='Fields')
    analysis_ids = fields.One2many(comodel_name='seta.analysis', inverse_name='table_id', string='Analysis')
    ...
    python_code_ids = fields.One2many(comodel_name='seta.table.python.code',
    ...
    def _constraint_name(self):
        for record in self:
            ...
            table_id = self.env['seta.table'].sudo().search(domain)
            if table_id:
    ...
    def create(self, vals_list):
            ...
            if 'is_stored' in vals:
                if vals.get('is_stored'):
                    table_cron = self.env['ir.cron'].sudo().create({
                        ...
                        'model_id': self.env.ref('seta_data.model_seta_table').id,
                        'state': 'code',
                        'code': False,
                        'interval_number': 1,
                        'interval_type': 'days',
        ...
        recs = super(SETATable, self).create(vals_list)
        for rec in recs:
    ...
    def write(self, vals):
        ...
        res = super(SETATable, self).write(vals)
        # User Defined Query / Table View Automatically Get Fields
    ...
    def unlink(self):
        ...
        for analysis_id in self.analysis_ids:
            analysis_id.unlink()
        return super(SETATable, self).unlink()
    ...
    def create_mart_table_from_dataframe(self):
        ...
        if self.user_defined and self.stored_option == 'direct' and self.is_direct and self.cron_id:
            ...
            store_table_name = 'seta_%s' % (re.sub('[^A-Za-z0-9]+', '_', store_name.lower()))
            field_ids = []
            ...
            seta_table.update_schema_store_table()
    ...
    def get_store_table_name(self):
        for seta_table in self:
            seta_table.store_table_name = seta_table.table_name or False
            if seta_table.user_defined and seta_table.is_stored:
                seta_table.store_table_name = 'seta_%s' % (re.sub('[^A-Za-z0-9]+', '_', seta_table.name.lower()))
                seta_table.table_name = 'seta_%s' % (re.sub('[^A-Za-z0-9]+', '_', seta_table.name.lower()))
            # seta_table.store_table_name = ('seta_%s_store_table_%s') % (seta_table.source_id.type, seta_table.id)
    ...
    def get_table_fields_from_dictionary(self, dictionary):
        ...
        for field_record in deleted_field_records:
            self.env['seta.analysis.dimension'].search([('field_id', '=', field_record.id)]).unlink()
            self.env['seta.analysis.metric'].search([('field_id', '=', field_record.id)]).unlink()
            self.env['seta.analysis.sort'].search([('field_id', '=', field_record.id)]).unlink()
            field_record.unlink()
    ...
    def get_table_datas(self):
        self.ensure_one()
        seta_analysis_obj = self.env['seta.analysis']
        ...
        if not self.table_name:
            if self.is_stored:
                ...
                table_query = self.db_query.replace(';', '')
                table_query = seta_analysis_obj.check_special_variable(table_query)
    ...
    def build_schema_store_table(self):
        for seta_table in self:
            if seta_table.is_stored and seta_table.user_defined and seta_table.field_ids:
                table_name = seta_table.store_table_name
                list_fields = []
                for seta_field in seta_table.field_ids:
                    list_fields.append("%s %s" % (seta_field.field_name, seta_field.field_type_origin.upper()))
    ...
    def update_schema_store_table(self):
        for seta_table in self:
            if seta_table.is_stored and seta_table.user_defined and seta_table.field_ids:
                seta_table.destroy_schema_store_table()
                seta_table.build_schema_store_table()
    ...
    def get_start_datetime(self):
        for seta_table in self:
            ...
            if seta_table.store_interval != 'custom':
                if seta_table.store_interval == 'yesterday':
                    start_datetime_obj = start_datetime_obj - relativedelta(days=1)
                elif seta_table.store_interval == 'this_week':
                    start_datetime_obj = start_datetime_obj - timedelta(days=start_datetime_obj.weekday())
                elif seta_table.store_interval == 'last_week':
                    start_datetime_obj = start_datetime_obj - relativedelta(weeks=1)
                    start_datetime_obj = start_datetime_obj - timedelta(days=start_datetime_obj.weekday())
                elif seta_table.store_interval == 'this_month':
                    start_datetime_obj = start_datetime_obj.replace(day=1)
                elif seta_table.store_interval == 'last_month':
                    start_datetime_obj = start_datetime_obj - relativedelta(months=1)
                    start_datetime_obj = start_datetime_obj.replace(day=1)
                elif seta_table.store_interval == 'this_year':
                    start_datetime_obj = start_datetime_obj.replace(day=1, month=1)
                elif seta_table.store_interval == 'last_year':
                    start_datetime_obj = start_datetime_obj - relativedelta(years=1)
                    start_datetime_obj = start_datetime_obj.replace(day=1, month=1)
                start_datetime = start_datetime_obj.strftime('%Y-%m-%d')

                seta_table.start_datetime = '%s 00:00:00' % start_datetime
            else:
                if seta_table.store_interval_custom_type == 'datetime_range':
                    if seta_table.store_start_datetime:
                        start_datetime_obj = seta_table.store_start_datetime

                elif seta_table.store_interval_custom_type == 'unit_of_time':
                    interval_value = seta_table.store_unit_of_time_value
                    if seta_table.store_unit_of_time == 'minutes':
                        start_datetime_obj = start_datetime_obj - relativedelta(minutes=interval_value)
                    elif seta_table.store_unit_of_time == 'hours':
                        start_datetime_obj = start_datetime_obj - relativedelta(hours=interval_value)
                    elif seta_table.store_unit_of_time == 'days':
                        start_datetime_obj = start_datetime_obj - relativedelta(days=interval_value)
                    elif seta_table.store_unit_of_time == 'weeks':
                        start_datetime_obj = start_datetime_obj - relativedelta(weeks=interval_value)
                    elif seta_table.store_unit_of_time == 'months':
                        start_datetime_obj = start_datetime_obj - relativedelta(months=interval_value)
                    elif seta_table.store_unit_of_time == 'years':
                        start_datetime_obj = start_datetime_obj - relativedelta(years=interval_value)

                seta_table.start_datetime = start_datetime_obj.strftime('%Y-%m-%d %H:%M:%S')

    @api.onchange('store_interval', 'store_interval_custom_type', 'store_start_datetime', 'store_end_datetime',
                  'store_unit_of_time', 'store_unit_of_time_value')
    def get_end_datetime(self):
        for seta_table in self:
            if seta_table.start_datetime:
                start_datetime_obj = datetime.strptime(seta_table.start_datetime, '%Y-%m-%d %H:%M:%S')
                ...
                if seta_table.store_interval != 'custom':
                    ...
                    seta_table.end_datetime = '%s 23:59:59' % end_datetime

                else:
                    if seta_table.store_interval_custom_type == 'datetime_range':
                        if seta_table.store_end_datetime:
                            end_datetime_obj = seta_table.store_end_datetime

                    elif seta_table.store_interval_custom_type == 'unit_of_time':
                        end_datetime_obj = datetime.today()

                    seta_table.end_datetime = seta_table.store_end_datetime.strftime('%Y-%m-%d %H:%M:%S')

    def method_direct_trigger(self):
        if self.cron_id:
            # res = self.cron_id.with_context(seta_table=self).method_direct_trigger()
            res = self.cron_id.with_context(seta_table=self).ir_actions_server_id.run()
    ...
    def get_user_defined(self):
        for seta_table in self:
            if seta_table.model_id:
                seta_table.user_defined = False
            else:
                seta_table.user_defined = True
...
class SETATableField(models.Model):
    _name = 'seta.table.field'
    _description = 'SETA Table Field'
    ...
    table_id = fields.Many2one('seta.table', string='Table', required=True, ondelete='cascade')
    foreign_table = fields.Char(string='Foreign Table')
    foreign_column = fields.Char(string='Foreign Column')
    analysis_metric_ids = fields.One2many(comodel_name='seta.analysis.metric',
                                          inverse_name='field_id', string='Analysis Metric')
    analysis_dimension_ids = fields.One2many(comodel_name='seta.analysis.dimension',
                                             inverse_name='field_id', string='Analysis Dimension')
    description = fields.Text('Description')

    _name_source_unique = models.Constraint(
        'unique(field_name, table_id)',
        'Table Field Name Already Exist.',
    )

    def get_field_type_mapping(self, type_origin, source_type):
        field_mapping = self.env['seta.table.field.mapping'].search(
...
class SETATableFieldMapping(models.Model):
    _name = 'seta.table.field.mapping'
    _description = 'SETA Table Field Mapping'
    ...
    _name_source_unique = models.Constraint(
        ...
        'Type Origin Already Exist.',
    )


class SETATablePythonCode(models.Model):
    _name = 'seta.table.python.code'
    _description = 'SETA Table Python Code'
    _order = 'sequence'

    table_id = fields.Many2one(comodel_name='seta.table', string='Table',
    ...
    _table_id_sequence_unique = models.Constraint(

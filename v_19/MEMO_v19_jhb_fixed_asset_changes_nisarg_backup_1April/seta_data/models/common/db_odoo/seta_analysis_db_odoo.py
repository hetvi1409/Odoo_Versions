...
class SETAAnalysisDBOdoo(models.Model):
    ...
    def get_field_metric_format_db_odoo(self, **kwargs):
        ...
        if kwargs.get('field_type') in ('date', 'datetime'):
            if kwargs.get('field_type') == 'datetime' and self.env.context('tz'):
                kwargs['field_name'] = "(%s::timestamp AT time zone 'UTC' AT time zone '%s')" % (kwargs.get('field_name'), self.env.context('tz'))
            field_query = {
...
class SETAAnalysisFilterOperatorDBOdoo(models.Model):
    _inherit = 'seta.analysis.filter.operator'

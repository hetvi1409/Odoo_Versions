...
class SETADataSourceItem(models.Model):
    _name = 'seta.data.source.item'
    _description = 'SETA Data Source Item'

    name = fields.Char(string='Name', required=True)
    type = fields.Selection(string='Type', selection=[], required=True)
    table_id = fields.Many2one('seta.table', string='Table')
    source_id = fields.Many2one('seta.data.source', string='Data Source')
    ...
    def default_get(self, fields_list):
        ...
        if self.env.context('table_id', False):
            table_id = self.env.context('table_id')
            table = self.env['seta.table'].browse(table_id)
...
class SETADataSource(models.Model):
    ...
    item_ids = fields.One2many(comodel_name='seta.data.source.item', inverse_name='source_id', string='Sources')

    _name_unique = models.Constraint(
        'unique(name)',
        'Data Source Name Already Exist.',
    )
    ...
    def get_source_tables(self):
        ...
        table_by_name = {}
        field_by_name = {}
        for seta_table in Table.search([('source_id', '=', self.id), ('model_id', '!=', False)]):

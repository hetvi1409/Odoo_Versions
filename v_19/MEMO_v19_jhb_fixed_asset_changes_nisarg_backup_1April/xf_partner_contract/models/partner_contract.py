...
class PartnerContract(models.Model):
    ...
    def _track_subtype(self, init_values):
        ...
        # Direct state flow checks
        if init_state:
            # Ensure that the state transition is allowed
            allowed_transitions = {
                'draft': ['approval', 'running', 'verify', 'cancelled'],
                'approval': ['running', 'cancelled'],
                'running': ['to_renew', 'cancelled'],
                'to_renew': ['expired', 'cancelled'],
                'expired': ['closed', 'cancelled'],
                'closed': [],
                'cancelled': [],
            }
            print(self.state)
            if init_state in allowed_transitions:
                if self.state not in allowed_transitions[init_state]:
                    raise UserError(
                        _('Incorrect flow! Allowed states from {} are: {}').format(
                            init_state,
                            ', '.join(allowed_transitions[init_state])))

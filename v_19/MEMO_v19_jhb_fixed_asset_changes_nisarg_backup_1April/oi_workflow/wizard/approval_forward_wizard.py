...
class ApprovalForwardWizard(models.TransientModel):
    ...
    def _get_user_id_domain(self):
        if not self.env.context('active_model'):
            return []
        
        model_id = self.env['ir.model']._get(self.env.context('active_model'))
        record = self.env[model_id.model].browse(self.env.context('active_id'))
        
        users = model_id.access_ids.filtered('perm_write').mapped('group_id.user_ids') - record.approval_user_ids
        ...
        for user in users:
            try:
                ...
                pass
        print(ids, 'ids Testing')
        return [('id', 'in', ids)]
                
    model = fields.Char(default = lambda self: self.env.context('active_model'), required = True)
    record_id = fields.Integer(default = lambda self: self.env.context('active_id'), required = True, string='Record ID')

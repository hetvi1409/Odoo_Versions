...
class ApprovalRecord(models.AbstractModel):
    ...
    def _get_state(self):
        print('_get_state')
        print(self._before_approval_states_custom())
        print(self._before_approval_states())
        print(self._approval_states())
        print(self._after_approval_states_custom())
        print(self._after_approval_states())
        print('!!!!!!!!!!!!!!!!')
        # print(self._before_approval_states_custom() or self._before_approval_states()) + \
    ...
    state = fields.Selection(_get_state, string='Status', copy=False, tracking=True, required=True,
                             group_expand='_expand_states')

    state_id = fields.Many2one('approval.config', compute='_calc_state_id', search='_search_state_id',
    ...
    approval_user_ids = fields.Many2many('res.users', string='Waiting User Approval', compute='_calc_approval_user_ids',
                                         search='_search_approval_user_ids')
    # without admin
    approval_user2_ids = fields.Many2many('res.users', string='Waiting User Approval (Without OdooBot)',
    ...
    canceled_record_count = fields.Integer(
        string="#No.", compute='_compute_canceled_record_count',
    ...
    def _compute_canceled_record_count(self):
        for record in self:
            record.canceled_record_count = record.env['cancellation.record'].sudo().search_count(
    ...
    def _before_approval_states_custom(self):
        model_id = self.env['ir.model']._get_id(self._name)
        states = self.env['approval.settings.state'].sudo().search(
            [('settings_id.model_id', '=', model_id), ('type', '=', 'before'), ('active', '=', True)])
        if states:
    ...
    def _before_approval_states(self):
        return self._before_approval_states_custom() or [('draft', 'Draft')]

    @api.model
    def _after_approval_states_custom(self):
        model_id = self.env['ir.model']._get_id(self._name)
        states = self.env['approval.settings.state'].sudo().search(
            [('settings_id.model_id', '=', model_id), ('type', '=', 'after'), ('active', '=', True)])
        if states:
            return [(state.state, state.name) for state in states]
        return False
    ...
    def _after_approval_states(self):
        return self._after_approval_states_custom() or [('approved', _('Approved')), ('rejected', _('Rejected')),
                                                        ('canceled', _('Canceled'))]
    ...
    def _get_reject_state(self):
        model_id = self.env['ir.model']._get_id(self._name)
        state = self.env['approval.settings.state'].search(
            [('settings_id.model_id', '=', model_id), ('type', '=', 'after'), ('active', '=', True),
             ('reject_state', '=', True)], limit=1)
    ...
    def _calc_state_id(self):
        model_id = self.env['ir.model']._get_id(self._name)
        for record in self:
            record.state_id = self.env['approval.config'].search(
                [('model_id', '=', model_id), ('state', '=', record.state)], limit=1).id
    ...
    def _get_delegation(self, user_ids, group_ids):
        ...
        if 'delegation' in self.env:
            ...
            for delegation in self.env['delegation.line'].sudo().get(
                    [('delegator_user_id', 'in', user_ids.ids), ('group_id', 'in', group_ids.ids)]):
    ...
    def _calc_approval_user_ids(self):
        ...
        for record in self:
            ...
            if state_id:
                ...
                approval_forward_id = self.env['approval.forward'].search(
                    [('model_id', '=', model_id), ('record_id', '=', record.id), ('state_id', '=', state_id.id),
                     ('active', '=', True)])
                if approval_forward_id:
                    ...
                    if state_id.filter_condition:
                        user_ids = user_ids.filtered(
                            lambda user_id: record._match_condition_user(state_id.filter_condition, user_id,
                                                                         state_id.group_ids.ids, delegation_filter))
                ...
                user_ids = user_ids.filtered(
                    lambda user: user != self.env.ref('base.user_root') and user != self.env.ref('base.user_admin'))
    ...
    def _search_approval_user_ids(self, operator, value):
        ...
        model_id = self.env['ir.model']._get_id(self._name)
        state_ids = self.env['approval.config'].search(
            [('model_id', '=', model_id), ('group_ids', 'in', user_ids.mapped('wkf_groups_ids.id'))])
        state_ids += self.env['approval.forward'].search(
            [('model_id', '=', model_id), ('user_id', 'in', user_ids.ids), ('active', '=', True)]).mapped('state_id')
        ids = []
        for state_id in state_ids:
    ...
    def _schedule_approval_activity(self, users=None):
        ...
        for user in users:
            self.env['mail.activity'].sudo().with_context(disable_message_subscribe=True,
    ...
    def _remove_approval_activity(self, action=None, reason=None, old_state_id=None, user_id=None, forward_user=None):
        model_id = self.env['ir.model']._get_id(self._name)
        self.env['approval.forward'].sudo().search(
            [('model_id', '=', model_id), ('record_id', 'in', self.ids), ('active', '=', True)]).write(
            {'active': False})
        for record in self:
            ...
            activity_ids.unlink()
            post_msg = (action == 'approved' and old_state_id.post_approval_msg) or (
                        action == 'rejected' and old_state_id.post_reject_msg) \
            ...
            if action and activity_record._isinstance('mail.thread') and post_msg:
                ...
                activity_record.message_post_with_view(view_xmlid,
                                                       ...
                                                       mail_activity_type_id=self.env.ref(
                                                           'oi_workflow.activity_type_approval').id)
    ...
    def action_approve(self):
        ...
        if self.state_id.is_sudo_approve:
            self = self.sudo()
        ...
        append_actions(self._action_approve())
    ...
    def _action_approve(self):
        ...
        for record in self:
            ...
            if not nextstate:
                ...
                actions.append(record._on_script_custom('on_approval', old_state=old_state,
                                                        new_state=record.state) or record._on_approval(old_state,
                                                                                                       record.state))
                if record.stop_approval_trigger:
                    continue
            else:
            ...
            if old_state != record.state:
                ...
                if not self.env.context.get('approval_no_mail'):
    ...
    def action_return(self):
        ...
        if self.env.context('fixed_return_state'):
            context.update({
                'default_state': self.env.context('fixed_return_state'),
            })
        model_id = self.env['ir.model']._get(self._name)
        return {
            'type': 'ir.actions.act_window',
    ...
    def action_transfer(self):
        ...
        if self.env.context('fixed_transfer_state'):
            context.update({
                'default_state': self.env.context('fixed_transfer_state'),
            })
        model_id = self.env['ir.model']._get(self._name)
        return {
            'type': 'ir.actions.act_window',
    ...
    def _action_forward(self, user_id, reason):
        ...
        for record in self:
            record._schedule_approval_activity()
            actions.append(record._on_script_custom('on_forward', user_id=user_id, reason=reason) or record._on_forward(
                user_id=user_id, reason=reason))

            if not self.env.context('approval_no_mail'):
                for template_id in record.state_id.template_ids:
                    template_id.with_context(active_model=self._name, reason=reason,
                                             forwarder_user=self.env.user).send_mail(record.id)
    ...
    def _action_return(self, state, reason):
        ...
        for record in self:
            record._schedule_approval_activity()
            actions.append(record._on_script_custom('on_return', new_state=state, old_state=old_states[record],
                                                    reason=reason) or record._on_return(new_state=state,
    ...
    def _action_transfer(self, state, reason):
        ...
        for record in self:
            record._schedule_approval_activity()
            actions.append(record._on_script_custom('on_transfer', new_state=state, old_state=old_states[record],
                                                    reason=reason) or record._on_transfer(new_state=state,
                                                                                          old_state=old_states[record],
                                                                                          reason=reason))
    ...
    def _expand_states(self, states, domain, order):
        ...
        if self.env.context('expand_states_full'):
            return values
    ...
    def get_approval_info(self):
        ...
        data = {
            'lines': [],
            'show_login_as': bool(
                self.env.user._is_system() and self.env['ir.module.module']._installed().get('oi_login_as'))
        }
    ...
    def action_approve_all(self):
        if self.env.context('active_model') != self._name:
            _logger.warning(f"active_model ({self.env.context('active_model')}) != model name")
            return

        if isinstance(self.env.context('active_domain'), list):

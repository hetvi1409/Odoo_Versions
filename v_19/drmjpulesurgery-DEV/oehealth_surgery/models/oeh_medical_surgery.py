...
class OeHealthSurgery(models.Model):
    ...
    name = fields.Char(string='Surgery #', size=64, required=True, default=lambda *a: '/')
    patient = fields.Many2one('oeh.medical.patient', string='Patient', help="Patient Name", required=True,
                              )
    admission = fields.Many2one('oeh.medical.appointment', string='Admission', help="Patient Name", )
    ...
    institution = fields.Many2one('oeh.medical.health.center', string='Health Center', help="Health Center",
                                  required=True, )
    ...
    operating_room = fields.Many2one('oeh.medical.health.center.ot', string='Operation Theater', required=True,
                                     )
    ...
    def create(self, vals):
        if isinstance(vals, list):
            for val in vals:
                if val.get('name', '/') == '/':
                    val['name'] = self.env['ir.sequence'].next_by_code('oeh.medical.surgery')
        else:
            if vals.get('name', '/') == '/':

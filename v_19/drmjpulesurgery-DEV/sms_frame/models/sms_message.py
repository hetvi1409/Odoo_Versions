...
class SmsMessage(models.Model):
    ...
    def _compute_record_name(self):
        ...
        for rec_sms in self:
            if rec_sms.model_id.model != False and rec_sms.record_id != False:
                my_record_count = self.env[rec_sms.model_id.model].search_count([('id','=',rec_sms.record_id)])
                if my_record_count > 0:
                    my_record = self.env[self.model_id.model].search([('id','=',rec_sms.record_id)])
                    rec_sms.record_name = my_record.name
                else:
                    rec_sms.record_name = rec_sms.to_mobile

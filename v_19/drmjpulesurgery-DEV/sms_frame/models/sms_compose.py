...
class SmsCompose(models.Model):
    ...
    model = fields.Char()
    sms_template_id = fields.Many2one('seta.sms.template', string="Template")
    ...
    def _onchange_sms_template_id(self):
        ...
        if self.sms_template_id.id != False:
            sms_rendered_content = self.env['seta.sms.template'].template_render(self.sms_template_id.template_body, self.sms_template_id.model_id.model, self.record_id)
            # self.from_mobile_id = self.sms_template_id.from_mobile_verified_id.id
            # template = self.env['seta.sms.template'].create({'name':'test sms temp',
            #                                             'body':'Hello'})
            # sms_render = template.template_render(self.sms_template_id.template_body, self.sms_template_id.model_id.model, self.record_id)
    ...
    def send_entity(self):
        ...
        sms_subtype = self.env['ir.model.data']._xmlid_to_res_id('sms_frame.sms_subtype')
        attachments = []
        if self.media_id:

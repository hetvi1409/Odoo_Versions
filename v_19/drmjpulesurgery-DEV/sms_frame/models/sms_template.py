...
class SmsTemplate(models.Model):
    ...
    def _valid_field_parameter(self, field, name):
        return name in ('tracking', 'placeholder') or super()._valid_field_parameter(field, name)
    ...
    def send_sms(self, template_id, record_id):
        ...
        sms_template = self.env['seta.sms.template'].browse( int(template_id) )
        rendered_sms_template_body = self.env['seta.sms.template'].template_render(sms_template.template_body, sms_template.model_id.model, record_id)
        rendered_sms_to = self.env['seta.sms.template'].template_render(sms_template.sms_to, sms_template.model_id.model, record_id)
        ...
        self.env['ir.model.data']._xmlid_to_res_id('sms_frame.sms_queue_check').active = True
        # self.env['ir.model.data'].get_object('sms_frame', 'sms_queue_check').active = True

    def template_render(self, template, model, res_id):

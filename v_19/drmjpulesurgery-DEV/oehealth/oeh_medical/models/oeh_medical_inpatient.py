...
class OeHealthInpatient(models.Model):
    ...
    name = fields.Char(string='Inpatient #', size=128, readonly=False, required=False, default=lambda *a: '/')
    patient = fields.Many2one('oeh.medical.patient', string='Patient', help="Patient Name", required=False, readonly=False, states={'Draft': [('readonly', False)]})
    institution = fields.Many2one('oeh.medical.health.center', string='Health Center', readonly=False, states={'Draft': [('readonly', False)]})
    admission_type = fields.Selection(ADMISSION_TYPE, string='Admission Type', required=False, readonly=False, states={'Draft': [('readonly', False)]})
    admission_reason = fields.Many2one('oeh.medical.pathology', string='Reason for Admission', help="Reason for Admission", required=False, readonly=False, states={'Draft': [('readonly', False)]})
    admission_date = fields.Datetime(string='Hospitalization Date', readonly=False, states={'Draft': [('readonly', False)]})
    discharge_date = fields.Datetime(string='Discharge Date', readonly=False, states={'Discharged': [('readonly', True)], 'Invoiced': [('readonly', True)]})
    attending_physician = fields.Many2one('oeh.medical.physician', string='Attending Doctor', readonly=False, states={'Draft': [('readonly', False)]}, default=_get_physician)
    operating_physician = fields.Many2one('oeh.medical.physician', string='Operating Doctor', readonly=False, states={'Draft': [('readonly', False)]})
    ward = fields.Many2one('oeh.medical.health.center.ward', string='Ward', required=False, readonly=False, states={'Draft': [('readonly', False)]}, domain="[('institution', '=', institution)]")
    bed = fields.Many2one('oeh.medical.health.center.beds', string='Bed', required=False, readonly=False, states={'Draft': [('readonly', False)]})
    ...
    admission_condition = fields.Text(string='Condition before Admission', readonly=False, states={'Invoiced': [('readonly', True)]})
    ...
    prescribed_medicines = fields.One2many('oeh.medical.inpatient.prescribed.medicine', 'inpatient_id',
                                           string='Prescribed Medicines', readonly=False,
    ...
    consumed_medicines = fields.One2many('oeh.medical.inpatient.consumed.medicine', 'inpatient_id',
                                         string='Consumed Medicines', readonly=False,
                                         states={'Hospitalized': [('readonly', False)]})

    ...
    def create(self, vals):
        if isinstance(vals, list):
            for vals_item in vals:
                vals_item["is_pharmacy"] = True
                vals_item["is_company"] = True
        else:
...
class OeHealthPharmacyLines(models.Model):
    ...
    #         # Create Invoice
    #         if pres.patient:
    #             invoice_lines = self.get_prescription_invoice_lines()
    #             default_journal = self._get_default_journal()
    #
    #             # Create Invoice
    #             invoice = self.env['account.move'].sudo().create({
    ...
    #                 'invoice_date': datetime.datetime.now().date(),
    #                 'date': datetime.datetime.now().date(),
    #                 'ref': "Prescription # : " + pres.name.name,
    #                 'prescription': pres.name.id,
    #                 'invoice_line_ids': invoice_lines
    #             })
    #         else:
    #             raise UserError(_('No patient selected !!'))

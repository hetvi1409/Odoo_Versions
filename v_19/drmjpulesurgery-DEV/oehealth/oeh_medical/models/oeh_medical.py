...
class OeHealthPatient(models.Model):
    ...
    clinical_sketchpad = fields.Html(string="clinical_sketchpad")
    clinical_note_ids = fields.One2many(
        "clinical.notes",
        "patient_id",
        string="Clinical Notes"
    )
    clinical_sketchpad_ids=fields.One2many(
        "patient.sketch.note",
        "patient_sketch_note_id",
        string="Clinical Sketch Notes"
    )
    note_count = fields.Integer(compute='_note_count', string="Clinical Notes")
    sketch_count = fields.Integer(compute='_sketch_count', string="Clinical Notes")
    ...
    def action_sketchpad(self):
        return {
            ...
            'view_mode': 'list,form',
            'target': 'current',
            'domain': [('patient_sketch_note_id', '=', self.id)],
            'context': {'default_patient_sketch_note_id': self.id},
        }
    ...
    def _note_count(self):
        for patient in self:
            patient.note_count = self.env['clinical.notes'].search_count([('patient_id', '=', patient.id)])

    def _sketch_count(self):
        for patient in self:
            patient.sketch_count = self.env['patient.sketch.note'].search_count([('patient_sketch_note_id', '=', patient.id)])
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            if not vals.get('identification_code'):
                vals['identification_code'] = sequence.next_by_code('oeh.medical.patient')
            vals['is_patient'] = True
        health_patients = super(OeHealthPatient, self).create(vals_list)
        for health_patient in health_patients:
    ...
    def _compute_attachment_count(self):
        ...
        for patient in self:
            patient.attachment_count = mapped_data.get(patient.id, 0)
        ...
        return {
            ...
            'help': """
                <p class="o_view_nocontent_smiling_face">
                    Add attachments for this patient
                </p>
            """,
        }


    def print_patient_sick_note(self):
        return self.env.ref('oehealth.action_report_patient_sick_note').report_action(self)
...
class OeHealthPhysician(models.Model):
    ...
    code = fields.Char(string='HPCSA No', help="Medical Staff License Number")
    ...
    practise_number = fields.Char("Practice Number")
    consultancy_start_time = fields.Char("Consultation Start Time")
    ...
    def print_medical_staff_badge(self):
        return self.env.ref('oehealth.action_report_medical_staff_badge').report_action(self)
...
class OeHealthAppointment(models.Model):
    ...
    appointment_end = fields.Datetime(string='Appointment End Date', readonly=True,
                                      states={'Scheduled': [('readonly', False)]})
    ...
    # partner_ids = fields.Char(string='Partners', compute='compute_partners_for_mail')
    mail_partner_ids = fields.Many2many(
        'res.partner',
        string='Mail Partners',
    ...
    def _compute_mail_partners(self):
        for rec in self:
            ...
            if rec.patient and rec.patient.partner_id:
                partners |= rec.patient.partner_id
            if rec.doctor and rec.doctor.user_id.partner_id:
                partners |= rec.doctor.user_id.partner_id
            rec.mail_partner_ids = partners
    ...
    #     partners = ''
    #     if self.patient:
    #         partners += str(self.patient.partner_id.id) + ','
    #         print("===========================patient======================", partners)
    #     if self.doctor.user_id:
    #         print("========================================525==================")
    #         partners += str(self.doctor.user_id.partner_id.id)
    #         print("========526=======", partners)
    #     self.partner_ids = partners
    #     print("=====532====", self.partner_ids)

    @api.depends('patient')
    def check_source_available(self):
        # for rec in self:
        source = self.env['oeh.medical.telemedicine.sources'].search([])
        print("====================514=============", self.check_source)
        if source:
            self.check_source = True
            print("====================51777777777777777=============", self.check_source)
        else:
    ...
    def action_meeting_invitation_send(self):
        ...
        # print("===============554=================", partner_ids)
        template_id = self.env['ir.model.data']._xmlid_to_res_id('oehealth.oeh_email_template_meeting_invitation',
                                                                raise_if_not_found=False)
        print("==================544=====================", template_id)
    ...
    def default_get(self, default_fields):
        # OVERRIDE
        values = super(OeHealthAppointment, self).default_get(default_fields)
    ...
    def create(self, vals_list):
        ...
        if not sequence:
            sequence = self.env['ir.sequence'].sudo().create({
                'name': f'Appointments ({company.name})',
                'code': 'oeh.medical.appointment',
                'company_id': company.id,
                'prefix': 'AP',
                'padding': 4,
        ...
        for vals in vals_list:
            ...
            # Check availability
            if vals.get('doctor') and vals.get('appointment_date'):
                self.check_physician_availability(
                    vals.get('doctor'),
...
class OeHealthPrescriptions(models.Model):
    ...
    def create(self, vals_list):
        ...
        return super(OeHealthPrescriptions, self).create(vals_list)
    ...
    def action_prescription_send(self):
        ...
        self.ensure_one()
        template_id = self.env['ir.model.data']._xmlid_to_res_id('oehealth.oeh_email_template_prescription', raise_if_not_found=False)
        lang = self.env.context.get('lang')
        template = self.env['mail.template'].browse(template_id)
        if template.lang:
    ...
    def _get_share_url(self, redirect=False, signup_partner=False, pid=None,share_token=False):
        self.ensure_one()
        print("=====================get share============",self.partner_id)
        auth_param = url_encode(self.partner_id.signup_get_auth_param()[self.partner_id.id])
        print("======================auth param==================",auth_param)

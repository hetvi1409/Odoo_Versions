...
class OeHealthInjuryExaminationProcedure(models.Model):
    ...
    name = fields.Char(string='Case #', size=128, readonly=True, default=lambda *a: '/')
    patient = fields.Many2one('oeh.medical.patient', string='Patient', help="Patient Name", domain=[('deceased','=',False)], required=True, states={'Draft': [('readonly', False)]})
    date = fields.Datetime(string='Injury Date', readonly=True, states={'Draft': [('readonly', False)]}, default=datetime.datetime.now())
    age = fields.Char(compute=_patient_age, size=64, string='Age at the time of Injury')
    doctor = fields.Many2one('oeh.medical.physician', string='Doctor', domain=[('is_pharmacist','=',False)], states={'Draft': [('readonly', False)]}, default=_get_physician)
    ...
    injury_type = fields.Selection(INJURY_TYPE, 'Type of Injury', readonly=True, states={'Draft': [('readonly', False)]}, default=lambda *a: 'Unintentional / Accidental')
    ...
    injury_details = fields.Text(string='Injury Details',readonly=True, states={'Draft': [('readonly', False)]})
    address = fields.Char(string='Address of Injury Occurrence', states={'Draft': [('readonly', False)]})
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            sequence = sequence_model.next_by_code('oeh.medical.injury.examination')
            vals['name'] = sequence or '/'
        injury_no = super(OeHealthInjuryExaminationProcedure, self).create(vals_list)
        return injury_no
    ...
    def print_injury_examination_report(self):
        return self.env.ref('oehealth.action_oeh_medical_report_injury_examination_result').report_action(self)

...
class account_move_line(models.Model):
    ...
    def auth_code_validation(self):
        for line in self:
            if line.authorization_code:
                if len(line.authorization_code) > 20:
                    raise ValidationError('This field can be a maximum of 20 character, please strip any white space or special characters')
...
class AccountMove(models.Model):
    ...
    medical_scheme = fields.Many2one('res.partner', 'Medical Scheme')
    responding_party = fields.Char(string="Responding Party")
    authorization_code = fields.Char(string="Auth Code")
    response_payload = fields.Text(string="Response Payload")
    test_response_payload = fields.Text(string="Test Payload Response")
    benefit_check = fields.Boolean(related='company_id.benefit_check')

    @api.constrains('authorization_code')
    def auth_code_validation(self):
        if self.authorization_code and len(self.authorization_code) > 20:
            raise ValidationError('This field can be a maximum of 20 character, please strip any white space or special characters')

    @api.onchange('partner_id')
    def onchange_medical_scheme(self):
        for rec in self:
            self.medical_scheme = rec.partner_id.medical_aid_id.id
...
P|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|\n""" % (
                ...
                invoice.patient_id.id_number or '', recall_exam_date or '', '', '', '', '', '', '', '', invoice.authorization_code or '',
                '', '01', '', '', invoice.id or '',
            )
            count = 1
            f1 = 'F'
            ...
            f12 = 0
            t11 = False
            invoice_line = self.env['account.move.line'].search([('id', '=', invoice.invoice_line_ids.ids),
            ...
            for line in invoice_line:
                ...
                paylod += """T|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|\n""" % (
                    ...
                    count, invoice_date, invoice_date, line.authorization_code or '', invoice.id or '',
                    line.id, '02',
                    ...
                    '', '', '', '', '', '', '11')
                # OP records
                if line.final_rx_id and not line.contact_final_rx_id:
                ...
                if not line.final_rx_id and line.contact_final_rx_id:
                    rx_data = line.contact_final_rx_id
                    ...
                    paylod += """OP|{v2}|{v3}|{v4}|{v5}|{v6}|{v7}|{v8}|{v9}|{v10}|{v11}|{v12}|{v13}|{v14}|{v15}|\n""".format(
                        ...
                        v9=int(rx_data.od_cyl * 100) or '',
                        v10=int(rx_data.od_axis * 100) or '',
                        v11=int(float(rx_data.od_add) * 100) or '',
                        v12='',
                        v13='', v14='',
                        v15=v4_val or '',
                    )
                    ...
                    paylod += """OP|{v2}|{v3}|{v4}|{v5}|{v6}|{v7}|{v8}|{v9}|{v10}|{v11}|{v12}|{v13}|{v14}|{v15}|\n""".format(
                        ...
                        v9=int(rx_data.os_cyl * 100) or '',
                        v10=int(rx_data.os_axis * 100) or '',
                        v11=int(float(rx_data.os_add) * 100) or '',
                        v12='',
                        v13='', v14='',
                        v15=v4_val or '',
                    )
                ...
                discount = ((((line.discount / 100) * (line.price_unit * line.quantity))) * 100) if line.discount else 0.0
                z1 = 'Z'
                z2 = str(int(line.price_unit * line.quantity * 100))
                ...
                z9 = ''
                z10 = ''
                z3 = str(int(z2 or 0) + int(z4 or 0) + int(z5 or 0) + int(z6 or 0) + int(z7 or 0) + int(z8 or 0) + int(
                    z9 or 0) + int(z10 or 0))
                z12 = str(int(discount))
                z11 = str(int(z3 or 0) - int(z12 or 0))
                z13 = ''
                z14 = ''
                z15 = ''
                z16 = str(int(z13 or 0) + int(z14 or 0) + int(z15 or 0))
                z17 = str(int(z11 or 0) - int(z16 or 0))
                z18 = ''
                ...
                f12 += int(z18 or 0)
                count += 1
            f4 = str(int(f3 or 0) - int(f5 or 0))
            paylod += """{}|{}|{}|{}|{}|{}|{}|{}|{}|{}|{}|{}|\n""".format(
                str(f1 if f1 else ''), str(f2 if f2 else ''), str(f3 if f3 else ''), str(f4 if f4 else ''), 
                str(f5 if f5 else ''), str(f6 if f6 else ''),
                str(f7 if f7 else ''), str(f8 if f8 else ''), str(f9 if f9 else ''),
                str(f10 if f10 else ''), str(f11 if f11 else ''), str(f12 if f12 else ''))
            paylod += """E|%s|%s|%s|\n""" % (
                # Footer (End of Message) Record – Type ‘E’
                invoice.id or '', '1', int(f2),
            )
            paylod = self.env['speacial.charcters'].speacial_char_escape(paylod)
            if self._context.get('is_testing'):
                invoice.write({'test_response_payload': paylod.strip()})
            else:
                ...
                if cmp_id.select_url == 'production_url':
                    ...
                    raise ValidationError(_('Mediswitch Gateway Offline!!'))
                    ...
                    if lines and lines[0] and lines[0].startswith('H'):
                        ...
                        if total_approved_price > 0 or (
                            ...
                            if invoice.company_id.do_payment:
                                ...
                                payment_transaction = self.env['payment.transaction'].default_get(['type'])
                                if payment_transaction.get('type') == 'out_invoice':
                                    vals['type'] = 'form'
                                ...
                                if not invoice.company_id.journal_id:
                                    raise ValidationError(_("Please Select Journal for Mediswitch Payment...!!"))
...
P|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|\n""" % (
            ...
            for line in invoice_line:
                ...
                discount = ((((line.discount / 100) * line.price_total)) * 100) if line.discount else 0.0
                z1 = 'Z'
                z2 = str(int(line.price_total * 100))
                z4 = ''
                z5 = ''
                z6 = ''
                z7 = ''
                ...
                z9 = ''
                z10 = ''
                z3 = str(int(z2 or 0) + int(z4 or 0) + int(z5 or 0) + int(z6 or 0) + int(z7 or 0) + int(z8 or 0) + int(z9 or 0) + int(z10 or 0))
                z12 = str(int(discount))
                z11 = str(int(z3 or 0) - int(z12 or 0))
                z13 = ''
                z14 = ''
    ...
    def id_msv(self):
        ...
        for partner in self:
            ...
            if not partner.id_number:
                raise ValidationError(_('Please enter the ID Number of the patient before you can perform the ID MSV'))
            if not partner.medical_aid_id.msv_allowed:
                raise ValidationError(_('Sorry, MSV is not enabled for the patients Medical Aid.'))
...
E|%s|%s|\n""" % (
            ...
            if responsepayload:
                ...
                strings = ("Invalid Missing", "Invalid")
                if any(s in lines[0] for s in strings):
                    raise ValidationError(_(lines[0]))
                if lines and lines[0] and lines[0].startswith('H'):
                    for line in lines:
...
class ProductTemplate(models.Model):
    ...
    def _constrains_default_code(self):
        if self.default_code and len(self.default_code) > 15:
            raise ValidationError(_('Valid Length for Products default code is 15.'))

...
class MediswitchSubmitClaim(models.Model):
    ...
    def fetch_operations(self):
        if self.env.context.get('fetch_response'):
            ...
            invoice_ids = self.env['account.move'].search([('claim_level_mediswitch_status', 'in', ['01', '02'])])
            ...
            for node in response_string.iter():
                if node.tag == 'originalSwref':
                    ...
                    if node.text and node.text == 'Switch System Error':
                        raise ValidationError(_('email Humint Support for assistance. support@humint.co.za or log a ticket at http://www.humint.co.za/helpdesk/humint-2/submit'))
                    ...
                    raise ValidationError(_(node.text))
                else:
...
class MediswitchFetchClaim(models.Model):
    ...
    claim_ref_id=fields.Many2one("mediswitch.submit.claim",string="Claim Ref",readonly="True")
...
class Globalfetch(models.Model):
    ...
    def fetch_response(self):
        ...
        #     url = data[0]
        # else:
        #     raise ValidationError(_('Mediswitch Gateway Busy!!'))
...
class BulkMsv(models.Model):
    ...
    def bulk_msv(self):
        ...
        for partner in self.env['res.partner'].search([('msv_later_button','=', True),('customer_rank','>',0)]):
...
class MsvMembers(models.Model):
    ...
    medical_id = fields.Many2one("res.partner", string="Medical Aid")
    is_medical_aid = fields.Boolean()
    medical = fields.Char()

    def search_record(self):
        if self.msv_response_id.name:
            medical_detail = self.env['res.partner'].search([('name', 'ilike', self.msv_response_id.name),
                                                             ('is_a_medical_aid', '=', True)])
            if len(medical_detail) == 1:
                self.is_medical_aid = False
                self.medical = False
                self.medical_id = False
            else:
                self.is_medical_aid = True
                self.medical = self.msv_response_id.name.split(" ")[0]
    ...
    def update_record(self):
        if not self.medical_id and self.is_medical_aid:
            raise ValidationError(_('Please add medical aid.'))
        medical_detail = False
        if self.is_medical_aid:
            medical_detail = self.env['res.partner'].search(
                [('name', '=', self.medical_id.name), ('is_a_medical_aid', '=', True)])
        else:
            if self.msv_response_id.name:
                medical_detail = self.env['res.partner'].search([('name', 'ilike', self.msv_response_id.name),
                                                                 ('is_a_medical_aid', '=', True)])

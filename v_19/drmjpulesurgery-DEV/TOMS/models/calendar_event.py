...
class CalendarSmsAlarm(models.Model):
    _name = 'calendar.sms.alarm'
...
class calendar_event(models.Model):
    ...
    patient_number = fields.Char(related='customer_id.patient_number')
    ...
    is_no_notification = fields.Boolean('No Notification',related='meeting_subject_id.no_notification')
    ...
    sms_reminder = fields.Many2many('calendar.sms.alarm', 'sms_reminder_calendar_alarm_calendar_event_rels',
                                    string='SMS Reminders')

    def send_sms(self, each_sms_rec, appointment_rec):
        sms_rendered_content = self.env['sms.template'].render_template(each_sms_rec.sms_template.template_body,
                                                                        each_sms_rec.sms_template.model_id.model,
                                                                        appointment_rec)
        sms_compose = self.env['sms.compose'].create({
            'sms_template_id': each_sms_rec.sms_template.id,
            # 'from_mobile_id': each_sms_rec.sms_template.id,
            'to_number': appointment_rec.mobile,
            'sms_content': sms_rendered_content,
            # 'media_id': template_body,
            'model': self._name,
            'record_id': appointment_rec.id
        })
        if sms_compose:
            sms_compose.with_context(from_cron=True).send_entity()

    def calendar_sms_alarm_reminder(self):
        for appointment_rec in self.search([('is_no_notification', '=', False), ('mobile', '!=', False)]):
            sms_reminder_list = []
            for each_sms_rec in appointment_rec.sms_reminder:
                if each_sms_rec.interval == 'days':
                    if datetime.now().replace(second=0, microsecond=0) + timedelta(
                            days=each_sms_rec.duration) >= appointment_rec.start_datetime:
                        self.send_sms(each_sms_rec, appointment_rec)
                        sms_reminder_list.append(each_sms_rec.id)

                if each_sms_rec.interval == 'hours':
                    if datetime.now().replace(second=0, microsecond=0) + timedelta(
                            hours=each_sms_rec.duration) >= appointment_rec.start_datetime:
                        self.send_sms(each_sms_rec, appointment_rec)
                        sms_reminder_list.append(each_sms_rec.id)

                if each_sms_rec.interval == 'minutes':
                    if datetime.now().replace(second=0, microsecond=0) + timedelta(
                            minutes=each_sms_rec.duration) >= appointment_rec.start_datetime:
                        self.send_sms(each_sms_rec, appointment_rec)
                        sms_reminder_list.append(each_sms_rec.id)

            for Reminders in sms_reminder_list:
                appointment_rec.sms_reminder = [(2, Reminders)]
    ...
    def create(self, vals):
        if vals.get('optometrist_id') and vals.get('start_datetime'):
            if any(rec for rec in self.env['calendar.event'].search([('optometrist_id', '=', vals.get('optometrist_id')),
                ...
                raise ValidationError(_('This appointment clashes with another one for the Optometrist. Please select a different time.'))
        res = super(calendar_event, self).create(vals)
        if not vals.get('optometrist_id') and vals.get('frontliner_id'):
            vals.update({'optometrist_id': vals.get('frontliner_id')})
        res_partner_rec = self.env['res.partner'].browse(vals.get('customer_id'))
        if res_partner_rec:
            res_partner_rec.write({
                'meeting_ids': [(4, res.id)]
            })
        return res
...
E|%s|%s|\n""" % (
            ...
            try:
                ...
                raise ValidationError(_('502 Bad Gateway'))
                ...
                if lines and lines[0] and lines[0].startswith('H'):
                    for line in lines:
                        if line.startswith('RV'):
                            ...
                            if split_line[1] == '01':
                                raise ValidationError(_(split_line[5]))
                if lines and lines[0] and lines[0].startswith('H'):
                    for line in lines:
...
class meeeting_subject(models.Model):
    ...
    no_notification = fields.Boolean(string="No Notification")

class event_alarm_manager(models.AbstractModel):
    ...
    def get_next_mail(self):
        ...
        for meeting in self.env['calendar.event'].browse(all_meetings):
            if not meeting.no_notification:
...
class CrmmLead(models.Model):
    ...
    def action_compose_sms_phone(self):
        ...
        return {
            ...
            'target': 'new',
            'type': 'ir.actions.act_window',
            'context': {'default_to_number': self.phone,

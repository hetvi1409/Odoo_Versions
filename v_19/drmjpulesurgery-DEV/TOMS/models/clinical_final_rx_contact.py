...
class clinical_final_rx_contact(models.Model):
    ...
    @api.onchange('contact_lens_trial_final')
    def dispense_boolean(self):
        if self.contact_lens_trial_final == 'trial':
            self.dispense = False
        else:
            self.dispense = True
    ...
    def create(self, vals):
        ...
        if res.final_rx_id and res.dispense:
            rec = res.read()[0]
            rec.update({'final_rx_id': False, 'final_rx_dis_id': res.final_rx_id.id, 'old_id1': res.id})
            self.create(rec)
        return res
    ...
    def write(self, vals):
        ...
        for each in self:
            rec = self.search(['|', ('old_id1', '=', each.id), ('old_id', '=', each.id)])
            if each.dispense and not each.final_rx_dis_id:
                value = each.read()[0]
                value.update({'final_rx_id': False, 'final_rx_dis_id': each.final_rx_id.id, 'old_id1': each.id})
                if rec:
                    rec.write(value)
                else:
                    rec.create(value)
...
#                             ('Final Contact Lens', 'Final Contact Lens')], string="Name")
    ...
    old_id1 = fields.Many2one('clinical.final.rx.contact')
    ...
    def add_to_job_queue(self):
        ...
        final_rx_po_lines = self.clinical_final_rx_ids.filtered(lambda l: ("BS" or "bs") not in l.product_id.default_code)
...
class wizard_final_rx_line_contact(models.TransientModel):
    ...
    def product_id_change(self):
        ...
        if float(final_rx_id.od_add) > 0 and float(final_rx_id.os_add) > 0:

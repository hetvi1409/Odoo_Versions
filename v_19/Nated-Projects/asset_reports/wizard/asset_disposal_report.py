...
class AssetDisposalReports(models.TransientModel):
    ...
    def get_xlsx_report(self, data, response):
        ...
        sql_query = """SELECT a.identification_number, a.description, 
                    a.acquisition_date, a.disposal_date, a.disposal_type, 
                    a.asset_reason, a.asset_barcode, a.condition, 
                    department.name, division.name, a.deed_number, a.custodian,
                    ownership.name, a.physical_city, a.physical_street,
        ...
        if data['to_date']:
            sql_query = sql_query + """ AND a.acquisition_date < '%s'""" % (
                data['to_date'])
        sql_query = sql_query + """ ORDER BY a.asset_type_id"""

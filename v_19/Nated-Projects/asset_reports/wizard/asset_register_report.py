...
class AssetRegisterReports(models.TransientModel):
    ...
    def get_xlsx_report(self, data, response):
        ...
        sql_query = """SELECT a.identification_number,
                    parent_asset.identification_number, 
                    parent_asset.description, 
                    parent_asset.identification_number, a.description,
                    a.barcode_past_year, a.asset_barcode, at.name, ac.name, 
                    sac.name, a.name, a.state, a.acquisition_date, 
                    a.disposal_date, a.asset_reason, a.impairment_date, 
                    a.write_date, a.condition, a.policy_number, a.method, 
                    a.opening_cost, a.original_value - (COALESCE(a.salvage_value, 0)) - COALESCE(a.already_depreciated_amount_import, 0) - 
                    (SELECT COALESCE(SUM(depreciation_value), 0) FROM account_move as move where move.asset_id = a.id and move.state='posted'),
                    a.re_valued_value, a.fair_value_less_cost_to_sell, 
                    a.accumulated_depreciation, a.disposal, a.transfer, 
                    a.impairment, a.transfer, a.carrying_amount, a.deed_number,
                    a.serial_number, a.custodian, a.physical_city, 
                    a.physical_street, a.building
                    ...
                    LEFT JOIN account_asset as parent_asset ON parent_asset.id = a.asset_parent_id

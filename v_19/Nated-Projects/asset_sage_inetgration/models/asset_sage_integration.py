    ...
    def action_get_general_ledger(self):
        ...
        if self.type == 'account':
            ...
            try:
                ...
                _logger.info('response hiii: %s', response)
                # _logger.info('response dict: %s', response.__dict__)
                # _logger.info('response reason: %s', response.reason)

                res = response.json()
                # _logger.info('response: %s', res['response'])
                # _logger.info('status_code: %s', res['status_code'])
                # _logger.info('response 0: %s', res['response'][0])
                ...
                if response.status_code == 200:
                    ...
                    res = response.json()
                    _logger.info('ress', res)
                    for rec in res['response']:
                        _logger.info('accountTypeID: %s', rec['accountTypeID'])
                        _logger.info('REsponse: %s', rec)
                        account_type = ''
                        if rec['accountTypeID'] == 18:
                            account_type = 'asset_non_current'
                        if account_type:
                            _logger.info('responsess')
                            account_vals = {
                                'code': rec['account'],
                            ...
                            raise UserError(rec['accountTypeID'])
        ...
        if self.type == 'transaction':
            ...
            try:
                ...
                # _logger.info('response text: %s', response.__dict__)
                if response.status_code == 200:
                   res = response.json()
                   for rec in res:
                       _logger.info(rec, 'Resssssss')

...
class AssetVerification(models.Model):
    ...
    # condition = fields.Many2many('asset.condition', string="Condition", help="Condition of the asset",required=True,default = lambda self: self.env['asset.condition'].search([('name','=','Good')]).ids)
    condition = fields.Many2many('asset.condition', string="Condition", help="Condition of the asset",required=True)
    ...
    def default_get(self, fields):
        ...
        if asset_id:
            ...
            if history and len(history) > 0:
                ...
                res['location_id'] = history.history_id.job_location_id.id if history.history_id else False
                res['condition'] = history.condition if history.condition else False
            else:
                res['asset_last_status'] = 'No previous verification history found.'
                res['location_id'] = self.env['account.asset'].search([('id', '=', asset_id)], limit=1).job_location_id.id if self.env['account.asset'].search([('id', '=', asset_id)], limit=1) else False
        return res
    ...
    def action_verify(self):
        ...
        if len(self.condition) > 1:
            raise UserError("You cannot select more than one condition.")

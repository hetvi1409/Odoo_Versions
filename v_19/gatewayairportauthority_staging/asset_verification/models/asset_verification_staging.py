...
class AssetVerificationStaging(models.Model):
    ...
    verify = fields.Selection([
        ('yes', 'Yes'),
        ('no', 'No')
    ], string="Verify", help="Whether to verify the asset or not")
    ...
    # image_ids = fields.One2many('asset.verification.image', 'new_asset_verification', string='Images')
    ...
    def action_approve(self):
        print('\n\n\n action_approve=====>',self)
        print('\n\n\n action_approve==DATE:===>',self.create_date)
        ...
        asset_vals = {
            ...
            'original_value': self.original_value if self.original_value else False,
        ...
        if self.create_date:
            self.env.cr.execute("""
                UPDATE asset_verification_history
                SET create_date = %s
                WHERE id = %s
            """, (self.create_date, asset_history.id))
        ...
        asset.message_post(
            ...
            message_type='email',
            subtype_xmlid='mail.mt_comment',
            # partner_ids=[self.custodian_id.related_contact_ids[0].id],
        )

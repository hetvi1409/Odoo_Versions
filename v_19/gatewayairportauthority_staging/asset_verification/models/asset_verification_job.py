...
class AssetVerificationJob(models.Model):
    ...
    def _get_period_domain(self, job, asset_ids):
        domain = [('history_id', 'in', asset_ids)]
        if job.verification_period_from:
            start_dt = datetime.combine(job.verification_period_from.date(), time.min)
            domain.append(('create_date', '>=', start_dt))
        if job.verification_period_to:
            end_dt = datetime.combine(job.verification_period_to.date(), time.max)
    ...
    def update_assets(self):
        ...
        for job in self:
            ...
            if assets:
                domain = self._get_period_domain(job, assets.ids)
                matched = history_model.search(domain)
                verified_asset_ids = set(matched.mapped('history_id').ids)

            new_lines = [
                (0, 0, {
                    'asset_id': asset.id,
                    'verified': asset.id in verified_asset_ids,
                })
                for asset in assets
            ]
            job.asset_ids = [(5, 0, 0)] + new_lines

            job._compute_total_assets()
            job._compute_verified_assets()
            job._compute_not_verified_assets()
            job._compute_completion_percentage()
    ...
    @api.depends('asset_ids', 'asset_ids.verified', 'verification_period_from', 'verification_period_to')
    def _compute_verified_assets(self):
        history_model = self.env['asset.verification.history']
        for record in self:
            asset_ids = record.asset_ids.mapped('asset_id').ids
            if not asset_ids:
                record.verified_count = 0
                continue

            domain = self._get_period_domain(record, asset_ids)
            matched = history_model.search(domain)
            record.verified_count = len(set(matched.mapped('history_id').ids))

    @api.depends('total_assets', 'verified_count')
    def _compute_not_verified_assets(self):
        for record in self:
            record.not_verified_count = record.total_assets - record.verified_count

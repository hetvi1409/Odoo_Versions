...
class AssetUSeFulReports(models.TransientModel):
    ...
    def get_xlsx_report(self, data, response):
        ...
        sql_query = """SELECT '',  ac.name,
                        a.original_value, a.asset_component, a.asset_barcode,
                        aul.remaining_useful_life, at.name, '',
                        '', a.impairment, at.name,
                        a.asset_barcode, a.id, a.disposal_date, 
                        '', a.acquisition_date, 
                        ac.description, '', '', 
                        '', '', 
                        a.asset_sub_category_id, a.disposal_reason,
                        '', a.current_condition_this_year
                        ...
                        LEFT JOIN (
                            SELECT *, ROW_NUMBER() OVER (PARTITION BY asset_id ORDER BY id DESC) AS rn
                                   FROM
                            asset_useful_life) AS aul ON aul.asset_id = a.id AND aul.rn = 1
                            WHERE
                            a.state != 'model'"""
        if data['from_date']:

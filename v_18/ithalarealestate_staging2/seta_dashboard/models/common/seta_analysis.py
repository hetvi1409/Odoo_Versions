...
class SETAAnalysisDrilldownDimension(models.Model):
    _inherit = 'seta.analysis.drilldown.dimension'
    visual_type_id = fields.Many2one(comodel_name='seta.visual.type', string='Visual Type')
...
class SETAAnalysis(models.Model):
    ...
    analysis_visual_config_ids = fields.One2many(
        comodel_name='seta.analysis.visual.config', inverse_name='analysis_id', string='Analysis Visual Config')
    ...
    metric_field_ids = fields.Many2many(comodel_name='seta.table.field', relation='metric_field_analysis_rel',
                                        column1='analysis_id', column2='field_id', string='Metric Fields', domain=[('field_type', 'in', ('numeric', 'number'))])
    dimension_field_ids = fields.Many2many(comodel_name='seta.table.field', relation='dimension_field_analysis_rel',
    ...
    def _get_default_visual_configs(self):
        ...
        visual_config = self.env['seta.visual.config'].search([('name', '=', 'rotateLabel')], limit=1)
        if visual_config:
        ...
        visual_config = self.env['seta.visual.config'].search([('name', '=', 'legendPosition')], limit=1)
        ...
        visual_config = self.env['seta.visual.config'].search([('name', '=', 'stacked')], limit=1)
        ...
        visual_config = self.env['seta.visual.config'].search([('name', '=', 'innerRadius')], limit=1)
        if visual_config and self.visual_type_id.name in ('pie'):
            default_visual_configs += [(0, 0, {
                'visual_config_id': visual_config.id,
                'string_value': '30',
    ...
    def get_visual_type_table(self):
        ...
        visual_type_table = self.env['seta.visual.type'].search([('name', '=', 'table')], limit=1)
    ...
    def ui_get_analysis_info(self):
        ...
        # Filter Operators
        filter_operators = self.env['seta.analysis.filter.operator'].search([('source_type', '=', self.source_id.type)])
    ...
    def update_visual_config(self, data):
        ...
        for item in data:
            for key, value in item.items():
                visual_config = self.env['seta.visual.config'].search([('name', '=', key)], limit=1)
                if visual_config:
                    default_visual_configs += [(0, 0, {
                        'visual_config_id': visual_config.id,
                        'string_value': value,
    ...
    def _prepare_export_vals(self, block_id=False):
        ...
        if block_id:
            ...
            block = self.env['seta.dashboard.block'].browse(block_id)
            if block:
    ...
    def ui_get_available_fields(self, kwargs):
        fields = self.env['seta.table.field'].search([
    ...
    def ui_execute_query(self, table_id, query):
        ...
        try:
            ...
            Metric = self.env['seta.analysis.metric']
            Dimension = self.env['seta.analysis.dimension']
            # Get Fields
            for field in self.table_id.field_ids:
    ...
    def save_analysis_visual_type(self, visual_type):
        self.ensure_one()
        vt = self.env['seta.visual.type'].search([('name', '=', visual_type)], limit=1)
        if vt:
    ...
    def get_analysis_data_dashboard(self, **kwargs):
        ...
        if kwargs.get('filters') and kwargs.get('filters').get('dynamic'):
            ...
            for dy in kwargs.get('filters').get('dynamic'):
                dyf = self.env['seta.dashboard.filter'].browse(dy['filter_id'])
                if dyf:
                    # All Filters

...
class SETADashboardBlock(models.Model):
    _name = 'seta.dashboard.block'
    _description = 'SETA Dashboard Block'
    ...
    visual_type_id = fields.Many2one(comodel_name='seta.visual.type',
    ...
    dashboard_id = fields.Many2one(comodel_name='seta.dashboard', string='Dashboard', required=True, ondelete='cascade')
    ...
    def copy(self, default=None):
        ...
        res = super(SETADashboardBlock, self).copy(default)
        if self.analysis_id:
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if 'analysis_id' in vals and 'dashboard_id' in vals:
                visual_type_id = self.env['seta.analysis'].browse(vals['analysis_id']).visual_type_id
                ...
                if 'gs_x' not in vals or 'gs_y' not in vals:
                    ...
                    dashboard = self.env['seta.dashboard'].browse(vals.get('dashboard_id'))
                    ...
                    if found:
                        vals['gs_x'] = new_x
                        vals['gs_y'] = new_y
        recs = super(SETADashboardBlock, self).create(vals_list)
        return recs

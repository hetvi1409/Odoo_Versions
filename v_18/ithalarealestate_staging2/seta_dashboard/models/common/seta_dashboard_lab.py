...
file_code = '''
table = seta_table.search([('table_name','=','%s')])
...
seta_table.get_table_fields_from_dictionary(sample)
seta_table.update_schema_store_table()
for data in res:
    seta.query_insert('%s',data)
'''
class SETADashboard(models.Model):
    _inherit = 'seta.dashboard'

    seta_lab_api_key = fields.Char('SETA Lab API Key', compute='_compute_seta_lab_api_key')
    seta_lab_url = fields.Char('SETA Lab URL', compute='_compute_seta_lab_api_key')
    base_url = fields.Char('Base URL', compute='_compute_seta_lab_api_key')
    seta_dashboard_access_token = fields.Char('SETA Dashboard Access Token', compute='_compute_seta_lab_api_key')

    def _compute_seta_lab_api_key(self):
        for rec in self:
            rec.seta_lab_api_key = self.env.user.company_id.seta_lab_api_key
            rec.seta_lab_url = self.env['ir.config_parameter'].sudo().get_param('seta_lab_url')
            rec.base_url = self.env['ir.config_parameter'].sudo().get_param('web.base.url')
            rec.seta_dashboard_access_token = self.env['ir.config_parameter'].sudo().get_param('seta_dashboard.access_token')
    ...
    def action_get_lab_analysis_config(self, analysis_id, analysis_name):
        ...
        if not seta_lab_url:
            raise UserError(_('Please set SETA Lab URL in System Parameters.'))
        res = requests.post('''%s/lab/analysis/%s/config''' % (seta_lab_url, analysis_id), json={
            'name': analysis_name,
            'seta_lab_api_key': self.env.company.seta_lab_api_key,
        })
        res = res.json()
        if res.get('result') and res.get('result').get('config'):
            data = res.get('result').get('config')
            # Call seta.dashboard.config.wizard to create dashboard
            if type(data) == dict:
                if data['method'] == 'table':
                    seta_table = self.create_mart_table(data)
                    # seta_table.method_direct_trigger()
                    res = {
                    ...
                    return res
            res = self.env['seta.dashboard.config.wizard'].create({
    ...
    def create_mart_table(self,data):
        seta_table_obj = self.env['seta.table']
        seta_table = seta_table_obj.create({
        ...
        store_table_name = seta_table.store_table_name

        data['query'] = f'SELECT * FROM {store_table_name} LIMIT 100;'
        data['table_id'] = seta_table.id
    ...
    def action_check_key(self):
        ...
        if not seta_lab_url:
            raise UserError(_('Please set SETA Lab URL in System Parameters.'))
        res = requests.post('''%s/lab/check''' % (seta_lab_url), json={
            'seta_lab_api_key': self.env.company.seta_lab_api_key,
        })
        res = res.json()
        res = res.get('result')
    ...
    def action_add_to_dashboard(self):
        ...
        analysis = self.env['seta.analysis'].search([('name', '=', 'Analysis From AI')])
        if not analysis:
            analysis = self.env['seta.analysis'].search([('name', '=', 'Analysis From AI Table')])

        if analysis:
            new_analysis = analysis.with_context(action_copy=True, action_copy_from_conversation=True).copy()
            self.env['seta.dashboard.block'].create({
    ...
    def action_execute_code(self, code, code_type):
        ...
        try:
            ...
            analysis = self.env['seta.analysis'].search([('name', '=', 'Analysis From AI Table')])
            analysis.unlink()
            table = self.env['seta.table'].search([('name', '=', 'Table From AI')])
            table.unlink()
            source = self.env['seta.data.source'].search([], limit=1)
            ...
            if code and code_type == 'sql':
                ...
                query = query.replace(';', '')
                if 'LIMIT' in query:
                    query_result = self.env['seta.tools'].sudo().query_fetch(query + ' ;')
                    if query_result:
                        query_value = query_result[0]
                else:
                    query_result = self.env['seta.tools'].sudo().query_fetch(query + ' LIMIT 1 ;')
                    if query_result:
                        query_value = query_result[0]
                ...
                # Create Analysis
                analysis = self.env['seta.analysis'].create({
                ...
                for key in query_value:
                    ...
                    if type(value) == bool and not isinstance(value, int):
                        type_origin = 'boolean'
                        field = self.env['seta.table.field'].search([('field_name', '=', key), ('table_id', '=', table.id)], limit=1)
                        if field:
                ...
                if len(metric_fields) == 2 and len(date_fields) == 0:
                    ...
                    analysis.limit = 50
                visual_type = self.env['seta.visual.type'].search([('name', '=', visual_type_name)], limit=1)
                ...
                for field_id in date_field_ids:
                    dimension = self.env['seta.analysis.dimension'].create({
                        'field_id': field_id,
                ...
                for field_id in metric_field_ids:
                    metric = self.env['seta.analysis.metric'].create({
                        'field_id': field_id,
                        'calculation': 'sum',
                        'analysis_id': analysis.id,
                    })
                for field_id in date_field_ids:
                    sort = self.env['seta.analysis.sort'].create({
                        'field_id': field_id,
                        'sort': 'asc',
                        'analysis_id': analysis.id,
                    })
            ...
            if analysis_config:
                if analysis_config.get('query'):
                    ...
                    if not table_name:
                        ...
                        seta_table_obj = self.env['seta.table']
                        search_code = f"table = seta_table.search([('table_name','=','{table_name}')])"
                        seta_table = seta_table_obj.search([('name','ilike','Mart Table From AI'),('main_code','ilike',search_code)])
                        if not seta_table:
                            seta_table = seta_table_obj.search([('name','ilike','Mart Table From AI')])
                            if seta_table:
                                count = len(seta_table)
                        ...
                        code = file_code % (str(table_name), seta_table.store_table_name)
                        seta_table.write({
                            'main_code': code
                        })
                        seta_table.method_direct_trigger()
                        ...
                        analysis = self.env['seta.analysis'].create({
                            ...
                            'method': analysis_method,
                            'table_id':  seta_table.id,
                            'model_id': seta_table.model_id.id if seta_table.model_id else False,
                        })
    ...
    def action_get_lab_ask(self, messages, retry_count=0):
        ...
        if not seta_lab_url:
            ...
            return  {
                'status': 500,
                'message': _('Please Set SETA Lab URL in System Parameters.'),
            }
        if retry_count > 3:
        ...
        if messages:
            ...
            try:
                res = requests.post('''%s/lab/analysis/ask''' % (seta_lab_url), json={
                    'seta_lab_api_key': self.env.company.seta_lab_api_key,
                    'messages': messages,
                ...
                if res.get('result') and res.get('result').get('status') == 200 and res.get('result').get('new_message_content'):
                    ...
                    if analysis_config:
                        ...
                        if (analysis_res and analysis_res['status'] == 200) :
                            ...
                            if analysis_data:
                                res_after_data = requests.post('''%s/lab/analysis/ask/explain''' % (seta_lab_url), json={
                                    'seta_lab_api_key': self.env.company.seta_lab_api_key,
    ...
    def generate_access_token(self, context, expiration_hour=False):
        if expiration_hour:
            ...
            token_expired_hours = self.env['ir.config_parameter'].sudo().get_param('seta_dashboard.token_expired_hours')
            if not token_expired_hours:
                token_expired_hours = 1
        token = self.env['seta.dashboard.token'].sudo().create({
...
class SETADashboardToken(models.Model):
    _name = 'seta.dashboard.token'
    _description = 'SETA Dashboard Token'

    name = fields.Char('Name')
    token = fields.Char('Token')
    dashboard_id = fields.Many2one('seta.dashboard', 'Dashboard')

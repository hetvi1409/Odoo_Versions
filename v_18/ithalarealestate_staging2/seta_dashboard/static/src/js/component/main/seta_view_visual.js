...
import amChartsComponent from "@seta_dashboard/js/component/chart/amcharts_component"
...
var SETAViewVisual = Widget.extend({
    template: 'SETAViewVisual',
    events: {
        'click .seta_reset_drilldown': '_onClickResetDrilldown',
        'click .seta_drillup': '_onClickDrillup',
    },
    ...
    _onClickResetDrilldown: function (ev) {
        ...
        self.$el.find('.seta_reset_drilldown').hide();
        self.$el.find('.seta_drillup').hide();
    },
    ...
    _onHitChart: function (ev, visual, val) {
        ...
        if (ev.target && ev.target.htmlContainer) {
            if (is_slide) {
                ...
                analysis_id = parseInt($(ev.target.htmlContainer).closest('.seta_view_visual').data('analysis_id'));
            }
            if (analysis_id && visual) {
                ...
                if (self && self.dimension_alias && val && !self.drilldown_wait) {
                    ...
                    rpc('/web/dataset/call_kw/seta.analysis/ui_get_available_fields', {
                        model: 'seta.analysis',
                        ...
                        res.forEach(item => {
                            ...
                            menu_item.addEventListener('click', () => {
                                ...
                                self._getDataAnalysis(args, function (result) {
                                    ...
                                    if (is_slide) {
                                        self._makeChartSlide(result);
                                        self.$el.parent().find('.seta_reset_drilldown').show();
                                        self.$el.parent().find('.seta_drillup').show();
                                    } else {
                                        self._makeChart(result);
                                        self.$el.find('.seta_reset_drilldown').show();
                                        self.$el.find('.seta_drillup').show();
                    ...
                    if (self.action_model && self.drilldown_level > self.max_drilldown_level) {
                        rpc('/web/dataset/call_kw/seta.analysis/ui_get_view_parameters', {
                            model: 'seta.analysis',
    ...
    _convertDomainToUTC(domain, callback) {
        var self = this;
        if (self.analysis_id) {
            rpc('/web/dataset/call_kw/seta.analysis/convert_domain_to_utc', {
                model: 'seta.analysis',
    ...
    _getLabAnalysisText: function (ai_analysis_data) {
        ...
        if (self.analysis_id) {
            ...
            rpc('/web/dataset/call_kw/seta.analysis/action_get_lab_description', {
                model: 'seta.analysis',
    ...
    runDataScript: function (context = {}, callback = undefined) {
        var self = this;
        if (self.analysis_id) {
            rpc('/web/dataset/call_kw/seta.analysis/run_data_script', {
                model: 'seta.analysis',

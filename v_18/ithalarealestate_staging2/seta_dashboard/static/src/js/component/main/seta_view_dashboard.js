...
import SETAViewDashboardBlock from "@seta_dashboard/js/component/main/seta_view_dashboard_block";
...
var SETAViewDashboard = Widget.extend({
    template: 'SETAViewDashboard',
    events: {
        'click input': '_onClickInput',
        'click button': '_onClickButton',
        'click .seta_view_dashboard_ask_bg': '_onClickBgDashboardAsk',
        'click .seta_view_analysis_dialog_bg': '_onClickBgDashboardAnalysisDialog',
        'click .seta_view_dashboard_ask_btn': '_onClickSubmitAsk',
        'keydown .seta_view_dashboard_ask_input': '_onKeydownInputAsk',
        'click .code_execution': '_onClickExecuteCode',
        'click .seta_view_dashboard_ask_result_configuration': '_onClickAskResultConfiguration',
        'click .seta_view_dashboard_ask_result_add_to_dashboard': '_onClickAskResultAddToDashboard',
        ...
        'click .seta_view_dashboard_ask_header_table': '_onClickHeaderTable',
        'click': '_globalClick',
        'dragenter': '_onDragEnterFileUploader',
        'dragover #seta_dashboard_file_uploader': '_onDragOverFileUploader',
        'dragleave #seta_dashboard_file_uploader': '_onDragLeaveFileUploader',
        'drop #seta_dashboard_file_uploader': '_onDropFileUploader',
    },
    ...
    start: function () {
        ...
        // File Uploader
        self.$viewDashboardFileUploader = self.$('#seta_dashboard_file_uploader');

        // Dashboard Ask
        self.$viewDashboardAsk = self.$('.seta_view_dashboard_ask');
        self.$viewDashboardAskContainer = self.$('.seta_view_dashboard_ask_container');
        self.$viewDashboardAskChart = self.$('.seta_view_dashboard_ask_result_chart');
        self.$viewDashboardAskQuickMessages = self.$('.seta_view_dashboard_ask_quick_messages');
    ...
    _updateDashboardTable: function (table_id) {
        var self = this
        rpc('/web/dataset/call_kw/seta.dashboard/update_dashboard_table', {
        ...
        new swal({
            ...
            if (result.isConfirmed) {
                if (self.selectedDashboard) {
                    rpc('/web/dataset/call_kw/seta.dashboard/action_add_to_dashboard', {
                        model: 'seta.dashboard',
    ...
    _openAnalysisDialogById: function (analysisId) {
        var self = this;
        self.$viewDashboardAnalysisDialog.closest('.seta_dialog').show();
        var args = {
            'id': -1,
        ...
        var $blockDialog = new SETAViewDashboardBlock(self, args);
    ...
    _generateAskChart: function (analysisId, analysisConfig) {
        ...
        if (analysisId) {
            ...
            // }
            var $visual = new SETAViewVisual(self, args);
    ...
    _onClickExecuteCode: function (ev) {
        ...
        if ($(ev.currentTarget) && $(ev.currentTarget).closest('.code_content_sql')) {
            ...
            if (query && self.selectedDashboard) {
                rpc('/web/dataset/call_kw/seta.dashboard/action_execute_code', {
                    model: 'seta.dashboard',
    ...
    _onClickQuickMessage: function (ev) {
        ...
        if (message) {
            self.$('.seta_view_dashboard_ask_input').val(message);
            self.quick_messages = [];
            self._renderQuickMessages();
    ...
    _renderAIMessages: function () {
        ...
        self.ai_messages.forEach(msg => {
            ...
            if (role == 'assistant' || role == 'user') {
                ...
                var style = (role == 'assistant') ? `background: url('/seta_dashboard/static/description/icon_avatar.png');background-size: contain;` : 'background: #875A7B';
    ...
    _onClickSubmitAsk: function (ev) {
        var self = this;
        var message_content = self.$('.seta_view_dashboard_ask_input').val();
        if (message_content) {
            ...
            self._renderAIMessages();
            self.$('.seta_view_dashboard_ask_input').val('');
        ...
        if (self.selectedDashboard && self.raw_messages) {
            ...
            rpc('/web/dataset/call_kw/seta.dashboard/action_get_lab_ask', {
                model: 'seta.dashboard',
                ...
                if (result.status == 200) {
                    if (result.new_messages) {
                        ...
                        if (result.analysis_id) {
                            ...
                            self.$viewDashboardAskContainer.find('.seta_view_dashboard_ask_result_title').show();
                        }
                        self._renderAIMessages();
                    }
    ...
    _onKeydownInputAsk: function (ev) {
        var self = this;
        if (self.$('.seta_view_dashboard_ask_input').is(':focus') && (ev.keyCode == 13) && (!ev.shiftKey)) {
            ev.preventDefault();
            self._onClickSubmitAsk();
        }
    },
    ...
    _onClickBgDashboardAsk: function (ev) {
        var self = this;
        self.$viewDashboardAsk.closest('.seta_dialog').hide();
    },

    _onClickBgDashboardAnalysisDialog: function (ev) {
        var self = this;
        self.$viewDashboardAnalysisDialog.closest('.seta_dialog').hide();
    },
    ...
    _onDragEnterFileUploader: function (ev) {
        var self = this;
        if (self.$el.hasClass('seta_edit_mode')) return;
        ev.stopPropagation();
        ev.preventDefault();
    ...
    _onDropFileUploader: function (ev) {
        ...
        fileReader.onload = function () {
            ...
            if (filetype == 'application/json') {
                rpc('/web/dataset/call_kw/seta.dashboard.config.wizard/process_wizard_model', {
                    model: 'seta.dashboard.config.wizard',
                ...
                rpc('/web/dataset/call_kw/seta.data.source.item/process_data_file', {
                    model: 'seta.data.source.item',
    ...
    _loadDashboard: function (filters, mode = false) {
        ...
        self._clear();
        if (self.selectedDashboard) {
            rpc('/web/dataset/call_kw/seta.dashboard.block/search_read', {
                ...
                self.dashboardBlocks.forEach(block => {
                    ...
                    if (block.analysis_id) {
                        ...
                        index += 1;
                        var $block = new SETAViewDashboardBlock(self, args);
...
export default SETAViewDashboard;
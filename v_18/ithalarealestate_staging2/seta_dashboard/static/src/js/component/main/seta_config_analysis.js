...
import SETASelectAnalysis from "@seta_dashboard/js/component/main/seta_select_analysis";
import SETASelectDimension from "@seta_dashboard/js/component/main/seta_select_dimension";
import SETASelectSort from "@seta_dashboard/js/component/main/seta_select_sort";
import SETASelectFilter from "@seta_dashboard/js/component/main/seta_select_filter";
import SETASelectMetric from "@seta_dashboard/js/component/main/seta_select_metric";
...
var SETAConfigAnalysis = Widget.extend({
    ...
    events: {
        ...
        'click .seta_select_analysis_explore': '_onClickSelectAnalysisExplore',
        'click .seta_select_analysis_edit': '_onClickSelectAnalysisEdit',
        'click .seta_select_analysis_insight': '_onClickSelectAnalysisInsight',
        'click .seta_exit_insight': '_onClickExitInsight',

        'click .seta_action_open_visual_script_editor': '_onClickVisualScriptEditor',
        'click .seta_action_open_data_script_editor': '_onClickDataScriptEditor',

        'click .seta_select_lang': '_onClickLangInsight',
    },
    ...
    start: function () {
        ...
        self.$tabContentInsight = self.$('.seta_tab_content_insight');
    ...
    _loadLanguange: function () {
        var self = this;
        rpc('/web/dataset/call_kw/seta.analysis/ui_get_languanges', {
            model: 'seta.analysis',
            method: 'ui_get_languanges',
            args: [[]],
            kwargs: {},
        }).then(function (result) {
            var languangeElement = self.$('.seta_languange')
            languangeElement.find('.dropdown-menu').empty();
            result.forEach(lang => {
                languangeElement.find('.dropdown-menu').append(`<a class="dropdown-item seta_select_lang">${lang}</a>`);
            })
        })
    },
    ...
    _onClickLangInsight: function (ev) {
        ...
        $("#seta_selected_languange").val(selectedLang)
    ...
    _onClickSelectAnalysisInsight: function (ev) {
        ...
        self.$('.seta_select_analysis_container').hide();
        self.$('.seta_config_analysis_tab').hide();
        self.$('.seta_blank_divider').hide();
    ...
    _generateInsights: function (ev) {
        var self = this;
        self.$('.seta_tab_content_insight_container').empty();
        self.insightContents = [];
        self._getInsight();
    },
    ...
    _getInsight: function () {
        ...
        if (self.$viewAnalysis.$visual && self.$viewAnalysis.$visual.temp_analysis_data && self.selectedAnalysis) {
            ...
            rpc('/web/dataset/call_kw/seta.analysis/action_get_lab_insight', {
                model: 'seta.analysis',
                ...
                if (res_json.status == 200) {
                    if (res_json.parent) {
                        ...
                        self.$('.seta_tab_content_insight_container').append(`
                            <div class="seta_insight_item seta_insight_parent seta_divider card-body seta_white seta_p10">
                                <div class="seta_insight_item_content">
                                    ${parent_content}
                                    <div class="seta_insight_item_tag">Parent Insight</div>
                                </div>
                                <div class="seta_insight_item_icon">
                    ...
                    insights.forEach(insight => {
                        ...
                        if (insight.records && insight.records.length == 1) {
                            ...
                            record_tag = `<div class="seta_insight_item_tag">${record_tag}</div>`
                        }
                        ...
                        self.$('.seta_tab_content_insight_container').append(`
                            <div class="seta_insight_item seta_divider card-body seta_white seta_p10">
                                <div class="seta_insight_item_content">
                                    ${content}
                                    ${record_tag}
                                </div>
    ...
    _onClickExitInsight: function (ev) {
        ...
        self.$('.seta_select_analysis_container').show();
        self.$('.seta_config_analysis_tab').show();
        self.$('.seta_blank_divider').show();
    },
        ...
        if (self.selectedAnalysis) {
            rpc('/web/dataset/call_kw/seta.analysis/ui_get_analysis_info', {
                ...
                self.metrics.forEach(metric => {
                    var $content = $(renderToElement('SETACurrentMetricItem', {
                        name: metric.name,
                        id: metric.id,
                ...
                self.dimensions.forEach(dimension => {
                    var $content = $(renderToElement('SETACurrentDimensionItem', {
                        name: dimension.name,
                        id: dimension.id,
                ...
                self.sorts.forEach(sort => {
                    var $content = $(renderToElement('SETACurrentSortItem', {
                        name: sort.name,
                        id: sort.id,
                ...
                self.filters.forEach(filter => {
                    var $content = $(renderToElement('SETACurrentFilterItem', {
                        name: filter.name,
                        id: filter.id,
    ...
    _renderVisualTypes: function () {
        var self = this;
        rpc('/web/dataset/call_kw/seta.visual.type/search_read', {
    ...
    _renderVisualConfigs: function () {
        var self = this;
        rpc('/web/dataset/call_kw/seta.visual.type/get_visual_config', {
            model: 'seta.visual.type',
                        ...
                        self.$selectVisualConfigContainer.append(`
                            <div class="flex-body mb-3">
                                ...
                                <div class="input-group flex-1">
                                    <input data-jscolor="{}" type="${input_type}" class="form-control seta_visual_config" id="${vc.name}${vc.id}" placeholder="" data-visual-config="${vc.name}" data-visual-config-id="${vc.id}" data-visual-config-type="${vc.config_type}" data-analysis-visual-config-id="${vc.analysis_visual_config_id}"></input>
                                </div>
                            </div>
                        `);
                    } else {
    ...
    _onClickRemoveMetric: function (ev) {
        ...
        if (self.selectedAnalysis) {
            rpc('/web/dataset/call_kw/seta.analysis/ui_remove_metric', {
                model: 'seta.analysis',
    ...
    _onClickRemoveDimension: function (ev) {
        ...
        if (self.selectedAnalysis) {
            rpc('/web/dataset/call_kw/seta.analysis/ui_remove_dimension', {
                model: 'seta.analysis',
    ...
    _onClickRemoveSort: function (ev) {
        ...
        if (self.selectedAnalysis) {
            rpc('/web/dataset/call_kw/seta.analysis/ui_remove_sort', {
                model: 'seta.analysis',
    ...
    _onClickRemoveFilter: function (ev) {
        ...
        if (self.selectedAnalysis) {
            rpc('/web/dataset/call_kw/seta.analysis/ui_remove_filter', {
                model: 'seta.analysis',
    ...
    _onUpdateCurrentFilter: function (ev) {
        ...
        if (self.selectedAnalysis) {
            ...
            rpc('/web/dataset/call_kw/seta.analysis/ui_update_filter_by_field', {
                model: 'seta.analysis',
    ...
    _onClickSelectCalculation: function (ev) {
        ...
        if (calculation && metric_id) {
            ...
            rpc('/web/dataset/call_kw/seta.analysis.metric/write', {
    ...
    _onClickSelectFormat: function (ev) {
        ...
        if (format && dimension_id) {
            ...
            rpc('/web/dataset/call_kw/seta.analysis.dimension/write', {
    ...
    _onClickSelectSortDirection: function (ev) {
        ...
        if (sort && sort_id) {
            ...
            rpc('/web/dataset/call_kw/seta.analysis.sort/write', {
    ...
    _onChangeVisualType: function (ev) {
        ...
        if (self.selectedAnalysis) {
            ...
            rpc('/web/dataset/call_kw/seta.analysis/save_analysis_visual_type', {
                model: 'seta.analysis',
    ...
    _onClickSaveAnalysisVisual: function () {
        ...
        if (self.selectedAnalysis) {
            ...
            rpc('/web/dataset/call_kw/seta.analysis/save_analysis_visual_config', {
                model: 'seta.analysis',
    ...
    _onClickAddDashboardBlock: function () {
        ...
        if (self.selectedAnalysis && self.selectedDashboard) {
            rpc('/web/dataset/call_kw/seta.dashboard.block/create', {
    ...
    _onChangeLimit: function () {
        ...
        if (self.selectedAnalysis) {
            ...
            rpc('/web/dataset/call_kw/seta.analysis/write', {
                model: 'seta.analysis',
                method: 'write',
                args: [self.selectedAnalysis, data],
                kwargs: {},
    ...
    _checkActionContext: function () {
        ...
        if (self.props) {
            if (self.props && self.props.context && self.props.context.analysis_id) {
                ...
                rpc('/web/dataset/call_kw/seta.analysis/search_read', {
                    model: 'seta.analysis',
                    method: 'search_read',
    ...
    _startAnalysisExplore: function (kwargs) {
        ...
        new SETAAutocomplete(self, {
            'elm': self.$viewAnalysis.$('#seta_select2_dashboard_explore'),
            'multiple': false,
            'placeholder': 'Select Dashboard',
            'minimumInput': false,
            'params': {
        ...
        // 
        self.$viewAnalysis.$viewAnalysisExplore.closest('.seta_dialog').show();
        self.$viewAnalysis.$viewAnalysisExplore.empty();
        self.$viewAnalysis.selectedAnalysisExplores = [];
        rpc('/web/dataset/call_kw/seta.analysis/start_lab_analysis_explore', {
            model: 'seta.analysis',
            ...
            if (result && result.analysis_explores) {
                result.analysis_explores.forEach(function (analysis) {
                    var $exploreVisual = new SETAViewVisual(self, {
                        'analysis_id': analysis.id,
                    });
                    var $exploreVisualContainer = $(`<div class="seta_view_analysis_explore_content"></div>`);
                    $(`<div class="seta_view_analysis_explore_container" data-analysis-id="${analysis.id}">
                        <div class="seta_view_analysis_explore_title">${analysis.name}</div>
    ...
    _initIconPicker: function () {
        ...
        $('input[data-visual-config="scorecardIcon"]').each(function () {
            var input = this;
            $('.seta_dialog_bg.material-icon-picker-bg').remove();
            ...
            var $picker = $('<div class="seta_dialog_bg material-icon-picker-bg"></div><div class="material-icon-picker" tabindex="-1"></div>');
            ...
            function onIconClick() {
                ...
                $('.seta_dialog_bg').fadeOut(200, function () {
                    // $(this).remove();
                });
            }
    ...
    _initHideIconPicker: function () {
        ...
        $('body').on('click', '.seta_dialog_bg', function (e) {
            $('.material-icon-picker').fadeOut(200, function () { $(this).remove(); });
            $('.seta_dialog_bg').fadeOut(200, function () { $(this).remove(); });
        });
    },
    ...
    _onClickDataScriptEditor: function (ev) {
        ...
        if (self.selectedAnalysis) {
            ...
            rpc('/web/dataset/call_kw/seta.analysis/get_data_script', {
                model: 'seta.analysis',
                ...
                if (result && result.length == 2 && result[1]) {
                    ...
                    self.$viewAnalysis.$editorContainer.find('.seta_update_script').attr('data-analysis-id', self.selectedAnalysis);
                    self.$viewAnalysis.$editorContainer.find('.seta_update_script').attr('data-analysis-name', self.selectedAnalysisName);
                    self.$viewAnalysis.$editorContainer.find('.seta_update_script').attr('data-block-id', 0);
                    self.$viewAnalysis.$editorContainer.find('.seta_update_script').attr('data-script-type', 'data');
...
`;
                    ...
                    self.$viewAnalysis.$editorContainer.find('.seta_update_script').attr('data-analysis-name', self.selectedAnalysisName);
                    self.$viewAnalysis.$editorContainer.find('.seta_update_script').attr('data-block-id', 0);
                    self.$viewAnalysis.$editorContainer.find('.seta_update_script').attr('data-script-type', 'visual');
                    self.$viewAnalysis.$editorContainer.show();
                }
            });
        }
...
export default SETAConfigAnalysis;
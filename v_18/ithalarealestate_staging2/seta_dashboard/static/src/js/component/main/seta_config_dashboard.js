...
import SETASelectDashboard from "@seta_dashboard/js/component/main/seta_select_dashboard";
import SETAAddAnalysis from "@seta_dashboard/js/component/main/seta_add_analysis";
import SETAAutocomplete from "@seta_dashboard/js/component/general/seta_autocomplete";
import amChartsTheme from "@seta_dashboard/js/component/chart/amcharts_theme";
...
var SETAConfigDashboard = Widget.extend({
    ...
    events: {
        ...
        'click .seta_embed_dashboard': '_onClickEmbedDashboard',
        'click .seta_share_dashboard': '_onClickShareDashboard',
        ...
        'click .seta_add_configuration': '_onClickAddConfig',
        'click .seta_import_data': '_onClickImportData',
        'click .seta_export_configuration': '_onClickExportConfiguration',
        'click .seta_add_filter': '_onClickAddFilter',
        // 'keyup #seta_dashboard_ai_search_input': '_onKeyUpAISearchInput',
        ...
        'click .seta_ai_generate': '_onClickAIGenerate',
        'click .seta_ai_explain': '_onClickAIExplain',
        'click .seta_ai_ask': '_onClickAIAsk',
        'click .seta_ai_slide': '_onClickAISlide',

        'click #seta_datefilter_button': '_onClickDateFilterButton',
        // filter sidebar
        'click .seta_filter_button': '_onClickFilterButton',
    },
    ...
    start: function () {
        ...
        // Dynamic Filter
        self.$dynamicFiltersContainer = self.$('#seta_dynamic_filter_container');
        self.$dynamicFilters = {};

        // Dashboard Search
        self.$dashboardSearchContainer = self.$('#seta_dashboard_search_container .seta_dashboard_filter_content');
    ...
    _checkWidth: function () {
        ...
        if (deviceWidth <= 792 || self.selectedDashboardUseSidebar) {
            $(".seta_filter_button").removeClass('d-none');
            $(".seta_config_dashboard_button_container").addClass('d-none');
                    ...
                    <div class='filter_sidebar_background filter_hide d-none'>
                        <div class='filter_sidebar_container'>
                            ...
                            <div class='filter_sidebar_footer'>
                                <div class="seta_btn seta_btn_blue close_sidebar_filter"><span class="material-icons-outlined seta_btn_icon_left">filter_alt</span> Accept Filter </div>
                            </div>
                        </div>
                    </div>
                `;
                $('#seta_sidebar').append(filter_sidebar);

                // Event listener untuk menutup sidebar
                $('#seta_sidebar').on('click', '.filter_sidebar_background', function (event) {
                    if ($(event.target).is('.filter_sidebar_background')) {
                        self._hideSidebar();
                    }
                });

                $('#seta_sidebar').on('click', '.close_sidebar_filter', function () {
                    self._hideSidebar();
                });
            }

            // Pindahkan elemen ke sidebar
            $('.seta_dashboard_btn_container').appendTo('.filter_sidebar_header');
            $('.seta_dashboard_filter_container').appendTo('.filter_sidebar_main');

            // versi desktop
            if(deviceWidth > 792){
                $('.filler_div').remove()
                $('.seta_filter_button').before('<div class="col-lg-8 filler_div"></div>');
                $('.filter_sidebar_container').addClass('desktop_sidebar')
            }
            self._updateFilterInfo();
        } else {
            $('.seta_config_dashboard .filler_div').remove()
            $(".seta_filter_button").addClass('d-none');
            $(".seta_config_dashboard_button_container").removeClass('d-none');
            self._hideSidebar(); // Sembunyikan sidebar pada desktop

            var innerConfigContainer = $(`#seta_inner_config_dashboard`);
            $('.seta_dashboard_filter_container').appendTo(innerConfigContainer);
            $('.seta_dashboard_btn_container').appendTo(innerConfigContainer);
    ...
    _updateFilterInfo: function (ev) {
        ...
        if (info_count > 0) {
            ...
            $('.seta_filter_button').append(info_el);
            $('.seta_filter_button span.material-icons-outlined').removeClass('seta_float_icon');
        }
    },
    _loadThemes: function (ev) {
        var self = this;
        self.$themeContainer.empty();
        rpc('/web/dataset/call_kw/seta.dashboard.theme/search_read', {
    ...
    _initDashboard: function (ev) {
        ...
        rpc('/web/dataset/call_kw/seta.dashboard/search_read', {
            ...
            method: 'search_read',
            args: [domain, ['id', 'name', 'write_date', 'theme_name', 'date_format', 'start_date', 'end_date', 'seta_lab_url', 'seta_dashboard_access_token', 'base_url', 'table_name', 'use_sidebar']],
            kwargs: {},
        }).then(function (results) {
            if (results.length > 0) {
                ...
                self.setaLabURL = results[0].seta_lab_url;
                if (self.$viewDashboard && self.$viewDashboard.$viewDashboardAskContainer) {
                    var selectElm = self.$viewDashboard.$viewDashboardAskContainer.find('.seta_view_dashboard_ask_header_table .select2-choice')
                    selectElm.removeClass('select2-default');
                    selectElm.find('.select2-chosen').text(results[0].table_name || 'Undefined');
                }
                self.setaDashboardAcessToken = results[0].seta_dashboard_access_token;
                self.baseUrl = results[0].base_url;
                self._initDashboardAISearch();
                if (!self.setaLabURL) {
                    new swal('Warning', 'Please set SETA Lab URL in System Parameters', 'warning');
                }
            } else {
                ...
                self.$selectDashboard.find('.seta_subtitle').text('Click to select or create a dashboard');
            }
        })
        rpc('/web/dataset/call_kw/seta.dashboard/get_user_groups', {
            model: 'seta.dashboard',
            ...
            if (user_group_dashboard == 'User' && user_group_analysis != '') {
                $('.seta_add_analysis').hide();
                $('.seta_edit_layout').hide();
                $('.seta_delete_dashboard').hide();
            } else {
                $('.seta_add_analysis').show();
                $('.seta_edit_layout').show();
                $('.seta_delete_dashboard').show();
            }
        })
    ...
    _initDashboardFilter: function () {
        ...
        // Date Range
        let $dateRangeGroup = self.filterDateRange.elm.find('.seta_dashboard_filter_content');
        ...
        $inputDateRange.on('change', function (ev) {
            let $fromInput = $dateRangeGroup.find('#seta_date_from'); // Input "Date From"
            let $toInput = $dateRangeGroup.find('#seta_date_to'); // Input "Date To"
            let subtitle;
            ...
            $('.seta_config_dashboard').find('.seta_subtitle').text(subtitle);
        });
        ...
        // }
        // self.pickerFrom = new tempusDominus.TempusDominus(document.getElementById('seta_date_from'), options);
        // self.pickerTo = new tempusDominus.TempusDominus(document.getElementById('seta_date_to'), options);
        ...
        // document.getElementById('seta_date_from').addEventListener('show.td', function(event) {
        //     if(self.pickerDate.from){
        //         self.pickerFrom.dates.setValue(self.pickerDate.from)
        //     }
        // })
        // document.getElementById('seta_date_to').addEventListener('show.td', function(event) {
        //     if(self.pickerDate.to){
        //         self.pickerTo.dates.setValue(self.pickerDate.to)
        //     }
        // })
        // document.getElementById('seta_date_from').addEventListener('change.td', function(event) {
        ...
        // document.getElementById('seta_date_to').addEventListener('change.td', function(event) {
        //     if(event.detail.oldDate === null){
        //         let today = new Date();
        ...
        //     const onPickerFrom = e.target.closest('#seta_date_from')
        //     const onPickerTo = e.target.closest('#seta_date_to')
        //     if (onPickerFrom) {
        //         self.pickerTo.hide()
        //     }else if(onPickerTo){
        ...
        //                 self.filterDateRange.values = [$("#seta_date_from").val(),$("#seta_date_to").val()]
        //                 self._loadFilteredDashboard();
        //             }
        //             ///////////////////////
        //         }
        ...
        // $("#seta_date_from").on('change', function(ev) {
        //     if(self.pickerFrom.dates._dates.length>0 && self.pickerTo.dates._dates.length>0){
        //         self.filterDateRange.values = [$("#seta_date_from").val(),$("#seta_date_to").val()]
        //         self._loadFilteredDashboard();            
        //     }
        // })
        // $("#seta_date_to").on('change', function(ev) {
        //     if(self.pickerFrom.dates._dates.length>0 && self.pickerTo.dates._dates.length>0){
        //         self.filterDateRange.values = [$("#seta_date_from").val(),$("#seta_date_to").val()]
        //         self._loadFilteredDashboard();
        //     }
        ...
        // var $dateFromElm = self.filterDateRange.elm.find('#seta_date_from');
        // $dateFromElm.bootstrap_datepicker({
        //     language: "en",
        //     format: "yyyy-mm-dd",
        //     autoclose: true,
        // });
        // var $dateToElm = self.filterDateRange.elm.find('#seta_date_to');
        ...
        rpc('/web/dataset/call_kw/seta.dashboard.filter/fetch_by_dashboard', {
            model: 'seta.dashboard.filter',
            ...
            results.forEach(function (filter) {
                ...
                if (results[0] == filter) {
                    self.$dynamicFiltersContainer.append(`
                        <div class="seta_dashboard_filter${disable_filter}">
                            <div class="seta_dashboard_filter_title dropdown seta_dropdown">
                                <span class="material-icons-outlined">filter_alt</span>
                            </div>
                            <div class="seta_dashboard_filter_content">
                                <span class="seta_select2_label">${filter.name}: </span>
                                <input type="hidden" class="seta_wfull seta_select2" id="filter_${filter.id}"/>
                            </div>
                        </div>`);
                } else {
                    self.$dynamicFiltersContainer.append(`
                ...
                if (filter.params) {
                    var $dF = new SETAAutocomplete(self, {
                        ...
                        onChange: function (values, name) {
                            ...
                            if(children_filter.length > 0){
                                children_filter.forEach(child_id => {
                                    ...
                                    if(values){
                                        children_el.closest('.seta_dashboard_filter').removeClass('disable_filter')
                                    }else{
                                        children_el.closest('.seta_dashboard_filter').addClass('disable_filter')
        ...
        if (self.$viewDashboard && self.$viewDashboard.$viewDashboardAsk) {
            self.$viewDashboard.$viewDashboardAsk.closest('.seta_view_dashboard_ask_container').hide();
    ...
    _initDashboardAISearch: function () {
        ...
        self.$dashboardSearchContainer.append(`
            <input id="seta_dashboard_search" class="seta_wfull seta_select2" placeholder="Input Keywords to Generate Analysis"/>
        `);
        new SETAAutocomplete(self, {
            elm: self.$dashboardSearchContainer.find(`#seta_dashboard_search`),
            multiple: true,
            ...
            tags: true,
            api: {
                url: `${self.setaLabURL}/lab/analysis`,
                method: 'POST',
                body: {
                    'query': '',
            ...
            formatFunc: function format(item) {
                ...
                if (false && item['premium']) {
                    premium_html = `<span class="seta_dashboard_option_premium">${item['premium']}</span>`
                }
                return `<div class="seta_dashboard_option">
                    <div class="seta_dashboard_option_header">
                        <div class="seta_dashboard_option_name">
                            ${material_icon_html}
                            ${item['name']}
                        </div>
                        <div class="seta_dashboard_option_category">
                            ${category_html}
                            ${premium_html}
                        </div>
                    </div>
                    <div class="seta_dashboard_option_visual">
    ...
    _getLabAnalysisConfig: function (id, name) {
        ...
        rpc('/web/dataset/call_kw/seta.dashboard/action_get_lab_analysis_config', {
            model: 'seta.dashboard',
            ...
            if (res && res.status == 200) {
                ...
                setTimeout(function () {
                    ...
                    $('.o_content').animate({ scrollTop: $('.seta_view_dashboard').height() }, 3000);
                    if (self.$viewDashboard && self.$viewDashboard.$grid) {
                        var layout = self.$viewDashboard.$grid.save(false)
                        if (layout) {
                            rpc('/web/dataset/call_kw/seta.dashboard.block/ui_save_layout', {
                ...
                if (res.status == 401) {
                    ...
                    self._getOwl().action.doAction({
                        ...
                        target: 'new',
                        res_model: 'seta.lab.api.key.wizard',
                        views: [[false, 'form']],
                        context: {},
                    }, {
                        onClose: function () {
    ...
    _onKeyUpAISearchInput: function (ev) {
        ...
        if (ev.key === 'Enter' || ev.keyCode === 13) {
            ...
            self.typingTimer = setTimeout(function () {
                ...
                rpc('/web/dataset/call_kw/seta.dashboard/action_ai_search', {
                    model: 'seta.dashboard',
    ...
    _onClickAIGenerate: function (ev) {
        var self = this;
        // Check if seta_dashboard_search_container is visible
        if (self.$('#seta_dashboard_search_container').is(':visible')) {
            self.$('#seta_dashboard_search_container').hide();
            self.$('#seta_dynamic_filter_container').show();
            self.$('#seta_dashboard_filter_date_format').show();
        } else {
            self.$('#seta_dashboard_search_container').show();
            self.$('#seta_dynamic_filter_container').hide();
            self.$('#seta_dashboard_filter_date_format').hide();
        }
    },
    ...
    _onClickAIAsk: function (ev) {
        ...
        var $container = self.$viewDashboard.$viewDashboardAsk.closest('.seta_view_dashboard_ask_container');
        self.$viewDashboard.$viewDashboardAsk.empty();
        ...
        if (theme_id && theme_name) {
            new swal({
                ...
                if (result.isConfirmed) {
                    ...
                    rpc('/web/dataset/call_kw/seta.dashboard/write', {
                        model: 'seta.dashboard',
                        method: 'write',
    ...
    _onClickEditLayout: function (ev) {
        ...
        self.$viewDashboard.$el.addClass('seta_edit_mode');
    },
    _onClickEmbedDashboard: function (ev) {
        var self = this;
        rpc('/web/dataset/call_kw/seta.dashboard/generate_access_token', {
            model: 'seta.dashboard',
            ...
            if (access_token) {
                self.setaDashboardAcessToken = access_token;
                var url = `${self.baseUrl}/seta/dashboard/${self.selectedDashboard}/page?access_token=${self.setaDashboardAcessToken}`
            ...
            if (layout) {
                rpc('/web/dataset/call_kw/seta.dashboard.block/ui_save_layout', {
                    ...
                    if (result.status == 200) {
                        ...
                        self.$viewDashboard.$el.removeClass('seta_edit_mode');
                        self._loadFilteredDashboard();
    ...
    _updateSubtitleText: function (date_format) {
        ...
        self.$selectDashboard.find('.seta_subtitle').text(subtitle);
    },
    _selectDashboard: function (id, name, write_date, theme_name, date_format, start_date, end_date, use_sidebar) {
        ...
        if (date_format == 'custom' && (start_date || end_date)) {
            ...
            self.filterDateRange.elm.find('#seta_date_from').val(start_date)
            self.filterDateRange.elm.find('#seta_date_to').val(end_date)
            self.filterDateRange.elm.show();
        } else if (date_format) {
            self.filterDateFormat.values = date_format;
            self.filterDateRange.elm.find('#seta_date_from').val('')
            self.filterDateRange.elm.find('#seta_date_to').val('')
            self.filterDateRange.elm.hide();
        } else {
            self.filterDateFormat.values = null;
            self.filterDateRange.values = [null, null];
        ...
        // self.$selectDashboard.find('.seta_subtitle').text(moment(write_date).format('LLL'));
        ...
        if (self.selectedDashboard) {
            new swal({
                ...
                if (result.isConfirmed) {
                    ...
                    rpc('/web/dataset/call_kw/seta.dashboard/unlink', {
                        model: 'seta.dashboard',
                        method: 'unlink',
                        args: [self.selectedDashboard],
                        kwargs: {},
    ...
    _onClickDateFilterButton: function(ev) {
        if ($(ev.target).closest('#seta_inner_config_dashboard').length > 0) {
            ...
            $(".seta_datefilter_dropdown").remove();

            // Pastikan dropdown ada sebelum dikloning
            if ($dropdownMenu.length === 0) return;

            // Kloning dropdown agar bisa dipindahkan ke body
            // let $clonedDropdown = $dropdownMenu.clone().addClass("seta_datefilter_dropdown").appendTo("body");
            let $clonedDropdown = $dropdownMenu.clone().addClass("seta_datefilter_dropdown").appendTo(".seta_config_dashboard");
            ...
            $(document).on("click", ".seta_datefilter_dropdown .dropdown-item", function () {
                let selectedValue = $(this).data("date_format");
                $button.text($(this).text()); // Ubah teks tombol
                $(".seta_datefilter_dropdown").remove(); // Hapus dropdown setelah memilih
            });
        }
    },
    ...
    _onClickAISlide: function (ev) {
        ...
        if (self.selectedDashboard) {
            var self = this;
            rpc('/web/dataset/call_kw/seta.dashboard/action_check_key', {
                model: 'seta.dashboard',
    ...
    _onClickExportConfiguration: function (ev) {
        ...
        rpc('/web/dataset/call_kw/seta.dashboard/export_all_config', {
            model: 'seta.dashboard',
    ...
    _onClickImportData: function (ev) {
        ...
        if (self.selectedDashboard) {
            self._getOwl().action.doAction({
                ...
                name: _t('Dashboard'),
                target: 'new',
                res_model: 'seta.data.source.item',
                views: [[false, 'form']],
    ...
    _onClickAddConfig: function (ev) {
        ...
        if (self.selectedDashboard) {
            self._getOwl().action.doAction({
                ...
                name: _t('Dashboard'),
                target: 'new',
                res_model: 'seta.dashboard.config.wizard',
                views: [[false, 'form']],
                context: { 'default_dashboard_id': self.selectedDashboard },
            }, {
                onClose: function () {
...
export default SETAConfigDashboard;
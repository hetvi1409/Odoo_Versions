...
$(function() {
    ...
    $.ajax(`${base_url}/seta/dashboard/${dashboard_id}?access_token=${access_token}`, {
        headers: {
        },
        type : 'GET',
    ...
    // Export Capture
    $('body').on('click', '#seta_export_capture', function(ev) {
    ...
    // Select date filter
    $('body').on('click', '.seta_select_date_format', function(ev) {
        _onClickSelectDateFormat(ev);
    });

    // Select theme
    $('body').on('click', '.seta_select_theme', function(ev) {
...
function _loadThemes(themes) {
    var self = {
        $themeContainer: $('.seta_select_theme_container')
    };
...
function _initDashboardFilter() {
    selfDashboard.filterDateRange = {
            elm: $('#seta_dashboard_filter_date_range'),
            values: [null, null],
        };
    selfDashboard.filterDateFormat = {
            elm: $('#seta_dashboard_filter_date_format'),
            values: null,
        };
    selfDashboard.$dynamicFiltersContainer = $('#seta_dynamic_filter_container');
    selfDashboard.$dynamicFilters = {};
    selfDashboard.dashboard_id = $('#dashboard_id').val();
    selfDashboard.base_url = window.location.origin;

    var $dateFromElm = selfDashboard.filterDateRange.elm.find('#seta_date_from');
    var $dateToElm = selfDashboard.filterDateRange.elm.find('#seta_date_to');
    ...
    if (selfDashboard.filters.date_format) {
        ...
        selfDashboard.filterDateFormat.elm.find('.seta_dashboard_filter_content .dropdown-toggle').text(dateFormatText);
    ...
    $.ajax(`${selfDashboard.base_url}/seta/dashboard/filters/${selfDashboard.dashboard_id}`, {
        headers: {
        },
        type : 'GET',
        success: async function(res, status){
                ...
                results.forEach(function (filter) {
                    if (results[0] == filter) {
                        selfDashboard.$dynamicFiltersContainer.append(`
                            <div class="seta_dashboard_filter">
                    ...
                    if (filter.params) {
                        var $dF = new SETAAutocomplete(selfDashboard, {
...
function _onClickSelectDateFormat(ev) {
    ...
    selfDashboard.filterDateFormat.elm.find('.seta_dashboard_filter_content .dropdown-toggle').text(text);
...
function _loadFilteredDashboard(mode=false) {
    ...
    if (date_format) {
        ...
        $('.seta_config_dashboard').find('.seta_subtitle').text('');
    }
...
function _updateSubtitleText(date_format){
    var self = {
        $configDashboard: $('.seta_config_dashboard')
    };
    var subtitle = "";
    ...
    self.$configDashboard.find('.seta_subtitle').text(subtitle);
}
...
function generateDashboard(data) {
    ...
    try {
        amChartsThemeExtra.applyTheme(themeName);
    } catch (error) {
    ...
    self.dashboardBlocks.forEach(block => {
        ...
        if (block.analysis_id) {
            ...
            index += 1;
            var $block = $(`
                <div class="seta_dashboard_block_item" data-id="${block.id}">
                    <div class="seta_dashboard_block_header">
                        <div class="seta_block_left seta_dropdown dropdown">
                            <h4 class="seta_dashboard_block_title dropdown-toggle" data-toggle="dropdown">${block.analysis_id[1]}</h4>
                            <div class="dropdown-menu">
                                <a class="dropdown-item seta_action_quick_open_analysis">Open Analysis</a>
                                <a class="dropdown-item seta_action_edit_analysis">Configuration</a>
                                <a class="dropdown-item seta_action_open_list_view">View List</a>
                                <a class="dropdown-item seta_action_export_excel" data-id="${block.id}">Export Excel</a>
                                <a class="dropdown-item seta_action_delete_block" data-id="${block.id}">Remove Analysis</a>
                            </div>
                        </div>
                    </div>
                    <div class="seta_dashboard_block_content">
                        <div class="seta_view_visual h-100"></div>
                    </div>
                </div>
            `);
...
function generateVisual(block, args = false) {
    ...
    $.ajax(`${base_url}/seta/analysis/${block.analysis_id[0]}/data?access_token=${access_token}&kwargs=${kwargs}`, {
        headers: {
        },
        type : 'GET',
        success: async function(res, status){
...
function _onHitChart(ev, visual, val) {
    ...
    var block_element = clicked_visual.closest('.seta_dashboard_block_item');

    if (block_element) {
        block_id = block_element.getAttribute('data-id');
        var visual_element = block_element.querySelector('.seta_view_visual');
    ...
    if (self && self.dimension_alias && val) {
        ...
        $.ajax(`${base_url}/seta/analysis/${self.analysis_id[0]}/ui_get_available_fields`, {
            headers: {
            },
            type : 'GET',
            success: async function(res, status){
                ...
                res.forEach(item => {
                    ...
                    menu_item.addEventListener('click', () => {
                        ...
                        _getDataAnalysis(self, args, function (result) {
                            ...
                            self.$el.find('.seta_reset_drilldown').show();
                            self.$el.find('.seta_drillup').show();
                        });
                        drilldown_background.remove();
                    });
...
function _getDataAnalysis(self, kwargs, callback) {
    ...
    if (self.analysis_id) {
        ...
        $.ajax(`${base_url}/seta/analysis/${self.analysis_id[0]}/try_get_analysis_data_dashboard?kwargs=${string_kwargs}&access_token=${this_access_token}`, {
            headers: {
            },
            type : 'GET',
            success: async function(res, status){
...
function makeChart(block, result) {
    var self = {
        $el: $(`.seta_dashboard_block_item[data-id="${block.id}"] .seta_view_visual`),
        analysis_id: block.analysis_id[0],
        block_id: block.id,
        animation: block.animation,
    ...
    self.$el.parent().removeClass('seta_view_background');
    self.$el.removeClass('seta_view_scrcard_container scorecard scorecard-sm');
    self.$el.parents(".seta_dashboard_block_item").removeClass("seta_dashboard_block_item_v_background");
    ...
    if (visual_type == 'custom') {
        ...
        if ((self.$el.attr('id')).indexOf('block') === -1) { // layout ketika di preview chart
            self.$el.parents(".seta_dashboard_block_item").addClass("seta_dashboard_block_item_v_background");
            self.$el.parent().addClass('seta_view_background');
            self.$el.addClass('seta_view_scrcard_container');
        }else{ // layout ketika di block Dashboard 
            self.$el.parents(".seta_dashboard_block_item").find(".seta_dashboard_block_title").text("");
            self.$el.parents(".seta_dashboard_block_item").find(".seta_dashboard_block_header").addClass("seta_dashboard_block_btn_config");
        }
    ...
    // Drill Up
    self.$el.find('.seta_reset_drilldown').remove();
    self.$el.append(`
    <button class="btn btn-sm btn-primary seta_reset_drilldown" `+ style +`>
    <i class="fa fa-chevron-up"></i>
    </button>
    `);

    // One step drill up
    self.$el.find('.seta_drillup').remove();
    self.$el.append(`
        <button class="btn btn-sm btn-primary seta_drillup" `+ style +`>
            <i class="fa fa-arrow-left"></i>
        </button>
    `);

    self.$el.find('.seta_reset_drilldown').on('click', function(event) {
        var visual = false
        var analysis_id = false;
    ...
    self.$el.find('.seta_drillup').on('click', function(event) {
        var visual = false
        var analysis_id = false;
        var clicked_visual = event.target;
        var block_element = clicked_visual.closest('.seta_dashboard_block_item');
        ...
        if(visual.drilldown_history.length > 1){
            ...
            _getDataAnalysis(visual, args, function (result) {
                ...
                visual.$el.find('.seta_reset_drilldown').show();
                visual.$el.find('.seta_drillup').show();
            })
        }else{
            resetDrilldown(visual)
        }
...
function resetDrilldown(visual) {
    ...
    _getDataAnalysis(visual, args, function (result) {
        ...
        makeChart(visual, result)
        visual.$el.find('.seta_reset_drilldown').hide();
        visual.$el.find('.seta_drillup').hide();
    })
}
...
function _onClickExportCapture(ev) {
    var self = {
        $titleDashboard: $('.seta_title_dashboard'),
        $btnExportCapture: $('#seta_export_capture'),
        $btnExportLoading: $('#seta_export_capture_loading'),
    ...
    if (self.selectedDashboard) {
        ...
        html2canvas(document.querySelector('.seta_view'), {useCORS: true, allowTaint: false}).then(function(canvas){
    ...
    initWithORMDynamic(){
        ...
        self.elm.select2({
            ...
            query: function (query) {
                ...
                $.ajax(`${self.base_url}/seta/analysis/fields/dynamic/${self.analysisId}?kwargs=${kwargs}`, {
                    headers: {
                    },
                    type : 'GET',
                    success: async function(res, status){
                        if (res.data && res.code == 200) {
                            ...
                            setTimeout(function() {
                                ...
                                var left_position = self.parent.$el.find("#seta_dynamic_filter_container_custom").position().left;
            ...
            query: function (query) {
                ...
                if (query && !loadingRPC && self.params) {
                    typingTimer = setTimeout(function() {
                        ...
                        $.ajax(`${self.base_url}/seta/dashboard/filters/values?kwargs=${kwargs}`, {
                            headers: {
                            },
                            type : 'GET',
                            success: async function(res, status){

...
class rental_contract(models.Model):
    ...
    def _default_security_deposit_account(self):
        ...
        return int(security_deposit_account) if security_deposit_account else ""
    ...
    def _default_income_account(self):
        ...
        return int(account_income) if account_income else ""
    ...
    region= fields.Many2one('regions','Portfolio', )
    ...
    select_type = fields.Selection([
        ('date', 'Fixed from Date'),
        ('months', 'Fixed from Month'),
        ('cpi_u', 'CPI-U'),
        ('fixed_percent', 'Fixed %'),
        ('min_max', 'Min/Max'),
        ('escalation_anniversary', 'Date of Escalation Anniversary')
    ], string='Type')
    month_type = fields.Selection([('6', '6'), ('12', '12'), ('18', '18'), ('24', '24')], string='Months')
    date_type = fields.Date('Date')
    percentage = fields.Float('Percentage(%)')
    late_fee = fields.Float('Late Fee')
    active = fields.Boolean(string='Active', default=True)

    energy_budget = fields.Float("Energy Budget")
    energy_consumption_total = fields.Float("Total Consumption", compute="_compute_energy_totals", store=True)
    energy_cost_total = fields.Float("Total Energy Cost", compute="_compute_energy_totals", store=True)
    integration_enabled = fields.Boolean("Integration Enabled")
    external_system_id = fields.Char("External System Reference")

    # Budget vs Actual
    energy_budget_variance = fields.Float("Budget Variance", compute="_compute_budget_analysis", store=True)
    energy_budget_percent = fields.Float("Budget % Used", compute="_compute_budget_analysis", store=True)

    @api.depends('energy_budget', 'energy_cost_total')
    def _compute_budget_analysis(self):
        for contract in self:
            if contract.energy_budget:
                contract.energy_budget_variance = contract.energy_budget - contract.energy_cost_total
                contract.energy_budget_percent = (contract.energy_cost_total / contract.energy_budget) * 100.0
            else:
                contract.energy_budget_variance = 0.0
                contract.energy_budget_percent = 0.0

    def _get_contract_meters(self):
        return self.env['utility.meter'].search([('contract_id', 'in', self.ids)])

    @api.depends('partner_id', 'building_unit')
    def _compute_energy_totals(self):
        for contract in self:
            meters = self.env['utility.meter'].search([('contract_id', '=', contract.id)])
            readings = self.env['utility.meter.reading'].search([('meter_id', 'in', meters.ids)])
            contract.energy_consumption_total = sum(readings.mapped('consumption'))
            contract.energy_cost_total = sum(readings.mapped('invoice_amount'))


    def process_escalation(self):
        for rec in self:
            if rec.select_type == 'date':
                escalation_date = rec.date_type
            else:
                months = int(rec.month_type)
                escalation_date = rec.date_from + relativedelta(months=months)

            if not escalation_date:
                continue

            affected_lines = rec.loan_line.filtered(lambda line: line.date and line.date > escalation_date)

            for line in affected_lines:
                old_amount = line.amount
                new_amount = old_amount * rec.percentage / 100.0
                line.amount = old_amount + new_amount
    ...
    def _check_percentage(self):
        for rec in self:
            if rec.percentage < 0 or rec.percentage >= 100:
                raise ValidationError("Percentage must be greater than 0 and less than 100.")
    ...
    def create(self, vals_list):
        ...
        return super(rental_contract, self).create(vals_list)
...
class loan_line_rs_rent(models.Model):
    ...
    _description = "Loan Line rs Rent"
    ...
    contract_region= fields.Many2one(related='loan_id.region',string="Portfolio", store=True)

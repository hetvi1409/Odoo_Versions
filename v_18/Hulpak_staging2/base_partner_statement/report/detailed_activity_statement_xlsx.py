...
class DetailedActivityStatementXslx(models.AbstractModel):
            ...
            [
                ...
                for col_pos, args in enumerate(
                    [
                        ...
                        (line.get("invoice_date") or line.get("date", ""), format_tcell_date_left),
                    ]
                )
            ]
            + [
    ...
    def _write_currency_lines(self, row_pos, sheet, partner, currency, data):
        ...
        sheet.merge_range(
            row_pos, 0, row_pos, 7, statement_header, FORMATS["format_left_bold"]
        )
        row_pos += 1
        ...
        return [
            ...
            for col_pos, args in enumerate(
                [
                    (line.get("move_id", ""), format_tcell_left),
                    (line.get("invoice_date") or line.get("date", ""), format_tcell_date_left),
                    (line.get("date_maturity") or line.get("invoice_date",""), format_tcell_date_left),
    ...
    def generate_xlsx_report(self, workbook, data, objects):
        ...
        report_model = self.env["report.base_partner_statement.detailed_activity_statement"]

...
class OutstandingStatementXslx(models.AbstractModel):
    ...
    def _get_currency_line_row_data(self, partner, currency, data, line):
        ...
        if line.get("name", "") and line.get("name", "") != "/":
            if not line.get("ref", ""):
                ...
                if line.get("ra_number", "")  and line.get("ref", "") : 
                    name_to_show = line.get("ra_number", "") + " - " + line.get("ref", "") + " - " + line.get("move_id", "")
                elif (line.get("ref", "") in line.get("name", "")) or (
                    line.get("name", "") == line.get("ref", "")
                ):
                    name_to_show = line.get("name", "")
                else:
                    name_to_show = line.get("name", "") + " - " + line.get("ref", "")
        return [
                {
                    "col_pos": col_pos,
                    "sheet_func": "write",
                ...
                for col_pos, args in enumerate(
                    [
                        ...
                        (line.get("statement_date", ""), format_tcell_date_left),
                        (line.get("statement_due_date", ""), format_tcell_date_left),
        ...
        return [
            {
                ...
                "sheet_func": "write",
                "args": (
                    partner_data.get("end_date") or partner_data.get("end"),
    ...
    def _write_currency_buckets(self, row_pos, sheet, partner, currency, data):
        report_model = self.env["report.base_partner_statement.outstanding_statement"]
        partner_data = data.get("data", {}).get(partner.id, {})
        currency_data = partner_data.get("currencies", {}).get(currency.id)
    ...
    def generate_xlsx_report(self, workbook, data, objects):
        ...
        FORMATS["format_tcell_date_left"] = copy_format(
            workbook, FORMATS["format_tcell_date_left"]
        )
        FORMATS["format_tcell_date_left"].set_num_format("dd\\/mm\\/yyyy")
        FORMATS["format_date_left"] = copy_format(workbook, FORMATS["format_date_left"])
        FORMATS["format_date_left"].set_num_format("dd\\/mm\\/yyyy")

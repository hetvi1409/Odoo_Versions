...
class ActivityStatement(models.AbstractModel):
    ...
    _name = "report.base_partner_statement.activity_statement"
    ...
    def _initial_balance_sql_q1(self, partners, date_start, account_type):
        ...
        return str(
            ...
            SELECT l.partner_id, l.currency_id, l.company_id, l.id, 
                ...
                TO_CHAR(l.invoice_date, 'DD/MM/YYYY') as invoice_date,
            ...
            GROUP BY l.partner_id, l.currency_id, l.company_id, l.balance, l.id, l.date_maturity, l.invoice_date, l.date
            ORDER BY COALESCE(l.date_maturity, l.invoice_date, l.date)
        """,
                locals(),
            ),
            "utf-8",
    ...
    def _display_activity_lines_sql_q1(
        ...
        return str(
            self._cr.mogrify(
                """
            SELECT m.name AS move_id, l.partner_id, TO_CHAR(l.date, 'DD/MM/YYYY') as date, 
                ...
                CASE
                    WHEN (aj.type IN ('sale', 'purchase')) AND l.name IS NOT NULL
                        THEN l.ref
                    WHEN (aj.type IN ('general')) AND l.name IS NOT NULL
                        THEN l.name
            ...
            WHERE l.partner_id IN %(partners)s
                ...
                AND CASE
                    ...
                    ELSE TRUE
                END
            GROUP BY l.partner_id, m.name, l.date, l.date_maturity, l.invoice_date, l.date, l.no_followup,
                CASE WHEN (aj.type IN ('sale', 'purchase'))
                    THEN l.name
                    ELSE '/'
    ...
    def _display_activity_lines_sql_q2(self, sub, company_id):
        ...
        return str(
            self._cr.mogrify(
                f"""
            SELECT {sub}.partner_id, {sub}.move_id, {sub}.date, {sub}.date_maturity, {sub}.invoice_date, {sub}.no_followup,
    ...
    def _get_account_display_lines(
        ...
        WITH Q1 AS ({}),
             Q2 AS ({})
        SELECT partner_id, move_id, date, date_maturity, invoice_date, no_followup, ids,
            COALESCE(name, '') as name, COALESCE(ref, '') as ref,
            debit, credit, amount, currency_id
        FROM Q2
        ORDER BY date, date_maturity, invoice_date, no_followup, move_id""".format(
    ...
    def _display_activity_reconciled_lines_sql_q2(self, sub, date_end):
        return str(
            self._cr.mogrify(
                f"""
            SELECT l.id as rel_id, m.name AS move_id, l.partner_id, TO_CHAR(l.date, 'YYYY-MM-DD') as date, l.name, l.full_reconcile_id,
            ...
            END AS open_amount_currency,
            TO_CHAR(l.date_maturity, 'YYYY-MM-DD') as date_maturity,
            ...
            GROUP BY l.id, l.partner_id, m.name, l.date, l.date_maturity, l.invoice_date, l.no_followup, l.name,
                CASE WHEN l.ref IS NOT NULL
                    ...
                    ELSE m.ref
                END, 
                l.currency_id, l.balance, l.amount_currency, l.company_id, l.full_reconcile_id,
            ORDER BY COALESCE(l.date_maturity, l.invoice_date, l.date)
        """,
                locals(),
            ),
    ...
    def _get_account_display_reconciled_lines(
        ...
        WITH Q1 AS ({}),
             ...
             Q5 AS ({}),
             Q6 AS ({})
        SELECT partner_id, currency_id, move_id, date, date_maturity, invoice_date, no_followup, debit,

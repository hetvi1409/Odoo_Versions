...
class ResPartner(models.Model):
    ...
    @api.depends('cus_overdue_ids', 'filter_by', 'period_from', 'period_to')
    ...
    cus_overdue_payment_ids = fields.One2many('account.payment', 'partner_id',
                                              compute='_compute_cus_overdue_payment_ids',
                                              string="Customer Overdue Payments")
    ...
    ageing_length = fields.Integer('Days Length', compute='_compute_cus_overdue_ids', default=30, readonly=False,
                                   store=True)
    ...
    def action_cus_overdue_statement_print(self):
        ...
        return self.env.ref('gt_customer_overdue_statement.print_customer_overdue_statement_report').report_action(self)
    ...
    def action_cus_overdue_statement_send(self):
        ...
        try:
            template_id = self.env.ref('gt_customer_overdue_statement.email_template_customer_overdue_statements').id
        except ValueError:
            template_id = False
        try:
        ...
        if not self.company_id:
            raise ValidationError(_('Company Not defined, please check.'))

        ctx.update({
            'default_model': 'res.partner',
            'default_res_ids': self.ids,  # Changed from default_res_id to default_res_ids

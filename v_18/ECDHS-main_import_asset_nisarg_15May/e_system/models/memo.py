...
class Memo(models.Model):
    ...
    def _compute_admin_it_group(self):
        ...
        for rec in self:
            user = rec.env.user
            is_it = it_group in user.groups_id if it_group else False
            is_admin = admin_group in user.groups_id if admin_group else False

            if is_it or is_admin:
                rec.admin_it_group = [(6, 0, [user.id])]
            else:
                rec.admin_it_group = [(6, 0, [])]

    def _compute_can_delete_lines(self):
        it_group = self.env.ref('e_system.group_it_team')  # replace with your XML ID
        for rec in self:
            rec.can_delete_lines = not (it_group in self.env.user.groups_id)
    ...
    def action_publish(self):
        # Publish stage is intentionally disabled.
        # Keep this method defined only so legacy views referencing
        # action_publish remain valid during module upgrades.
        #
        # Disabled behavior (do not remove for traceability):
        # - publish-state validation requirement
        # - publish stage transition
        # - publish notifications / emails
        #
        # for memo in self:
        #     if memo.state != 'approved':
        ...
        #         raise ValidationError(_("Please approve the document before publish."))
        #
        #     memo.message_post(
        #         body=_('Document %s has been published.', memo.name or _('(no title)')),
        #         message_type='notification',
...
class MemoApprover(models.Model):
    ...
    def _compute_is_editable(self):
        ...
        for rec in self:
            ...
            ] or it_group in current_user.groups_id
            )

...
class ShortTermPlaning(models.Model):
    """ This model represents short.term.planing."""
    _name = 'short.term.planing'
    _description = 'Short Term Planing'
    ...
    medium_term_id = fields.Many2one('medium.term.planing', string="Medium Term")

    @api.onchange('medium_term_id', 'long_term_id', 'indicator_id', 'output_id', 'outcome_id',
                  'sub_programme_id', 'programme_id')
    def _onchange_sub_programme_id(self):
        """Triggered when the sub_programme_id changes to update related values."""
        if self.medium_term_id:
            self.long_term_id = self.medium_term_id.long_term_id.id

...
class InternalAuditRevert(models.TransientModel):
    ...
    def action_submit(self):
        ...
        if self.audit_action_plan_id:
            ...
            if record.stages == 'preparer':
                res_user_id = record.preparer_id
            if record.stages == 'rejected':
                res_user_id = record.reviewer_id
        ...
        if self.financial_statement_id:
            ...
            previous_state = record.state
            record.state = 'reverted'
            record.env['financial.statement.tracking'].create({
        ...
        if record:
            if self.audit_method_id:
                ...
                record.state = 'reverted'
                return
            elif self.internal_audit_plan_id:
                record.env['audit.revert'].create({
                    'res_id': record.id,
                    'res_model': record._name,
                    'comments': self.comments,
                ...
                record.state = 'reverted'
                return
            elif self.audit_action_plan_id:
                record.env['audit.revert'].create({
                    'res_id': record.id,
                    'res_model': record._name,
                    'comments': self.comments,
                ...
                record.state = 'reverted'
                return
            elif self.financial_statement_id:
                record.env['audit.revert'].create({
                    'res_id': record.id,
                    'res_model': record._name,
                    'comments': self.comments,
                ...
                record.state = 'reverted'
                return
            elif self.internal_quality_control_id:
                record.env['audit.revert'].create({
                    'res_id': record.id,
                    'res_model': record._name,
                    'comments': self.comments,

...
class AccountAsset(models.Model):
    ...
    def get_movable_asset_details(self):
        ...
        sql_query = """
            ...
            INNER JOIN asset_category AS categ ON asset.afs_classification = categ.id
            WHERE asset.afs_classification IS NOT NULL
    ...
    def get_movable_asset_graph_details(self):
        sql_query = """SELECT categ.name,
                    (SELECT COALESCE(SUM(asset.original_value), 0) FROM account_asset AS asset WHERE asset.afs_classification = categ.id)
    ...
    def get_overall_movable_asset_details(self):
        ...
        for categ in categories:
            assets = self.env['account.asset'].search([('afs_classification', '=', categ.id)])
    ...
    def get_asset_verification_number(self):
        sql_query = """
            SELECT
                COUNT(DISTINCT CASE WHEN avh.is_verified = TRUE THEN asset.id END) as verified_count,
                COUNT(DISTINCT CASE WHEN avh.is_verified IS NULL OR avh.is_verified = FALSE THEN asset.id END) as not_verified_count
            FROM account_asset AS asset
            LEFT JOIN asset_verification_history AS avh ON avh.history_id = asset.id
        """
    ...
    def get_asset_verification_carrying_value(self):
        sql_query = """
            SELECT
                COALESCE(SUM(CASE WHEN avh.is_verified = TRUE THEN asset.original_value ELSE 0 END), 0) as verified_value,
                COALESCE(SUM(CASE WHEN avh.is_verified IS NULL OR avh.is_verified = FALSE THEN asset.original_value ELSE 0 END), 0) as not_verified_value
            FROM account_asset AS asset
            LEFT JOIN asset_verification_history AS avh ON avh.history_id = asset.id
        """
    ...
    def get_asset_not_verified(self):
        ...
        sql_query = """
            ...
            INNER JOIN asset_category AS categ ON asset.afs_classification = categ.id
            LEFT JOIN asset_verification_history AS avh ON avh.history_id = asset.id
            WHERE avh.is_verified = FALSE OR avh.is_verified IS NULL
            GROUP BY categ.id, categ.name
            ORDER BY categ.name
        """

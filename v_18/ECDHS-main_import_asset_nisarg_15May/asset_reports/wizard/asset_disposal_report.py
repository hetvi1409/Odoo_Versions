...
class AssetDisposalReports(models.TransientModel):
    ...
    def print_sample_report(self):
        ...
        return self.env.ref(
            'asset_reports.action_report_asset_disposal').report_action(None,
                                                                        data=data)
    ...
    def get_xlsx_report(self, data, response):
        ...
        sheet.write('AL5', 'Accumulated Depreciation Closing Balance',
                    cell_format)
        sheet.write('AM5', 'Accumulated Impairment Closing Balance',
                    cell_format)
        sheet.write('AN5', 'Amount Realised', cell_format)
        sheet.write('AO5', 'Profit/Loss On Disposal', cell_format)
        sql_query = """
            SELECT 
                '', a.acquisition_date, a.disposal_date, a.disposal_method,a.disposal_method,a.disposal_method,a.disposal_method,a.disposal_method,a.disposal_method,a.disposal_method,a.disposal_method,a.disposal_method,a.disposal_method,a.disposal_method,a.disposal_method,a.disposal_method,a.disposal_method 
            FROM 
                account_asset AS a
            WHERE 
                a.state != 'model'
        """
        # a.condition, department.name AS department_name, division.name AS division_name,ownership.name AS ownership_name, a.deed_number, a.custodian,
        #                 # a.physical_city, a.physical_street,
        #                 # a.building, a.room_number
        # LEFT JOIN
        #     department ON department.id = a.department_id
        # LEFT JOIN
        #     division ON division.id = a.division_id
        # LEFT JOIN
        #     ownership ON ownership.id = a.ownership_id
        # LEFT JOIN
        #     asset_type AS at ON at.id = a.asset_type_id
        # LEFT JOIN
        #     asset_category AS ac ON ac.id = a.asset_category_id
        if data.get('from_date'):
            sql_query += " AND a.acquisition_date > %(from_date)s"
        if data.get('to_date'):
            sql_query += " AND a.acquisition_date < %(to_date)s"

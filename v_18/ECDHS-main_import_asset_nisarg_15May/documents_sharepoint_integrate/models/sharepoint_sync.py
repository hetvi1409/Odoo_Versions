...
class SharePointSync(models.Model):
    ...
    def compute_sharepoint_folder_path(self, folder):
        ...
        while current_folder:
            ...
            current_folder = current_folder.folder_id  # Move to parent folder
    ...
    def sync_folders_to_sharepoint(self, access_token, parent_folder=None, sharepoint_parent_path="",
        ...
        domain = [("folder_id", "=", parent_folder.id),("type", "=", "folder")] if parent_folder else [("folder_id", "=", False),("type", "=", "folder")]
        folders = self.env["documents.document"].search(domain)
        ...
        for folder in folders:
            ...
            # Upload files
            documents = self.env["documents.document"].search([("folder_id", "=", folder.id),("type", "=", "binary")])
            ...
            subfolders = self.env["documents.document"].search([("folder_id", "=", folder.id),("type", "=", "folder")])
    ...
    def sync_sharepoint_folders_to_odoo(self, access_token):
        ...
        root_folders = self.env["documents.document"].search([("folder_id", "=", False), ("type", "=", "folder")])
        ...
        for root_folder in root_folders:
            ...
            for item in sharepoint_items:
                if "folder" in item:  # It's a folder
                    ...
                    parent_folder = folder_mapping.get(parent_path)
                    ...
                    if not parent_folder:
                        parent_folder = self.env["documents.document"].search([("name", "=", parent_path), ("type", "=", "folder")], limit=1)
                    ...
                    if not existing_folder:
                        ...
                        new_folder = self.env["documents.document"].create({
                            "name": folder_name,
                            "folder_id": parent_folder.id, # Assign to correct parent
                            "type": "folder"
                        })
                ...
                # ✅ Process Files in Parallel
                    ...
                    if not odoo_folder:
                        ...
                        for part in folder_parts:
                            existing_folder = self.env["documents.document"].search([
                                ("name", "=", part),
                                ("folder_id", "=", current_parent.id if current_parent else False),
                                ("type", "=", "folder")
                            ], limit=1)

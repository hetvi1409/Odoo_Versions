...
class AssessmentTransaction(models.TransientModel):
    ...
    def action_submit(self):
        ...
        transaction = self.env['documents.folder'].sudo().search([
            ('name', '=', 'Transaction Report'),
            ('parent_folder_id', '=', enquiry_folder.id)
        ])
        if not transaction:
            transaction = self.env['documents.folder'].sudo().create({
                'name': 'Transaction Report',
                'parent_folder_id': enquiry_folder.id,
            })
        sign_transaction = self.env['documents.folder'].sudo().search([
            ('name', '=', 'Sign Transaction Report'),
            ('parent_folder_id', '=', transaction.id)
        ])
        if not sign_transaction:
            sign_transaction = self.env['documents.folder'].sudo().create({
                'name': 'Sign Transaction Report',
                'parent_folder_id': transaction.id,
            })

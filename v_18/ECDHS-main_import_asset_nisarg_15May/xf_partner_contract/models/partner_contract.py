...
class PartnerContract(models.Model):
    ...
    ref = fields.Char(
        ...
        required=True,
        tracking=True,
        copy=False,default='000'
    )
    active = fields.Boolean(
        default=True,
    )
    kanban_state = fields.Selection([('draft', 'Draft'), ('running', 'Running'),
                                     ('expired', 'Expired')],
                                    string='Kanban State',
                                    default='draft',
                                    tracking=True,
    ...
    has_access_field = fields.Boolean(
    ...
    use_lines = fields.Boolean(
        string='Bill of Quantities/Deliverables',
        default=True,
        readonly=True,
    )
    ...
    document_ids = fields.One2many(
        ...
        inverse_name='res_id',
        readonly=True,
        domain=[('res_model', '=', 'xf.partner.contract'), ('shortcut_document_id', '=', False)]
    )
    ...
    contract_duration = fields.Integer(
        string='Contract Duration (in months)', required=True
    )
    variation_order_amount = fields.Float(string="Variation Order Amount")
    cost_based_progress = fields.Float(string="Cost-Based Process")
    variation_order_percentage = fields.Float(
        string="Variation Order",
        help="This must not exceed 20% of the Contract Award Amount"
    )
    raised_contract_amount = fields.Float(
        string="Raised Contract Amount",
        help="Initial Contract Amount + Variation Order"
    )
    remaining_contract_duration = fields.Integer(string="Remaining Contract Duration (months)")
    time_based_process = fields.Float(string="Time-Based Process")
    extension_period = fields.Integer(string="Extension Period (months)")
    revised_end_date = fields.Date(
        string="Revised End Date",
        store=True
    )
    region_id = fields.Many2one('res.region', string='Region', related='municipality_id.region_id')
    municipality_id = fields.Many2one('res.municipality', string='Municipality')
    project_id = fields.Many2one('project.project', string='Project')
    project_description = fields.Html(related='project_id.description')
    date_doc_signed = fields.Date(string='Date Document Signed')
    scmu = fields.Char(string='SCMU')
    sla_contract_signed = fields.Binary(string='SLA/Contract')
    ...
    penalties = fields.Binary(string='Penalties and Other Correspondence')
    ...
    acceptance_letter = fields.Binary(string='Acceptance Letter')
    project_plan = fields.Binary(string='Project Plan')
    terms_of_reference = fields.Binary(string='Terms of Reference')
    public_liability = fields.Binary(string='Public Liability')
    ...
    terms_of_reference_html = fields.Html(
        string='Terms of Reference',
        copy=True,
    )
    template_variable_ids = fields.One2many(
        related='template_id.variable_ids',
    ...
    def _onchange_template_id(self):
        if self.template_id:
            self.contract_body_html = self.template_id.body_html
            self.scope_of_work = self.template_id.scope_of_work
            self.terms_of_reference_html = self.template_id.terms_of_reference
    ...
    tender_id = fields.Many2one(
        ...
        ondelete='set null',
        tracking=True,
        help='Awarded tender from SCM that originated this contract',
    )

    # Generate Contract Number on Save
    @api.model
    def create(self, vals):
        if vals.get('type') and vals.get('company_id'):
            company_abbr = self.env['res.company'].sudo().browse(int(vals.get('company_id'))).company_abbreviation
            sequence = self.env['ir.sequence'].next_by_code('contract.sequence') or '001'
            year = datetime.now().year
            vals['ref'] = f"{company_abbr.upper()}-{vals.get('type')[:3].upper()}-{sequence}-{year}"
    ...
    def _compute_approver(self):
        group = 'xf_partner_contract.group_xf_partner_contract_user'
        for contract in self:
            if not self.env.user.has_groups(group) or not contract.approval_team_id:
            ...
            contract.is_current_approver = (
                        (contract.current_approver and contract.current_approver.user_id == self.env.user)
    ...
    def _compute_access(self):
        """Compute if the current user has access to edit the contract."""
        group = 'xf_partner_contract.group_xf_partner_contract_user'
        for contract in self:
            if not self.env.user.has_groups(group):
                contract.has_access_field = False
                contract.can_edit = False
                continue
            # Responsible person and superuser have initial access to edit/manage contract
            contract.has_access_field = self.env.user == contract.user_id or self.env.is_superuser()
            if not contract.has_access_field and contract.approval_team_id:
                team_users = contract.approval_team_id.approver_ids.mapped('user_id')
                # Approval team leader and members also have access to edit/manage contract
                contract.has_access_field = (
                            self.env.user == contract.approval_team_id.user_id or self.env.user in team_users)
            # Whoever has access can edit in draft stage or before their own approval according to team settings
            contract.can_edit = ((contract.state == 'draft' and contract.has_access_field) or (
                        contract.is_current_approver and contract.current_approver.can_edit))
    ...
    def _check_active_state(self):
        for record in self:
            ...
            if record.state == 'approval':
                raise UserError(
                    _('You cannot archive contracts when they are in the approval stage! Please cancel them first!'))
    ...
    def _track_subtype(self, init_values):
        ...
        # Broad allowed transitions — management module adds extra states
        # so we keep the guard loosely defined at the base level.
        if init_state:
            allowed_transitions = {
                'draft': [
                    'approval', 'running', 'cancelled',
                    # management states
                    'verify', 'send_service', 'provider_signed',
                    'review_amend', 'verify_contract', 'send_for_betting',
                    'recommend_by_deputy', 'recommend_by_legal',
                    'sign', 'onsite_meeting',
                ],
                'approval': [
                    'running', 'cancelled', 'draft',
                    'verify', 'send_service', 'provider_signed',
                    'review_amend', 'verify_contract', 'send_for_betting',
                    'recommend_by_deputy', 'recommend_by_legal',
                    'sign', 'onsite_meeting',
                ],
                'running': ['to_renew', 'cancelled', 'closed'],
                'to_renew': ['expired', 'cancelled', 'draft'],
                'expired': ['closed', 'cancelled', 'to_renew'],
                'closed': ['draft'],
                'cancelled': ['draft'],
                # management extra states — allow forward and return-to-draft
                'verify': ['send_service', 'draft', 'cancelled',
                           'review_amend', 'send_for_betting'],
                'send_service': ['provider_signed', 'review_amend',
                                 'draft', 'cancelled'],
                'provider_signed': ['review_amend', 'draft', 'cancelled'],
                'review_amend': ['verify_contract', 'draft', 'cancelled'],
                'verify_contract': ['send_for_betting', 'draft', 'cancelled'],
                'send_for_betting': [
                    'recommend_by_deputy', 'draft', 'cancelled'],
                'recommend_by_deputy': [
                    'recommend_by_legal', 'approval', 'draft', 'cancelled'],
                'recommend_by_legal': ['approval', 'draft', 'cancelled'],
                'sign': ['onsite_meeting', 'running', 'draft', 'cancelled'],
                'onsite_meeting': ['running', 'draft', 'cancelled'],
            }
        ...
        if init_state and self.state == 'running':
            ...
            return self.env.ref(
                'xf_partner_contract.mt_contract_confirmed_and_sent')
        ...
        for contract in self:
            if not contract.approval_team_id:
                ...
                if contract.is_current_approver:
                    ...
                    raise AccessError(
                        _('The contract is waiting for the approval from {}') % contract.current_approver.display_name)
    ...
    def set_state(self, from_state, to_state, comments=None):
        print('Setting state', from_state, to_state, comments)
        for record in self:
    ...
    def read_group(self, domain, fields, groupby, offset=0, limit=None, orderby=False, lazy=True):
        ...
        if groupby and groupby[0] == "state":
            return [self._state_group(state, domain, groupby) for state in
                    ['draft', 'approval', 'running', 'to_renew', 'expired', 'closed', 'cancelled']
                    ]
        else:
    ...
    def _state_group(self, state, domain, groupby):
        ...
        return {
            ...
            '__fold': state[0] in ['expired', 'closed', 'cancelled']
        }
    ...
    def _prepare_invoice(self, move_type=False):
        ...
        partner_bank_id = (self.partner_id.commercial_partner_id.bank_ids.
                           filtered_domain(['|', ('company_id', '=', False), ('company_id', '=', self.company_id.id)])[
                           :1])
        return {

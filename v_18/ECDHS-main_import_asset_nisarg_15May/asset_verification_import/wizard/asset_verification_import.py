    ...
    def action_import(self):
        ...
        if self.filename == 'asset_verification_data.xlsx' or self.filename.startswith('asset_verification_data ('):
            ...
            for row in range(2, sheet.max_row + 1):
                ...
                # Create Asset Verification History Record
                ...
                # Update Asset state based on last verification history's condition
                latest_history = AssetVerification.search(
                    [('history_id', '=', asset.id)],
                ...
                if latest_history:
                    if latest_history.condition and latest_history.condition.name == 'Disposal':
                        asset.state = 'asset_disposed'
                    elif latest_history.condition and latest_history.condition.name == 'Good':
                        asset.action_custom_validate()
                    elif latest_history.condition and latest_history.condition.name == 'Bad':
                        asset.state = 'damaged'

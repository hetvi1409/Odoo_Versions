...
class StockPicking(models.Model):
    ...
    responsible_driver = fields.Many2one('res.users')
    ...
    def _compute_delivery_date(self):
        for record in self:
            record.delivery_date = record.scheduled_date.date() if record.scheduled_date else False
    ...
    def button_validate(self):
        ...
        if not self._context.get("from_picking"):

...
export class DeliveryPlan extends Component {
  ...
  setup() {
    ...
    onMounted(async () => {
      try {
        const uid = this.uid;
        const userData = await this.orm.call("res.users", "read", [[uid], ["partner_id"]]);
  ...
  loadMap() {
    ...
    // HTML-based source marker (company location)
    ...
    // Store destination icon generator on this so you can call it later
  ...
  async loadCustomerData() {
    ...
    try {
        ...
        const pickings = await this.orm.searchRead(
            "stock.picking",
            domain,
            ["partner_id", "name", "move_ids_without_package", "location_id", "responsible_driver"]
        );
        ...
        pickings.forEach((picking) => {
            ...
            const orderLines = moves
                .filter((move) => picking.move_ids_without_package.includes(move.id))
                .map((move) => ({
  ...
  async showOrderDetails(event) {
    ...
    popup.css({
      top: offset.top + target.outerHeight() - 50 + "px",

...
export class DocumentsSearchPanel extends SearchPanel {
    ...
    getResModelResIdFromValueGroup(section, value, group) {
        if (value) {
            const field = this.env.model.root.activeFields[section.fieldName];
            return [field.relation || (field.propsFromAttrs && field.propsFromAttrs.relation), section.values.get(value).id];
        } else if (group) {

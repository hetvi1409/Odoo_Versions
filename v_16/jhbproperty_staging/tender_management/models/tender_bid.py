...
class TenderBid(models.Model):
    ...
    def _ensure_editable_in_draft(self, vals):
        """Lock bid field edits outside draft, except controlled workflow/system updates."""
        ...
        if blocked_records:
            raise UserError(
                _("You can only edit bid fields while status is Draft. Use status buttons for workflow changes.")
            )
    ...
    def write(self, vals):
        ...
        return super(TenderBid, self).write(vals)

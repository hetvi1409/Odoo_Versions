...
class FrontdeskVisitor(models.Model):
    ...
    visitor_properties = fields.Properties('Properties', definition='station_id.visitor_properties_definition',
                                           copy=True)
    ...
    type = fields.Selection([('social_lease', 'Social Lease/ Sale'),
                             ...
                             ('outdoor', 'Outdoor Advertising'), ('human_capital', 'Human Capital Management'),
                             ('finance', 'Finance'),
                             ('it', 'I.T'),
                             ('mega_projects', 'Mega Projects'),
                             ('risk_compliance', 'Risk and compliance'),
                             ('property_intelligence', 'Property intelligence and regulatory compliance'),
                             ('special_projects', 'Special Projects'),
                             ('communications', 'Communications'),
                             ('client_operations', 'Client Business Operations'),
                             ('leasing_management', 'Leasing Management'),
                             ('company_secretary', 'Company secretary'),
                             ('legal', 'Legal'),
                             ('strategy_operations', 'Strategy and Operations'),
                             ('informal_trading', 'Informal trading'),
                             ('commercial_city', 'Commercial and city focused interventions'),
    ...
    def create(self, vals_list):
        ...
        vals_list['sequence'] = self.env['ir.sequence'].next_by_code(sequence_code)
        res = super(FrontdeskVisitor, self).create(vals_list)
        return res

    @api.depends("check_in", "state")
    def _compute_is_late(self):
        now = fields.Datetime.now()
        for record in self:
            record.is_late = bool(record.state == "planned" and record.check_in and record.check_in < now)
    ...
    def _notify_by_discuss(self, recipients, msg, is_host=False):
        for recipient in recipients:
            ...
            channel = self.env['mail.channel'].sudo().search([]).filtered(
                lambda l: partners_to in l.channel_partner_ids.ids)
            if channel:
                channel[0].message_post(body=msg, author_id=odoobot_id, message_type="comment",

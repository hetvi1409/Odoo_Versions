...
class Frontdesk(http.Controller):
    def _get_additional_info(self, frontdesk, lang, is_mobile=False):
        return request.render('frontdesk.frontdesk_action_template', {
            'frontdesk': frontdesk,
            'is_mobile': is_mobile,
            'current_lang': lang,
        })
    ...
    @http.route('/get_host_data/<int:frontdesk_id>/<string:token>', type='http', auth='public', website=True)
    def get_host_data_frontdesk(self, frontdesk_id, token, lang='en_US'):
        frontdesk = request.env['frontdesk.frontdesk'].sudo().browse(frontdesk_id)
        return {
    ...
    @http.route('/frontdesk/<int:frontdesk_id>/<string:token>/get_department', type='json', auth='public', csrf=False)
    def get_department(self, frontdesk_id, token):
        department = request.env['hr.department'].sudo().search_read([], ['name', 'id'])
        return department
    ...
    def prepare_visitor_data(self, frontdesk_id, token, visitor_id=None, **kwargs):
        ...
        if visitor:
            ...
            vals.update({
                ...
                'host_ids': [(4, host_id) for host_id in kwargs.get('host_ids')] if kwargs.get('host_ids') != [False] else False,
            })

...
class ResUsers(models.Model):
    ...
    @api.model
    def _microsoft_normalize_email(self, email):
    ...
    def _microsoft_generate_signup_values(self, provider_id, params):
        email = self._microsoft_normalize_email(params.get("email"))
    ...
    def _microsoft_auth_oauth_signin(self, provider_id, params):
        try:
            normalized_email = self._microsoft_normalize_email(params.get("email"))

            users = self.sudo().search([("oauth_uid", "=", params["user_id"]), ("oauth_provider_id", "=", provider_id)], limit=1)
            if not users and normalized_email:
                users = self.sudo().search([
                    "|",
                    ("login", "=ilike", normalized_email),

...
class AssetUSeFulReports(models.TransientModel):
    ...
    def get_xlsx_report(self, data, response):
        ...
        sql_query = """SELECT at.name, ac.name, a.state, '',
                            a.acquisition_date, '', 
                            a.fair_value_less_cost_to_sell, a.closing_book_value_2023, 
                            am.depreciation_value, '', a.carrying_amount,
                            sac.name
        ...
        conditions = []
        if data['from_date']:
            conditions.append("a.acquisition_date > '%s'" % (data['from_date']))
        if data['to_date']:
            conditions.append("a.acquisition_date < '%s'" % (data['to_date']))

        # Join conditions to the WHERE clause if any exist
        if conditions:

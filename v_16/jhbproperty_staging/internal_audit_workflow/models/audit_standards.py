...
class AuditStandards(models.Model):
    _inherit = 'audit.standards'
    ...
    def _compute_can_edit(self):
        ...
        for record in self:
            ...
            # Draft → only Preparer
            if record.state == 'draft' and current_user == record.preparer_id:
                record.can_edit = True

            # Preparer Stage → Reviewer 1
            elif record.state == 'preparer' and current_user == record.reviewer_id:
                record.can_edit = True

            # Second Reviewer Stage → Approver
            elif record.state == 'second_reviewer' and current_user == record.approver_id:
                record.can_edit = True

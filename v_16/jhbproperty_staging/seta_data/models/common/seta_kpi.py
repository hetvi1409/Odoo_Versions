...
class setaKPI(models.Model):
    ...
    category_id = fields.Many2one('seta.kpi.category', string='Category', required=True)
    parent_id = fields.Many2one('seta.kpi', string='Parent')
    child_ids = fields.One2many('seta.kpi', 'parent_id', string='Childs')
    child_count = fields.Integer('Count of Child KPIs', compute='_compute_child_count')
    period_ids = fields.Many2many('seta.kpi.period', string='Periods', required=True)
    ...
    long_sequence = fields.Char('Long Sequence', recursive=True, store=True, compute='_compute_name_and_sequence')
    ...
    line_ids = fields.One2many('seta.kpi.line', 'kpi_id', string='Details', copy=False)
    ...
    date_field_id = fields.Many2one('seta.table.field', string='Date Field', required=False)
    measurement_field_id = fields.Many2one('seta.table.field', string='Measurement', required=False)
    ...
    def create(self, vals):
        rec = super(setaKPI, self).create(vals)
        rec.parent_validation()
        return rec

    def write(self, vals):
        res = super(setaKPI, self).write(vals)
        fields = [
        ...
        if self.child_ids and self.summarize_childs:
            ...
            for period_id in kpi_data_by_period:
                for line_name in kpi_data_by_period[period_id]:
                    ...
                    new_line = self.env['seta.kpi.line'].create(kpi_data_by_period[period_id][line_name])
            ...
            if interval_start_date and interval_end_date and interval_delta:
                ...
                while cur_date <= interval_end_date:
                    ...
                    if cur_name in period_lines_by_name:
                        ...
                        new_line = self.env['seta.kpi.line'].create(line_values)
...
class setaKPICategory(models.Model):
    _name = 'seta.kpi.category'
    _description = 'SETA Kpi Category'

    name = fields.Char('Name')


class setaKPIPeriod(models.Model):
    _name = 'seta.kpi.period'
    _description = 'seta KPI Period'
...
class setaKPILine(models.Model):
    ...
    kpi_id = fields.Many2one('seta.kpi', string='KPI', required=True, ondelete='cascade')
    period_id = fields.Many2one('seta.kpi.period', string='Period')
    name = fields.Char('Name')

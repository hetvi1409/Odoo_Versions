...
class setaAnalysis(models.Model):
    ...
    table_view_id = fields.Many2one('seta.table', string='Table', required=False, ondelete='cascade')
    table_model_id = fields.Many2one('seta.table', string='Table', required=False, ondelete='cascade')
    ...
    def create(self, vals):
        res = super(setaAnalysis, self).create(vals)
        for analysis in res:
    ...
    def get_analysis_data_model(self, **kwargs):
        ...
        for metric in self.metric_ids:
            ...
            for character in special_characters:
                metric_alias = metric_alias.replace(character, '')
            # Megha end
            # metric_alias = "%s of %s" % (metric.calculation.title(), metric.field_id.field_name)
...
class setaAnalysisMetric(models.Model):
    ...
    field_id = fields.Many2one('seta.table.field', string='Field', required=True)
    ...
    def create(self, vals):
        ...
        for metric in analysis_id.metric_ids:
            if metric.field_id.id == vals.get('field_id'):
                ...
                break
        res = super(setaAnalysisSort, self).create(vals)
        return res

...
class HrTimesheetGenerator(models.Model):
    ...
    attendance_ids = fields.One2many('hr.attendance', 'timesheet_generator_id', string='Attendance Lines',
                                     order='check_in asc')
    attendance_extra_ids = fields.One2many('hr.attendance', 'timesheet_generator_id', string='Attendance Extra Info',
                                           order='check_in asc')
...
class HrAttendanceInherit(models.Model):
    ...
    attendance_presence_status = fields.Selection([
        ('present', 'Present'),
        ('absent_on_leave', 'Absent/On-Leave')
    ], string='Status', tracking=True)
    attendance_date = fields.Date(string='Date', compute='_compute_attendance_date', store=True)
    ...
    def _compute_attendance_date(self):
        for rec in self:
            if rec.check_in:
                tz_name = (rec.employee_id.user_id.tz or rec.env.user.tz) or 'UTC'
                local_dt = fields.Datetime.context_timestamp(rec.with_context(tz=tz_name), rec.check_in)
                rec.attendance_date = local_dt.date()
            else:
                rec.attendance_date = False
    ...
    def _prepare_presence_time_values(self):
        self.ensure_one()
        base_date = self.attendance_date
        if not base_date and self.check_in:
            base_date = fields.Datetime.context_timestamp(self, self.check_in).date()
        if not base_date:
        ...
        if self.attendance_presence_status == 'present':
            local_check_in = datetime(base_date.year, base_date.month, base_date.day, 8, 0, 0)
            local_check_out = datetime(base_date.year, base_date.month, base_date.day, 17, 0, 0)
        else:
            local_check_in = datetime(base_date.year, base_date.month, base_date.day, 0, 0, 0)
            local_check_out = datetime(base_date.year, base_date.month, base_date.day, 0, 0, 0)
        ...
        try:
            ...
            utc_co = local_co.astimezone(pytz.utc)
            return {
                'check_in': fields.Datetime.to_string(utc_ci),
                'check_out': fields.Datetime.to_string(utc_co),
            }
        except Exception:
            return {
                'check_in': fields.Datetime.to_string(local_check_in),
                'check_out': fields.Datetime.to_string(local_check_out),
            }
    ...
    def _compute_is_weekend_or_holiday(self):
        for rec in self:
            ...
            if rec.check_in:
                tz_name = (rec.employee_id.user_id.tz or rec.env.user.tz) or 'UTC'
                dt = fields.Datetime.context_timestamp(rec.with_context(tz=tz_name), rec.check_in)
                # Check weekend
                if dt.weekday() >= 5:
                    is_special = True
                else:
    ...
    def write(self, vals):
        ...
        if 'attendance_presence_status' in vals and not self.env.context.get('skip_presence_status_sync'):
            for rec in self.filtered(lambda record: record.timesheet_generator_id):
                time_vals = rec._prepare_presence_time_values()
                super(HrAttendanceInherit, rec.with_context(skip_presence_status_sync=True)).write(time_vals)
    ...
    def _validate_overtime_and_leave_rules(self):
        ...
        for att in self:
            ...
            if att.is_weekend_or_holiday:
                ...
                if total_hours > 8.0:
                    # requires is_overtime, manager approval and proof letter
                    if not att.is_overtime:
                        raise ValidationError(_("For more than 8 hours, mark 'Is Overtime?' for the line."))
                    if not att.overtime_approved:
                        raise ValidationError(_("Overtime must be approved before logging more than 8 hours."))
                    if not att.approved_document_id:
                        raise ValidationError(_("Please attach the overtime approval letter."))

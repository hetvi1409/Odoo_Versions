...
class HrAttendance(models.Model):
    ...
    overtime_approved = fields.Boolean(string="Overtime Approved", readonly=False, default=False)
    ...
    def _check_overtime_approval(self):
        for rec in self:
            if rec.check_in and rec.check_out:
                ...
                if hours_worked > 8 and (not rec.overtime_approved or not rec.approved_document_id):
    ...
    def _check_validity(self):
        # Bypass this constraint during automated operations
        if self.env.context.get('bypass_attendance_constraints'):
            return
        ...
        for attendance in self:
            ...
            if last_attendance_before_check_in and last_attendance_before_check_in.check_out and \
                ...
                raise ValidationError(_("Cannot create new attendance record for %(empl_name)s, the employee was already checked in on %(datetime)s") % {
                    ...
                    'datetime': fields.Datetime.to_string(attendance.check_in),
                })
            ...
            if not attendance.check_out:
                ...
                if no_check_out_attendances:
                    raise ValidationError(_("Cannot create new attendance record for %(empl_name)s, the employee hasn't checked out since %(datetime)s") % {
                        'empl_name': attendance.employee_id.name,
                        'datetime': fields.Datetime.to_string(no_check_out_attendances.check_in),
                    })
            else:
                ...
                if last_attendance_before_check_out and last_attendance_before_check_in != last_attendance_before_check_out:
                    raise ValidationError(_("Cannot create new attendance record for %(empl_name)s, the employee was already checked in on %(datetime)s") % {
                        'empl_name': attendance.employee_id.name,
                        'datetime': fields.Datetime.to_string(last_attendance_before_check_out.check_in),
                    })
    ...
    def create(self, vals_list):
        ...
        for rec in records:
            ...
            context = {}
            limit = float(self.env['ir.config_parameter'].sudo().get_param('auto_attendance_flow.ot_time_limit', default='600'))
            ...
            if emp_attendances:
                ...
                context.update({'total_hours': total_hours or 0.0})

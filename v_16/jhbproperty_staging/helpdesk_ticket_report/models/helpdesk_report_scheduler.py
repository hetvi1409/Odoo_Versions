...
class HelpdeskReportScheduler(models.Model):
    ...
    def _generate_and_send_report(self, template_xml_id, report_ref, report_type, data):
        ...
        wizard = self.env['helpdesk.ticket.report.wizard'].create({
            ...
            'end_date': datetime.strptime(data.get('end_date'), '%Y-%m-%d').strftime('%Y-%m-%d') if data.get('end_date') else '',
        ...
        # Send email with attachment without mutating the template's static attachments.
        mail_template.with_context(today=date.today()).send_mail(
            wizard.id,
            force_send=True,
    ...
    def send_weekly_all_tickets_report(self):
        """Send weekly report of all tickets created in the last 7 days."""
        today = date.today()
    ...
    def send_monthly_by_department_report(self):
        """Send month-to-date report grouped by department."""
        today = date.today()

...
class building(models.Model):
    ...
    is_disposed = fields.Boolean(string='Is Disposed', compute='_compute_is_disposed', store=False)

    @api.depends('active')
    def _compute_is_disposed(self):
        """Check if building has an approved removal approval record"""
        for record in self:
            removal = self.env['building.removal.approval'].search([
                ('building_id', '=', record.id),
                ('state', '=', 'approved')
            ], limit=1)
            record.is_disposed = bool(removal)
    ...
    def action_remove_asset(self):
        """Fallback removal action so object button validation always succeeds."""
        self.ensure_one()
        if 'building.removal.approval' not in self.env:
            raise UserError(_('Building removal approval model is not available.'))

...
class RecruitmentRequisitionInterviewWizard(models.TransientModel):
    ...
    @api.depends('application_id.applicant_id')
    def _compute_is_pool_applicant(self):
        # v16: hr.applicant doesn't have is_pool_applicant; the backport uses
        # applicant_id (Many2one to applicant.profile) to identify pool entries.
        for rec in self:

...
class RecruitmentOffer(models.Model):
    ...
    job_id = fields.Many2one('hr.job', related='requisition_id.job_id', store=True, readonly=True)
    interview_session_id = fields.Many2one('recruitment.interview.session')
    applicant_id = fields.Many2one('hr.applicant', required=True, domain="[('job_id', '=', job_id)]")

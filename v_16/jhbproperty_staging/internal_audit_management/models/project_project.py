...
class ProjectProject(models.Model):
    ...
    upload_audit_document_ids = fields.Many2many('ir.attachment', 'project_audit_document_rel', 'project_id',
                                                 'document_id', string="Attachment")
    ...
    feedback = fields.Char(string="Audit Reverts / Review Notes",
                           tracking=True)
    share_doc_ids = fields.Many2many('documents.document', string='Share Document', compute='_compute_share_doc_ids',
                                     store=False)

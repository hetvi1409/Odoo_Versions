...
class QualityControlInternalPeerReview(models.Model):
    """Quality control - Internal Peer review """
    _name = 'quality.control.peer'
    _description = "Quality Control - Internal Peer Review "
    _rec_name = 'name'
    ...
    attachment_ids = fields.Many2many('ir.attachment',
                                      string="Upload Internal Peer Review Reports")
    ...
    quality_control_peer_tracking_ids = fields.One2many('quality.control.peer.tracking', 'quality_control_peer_tracking_id', string='Tracking')
    document_ids = fields.Many2many('ir.attachment',
                                    'documents_attachment_peer_quality_rel',
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            vals['sequence_no'] = self.env['ir.sequence'].next_by_code('quality.control.peer')

        records = super(QualityControlInternalPeerReview, self).create(vals_list)
        ...
        for record, vals in zip(records, vals_list):
            ...
            # Sync attachments/documents if present
    ...
    def write(self, vals):
        ...
        res = super(QualityControlInternalPeerReview, self).write(vals)
    ...
    def _create_folder(self, name):
        ...
        documents_folder_id = self.env.ref(
            'internal_audit_management.documents_internal_peer_review_folder').id
    ...
    def unlink(self):
        ...
        return super(QualityControlInternalPeerReview, self).unlink()
    ...
    def get_list_url(self):
        ...
        Urls = urls.url_join(base_url,
                             'web#id=%s&model=quality.control.peer&view_type=list' % self.id)
        return Urls
    ...
    def action_review(self):
        ...
        for record in self:
            self.env['quality.control.peer.tracking'].create({
                'quality_control_peer_tracking_id': record.id,
        ...
        mail_template = self.env.ref(
            'internal_audit_management.email_template_quality_control_peer_review')
    ...
    def action_approve(self):
        ...
        mail_template = self.env.ref(
            'internal_audit_management.email_template_quality_control_peer_approve')
        mail_template.send_mail(self.id, force_send=True)
    ...
    def action_mark_discrepancy(self):
        ...
        mail_template = self.env.ref(
            'internal_audit_management.email_template_quality_control_peer_refuse')
        mail_template.send_mail(self.id, force_send=True)
    ...
    def action_send_back_review(self):
        ...
        return {
            ...
            'context': {
                'default_quality_control_peer_id': self.id
            },
            'view_mode': 'form',
            'target': 'new',
        }
    ...
    def action_send_back_new(self):
        ...
        mail_template = self.env.ref(
            'internal_audit_management.email_template_quality_control_peer_new')
        mail_template.send_mail(self.id, force_send=True)
...
class QualityControlOutsourcedPeerReviews(models.Model):
    """Quality control - Internal Peer review """
    _name = 'quality.control.outsourced.peer'
    _description = "Quality Control - Outsourced Peer Reviews"
    _rec_name = "name"
    _inherit = ['mail.thread', 'mail.activity.mixin']
    ...
    state = fields.Selection([('preparer', 'Preparer'),
        ...
        ('rejected', 'Rejected')],
                             default="preparer", string="State")
    attachment_ids = fields.Many2many('ir.attachment',
                                      string="Upload Outsourced peer review documents")
    requirements = fields.Html(string="Requirements", required=True,
                               help="Requirements")
    name = fields.Char(string="Name", required=True,
                               help="Name")
    ...
    user_id = fields.Many2one('res.users', tracking=True,
                              string="Manager")
    feedback = fields.Char(string="Feedback", tracking=True)
    quality_control_outsourced_peer_tracking_ids = fields.One2many('quality.control.outsourced.peer.tracking', 'quality_control_outsourced_peer_tracking_id', string='Tracking')
    # comment_ids = fields.One2many('audit.comments', 'outsourced_peer_id',
    #                               tracking=True, string="Comments")
    document_ids = fields.Many2many('ir.attachment',
                                    'documents_attachment_outsourced_rel',
                                    string="Upload Documents")
    user_ids = fields.Many2many('res.users', string="Users",
                                compute="_compute_user_ids")
    user_preparer_ids = fields.Many2many('res.users', 'preparer_out_peer_rel',
                                         required=True,
                                         string="Preparer", tracking=True,
                                         default=lambda self: self.env.user)
    user_reviewer_1_ids = fields.Many2many('res.users',
                                           'first_reviewers_out_peer_rel',
                                           string="First Reviewer",
                                           tracking=True)
    user_reviewer_2_ids = fields.Many2many('res.users',
                                           'second_reviewers_out_peer_rel',
                                           string="Second Reviewer",
                                           tracking=True)
    user_approver_ids = fields.Many2many('res.users', 'approvers_out_peer_rel',
    ...
    @api.model_create_multi
    def create(self, vals_list):
        """Create multiple records with sequence and folder/doc sync."""
        records_to_create = []
        for vals in vals_list:
            vals['sequence_no'] = self.env['ir.sequence'].next_by_code('quality.control.outsourced.peer')
            records_to_create.append(vals)
        records = super(QualityControlOutsourcedPeerReviews, self).create(records_to_create)
        for record, vals in zip(records, records_to_create):
            if not record.folder_id:
                folder = record._create_folder(record.name)
                record.folder_id = folder.id
            if 'attachment_ids' in vals:
    ...
    def write(self, vals):
        existing_attachments = self.attachment_ids
        existing_documents = self.document_ids
        res = super(QualityControlOutsourcedPeerReviews, self).write(vals)
        if not self.folder_id:
            folder = self._create_folder(self.name)
            self.folder_id = folder.id
        if 'attachment_ids' in vals:
    ...
    def _create_folder(self, name):
        ...
        folder = self.env['documents.document']
        documents_folder_id = self.env.ref(
            'internal_audit_management.documents_outsourced_peer_review_folder').id
    ...
    def get_list_url(self):
        ...
        base_url = self.env['ir.config_parameter'].sudo().get_param(
            'web.base.url')
        Urls = urls.url_join(base_url,
                             'web#id=%s&model=quality.control.outsourced.peer&view_type=list' % self.id)
        return Urls
    ...
    def action_review(self):
        ...
        mail_template = self.env.ref(
            'internal_audit_management.email_template_quality_control_outsourced_peer_review')
        mail_template.send_mail(self.id, force_send=True)
    ...
    def action_approve(self):
        ...
        mail_template = self.env.ref(
            'internal_audit_management.email_template_quality_control_outsourced_peer_approve')
        mail_template.send_mail(self.id, force_send=True)
    ...
    def action_mark_discrepancy(self):
        ...
        mail_template = self.env.ref(
            'internal_audit_management.email_template_quality_control_outsourced_peer_refuse')
        mail_template.send_mail(self.id, force_send=True)
    ...
    def action_send_back_review(self):
        ...
        return {
            ...
            'res_model': 'internal.audit.revert.wizard',
            'context': {
                'default_quality_control_outsourced_peer_id': self.id
            },
            'view_mode': 'form',
            'target': 'new',
        }
    ...
    def action_send_back_new(self):
        ...
        mail_template = self.env.ref(
            'internal_audit_management.email_template_quality_control_outsourced_peer_new')
        mail_template.send_mail(self.id, force_send=True)

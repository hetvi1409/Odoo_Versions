...
class ClientTransaction(models.Model):
    ...
    def create(self, values):
        ...
        if values.get('name', _('New')) == _('New'):
            values['name'] = self.env['ir.sequence'].next_by_code(
                'client.transaction') or _('New')
        res =  super(ClientTransaction, self).create(values)
    ...
    def action_send_for_signing(self):
        ...
        attachment = self.transaction_attachment_ids[0]
        ...
        sign_request = self.env['sign.request'].create({
            ...
            'subject': 'Signature Request - ' + attachment.name,
